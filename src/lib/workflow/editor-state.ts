...
import { getExpressionIssues } from "@/lib/workflow/expression-suggestions"
...
export function buildWorkflowFingerprint(input: { title: string; summary: string; definition: WorkflowDefinition }) {
...
export function getWorkflowDesignIssues(input: {
  ...
  if (startNodes.length === 0) {
    issues.push({
      ...
      label: "Workflow",
      message: "Add exactly one start node before publishing or running the workflow.",
      severity: "error",
    })
  }
  if (startNodes.length > 1) {
    issues.push({
      nodeId: startNodes[1]?.id ?? startNodes[0]?.id ?? "workflow",
      label: "Workflow",
      message: "Keep only one active start node in the workflow graph.",
      severity: "error",
    })
  }
    ...
    .flatMap((node) => {
      ...
      if (node.data.kind !== "start" && (inboundCount.get(node.id) ?? 0) === 0) {
        nodeIssues.push({
          nodeId: node.id,
          label,
          message: "Node is disconnected from the upstream workflow path.",
          severity: "error",
        })
      }
      if (node.data.kind !== "end" && (outboundCount.get(node.id) ?? 0) === 0) {
        nodeIssues.push({
          nodeId: node.id,
          label,
          message: "Node has no downstream path or terminal step.",
          severity: "error",
        })
      }
      ...
      if (cyclicNodes.has(node.id)) {
        nodeIssues.push({
          nodeId: node.id,
          label,
          message: "Node is part of an invalid cyclic loop graph.",
          severity: "error",
        })
      }
      ...
      if (node.data.kind === "agent" && node.data.agentId?.trim() && !availableAgentIds.includes(node.data.agentId)) {
        nodeIssues.push({
          nodeId: node.id,
          label,
          message: "Agent binding no longer exists in the workspace.",
          severity: "error",
        })
      }
      if (node.data.kind === "agent" && node.data.modelId?.trim() && !availableModelIds.includes(node.data.modelId)) {
        nodeIssues.push({
          nodeId: node.id,
          label,
          message: "Model override no longer exists in the configured provider catalog.",
          severity: "error",
        })
      }
      if (node.data.kind === "http" && !node.data.integrationId?.trim() && !node.data.url?.trim()) {
        nodeIssues.push({
          nodeId: node.id,
          label,
          message: "HTTP node needs a bound integration or URL.",
          severity: "error",
        })
      }
      if (
        ...
        !availableIntegrationIds.includes(node.data.integrationId)
      ) {
        nodeIssues.push({
          nodeId: node.id,
          label,
          message: "Bound integration no longer exists in the workspace.",
          severity: "error",
        })
      }
      if (node.data.kind === "document-retrieval" && !node.data.documentId?.trim()) {
        nodeIssues.push({
          nodeId: node.id,
          label,
          message: "Document retrieval node has no source document.",
          severity: "error",
        })
      }
      if (
        ...
        !availableDocumentIds.includes(node.data.documentId)
      ) {
        nodeIssues.push({
          nodeId: node.id,
          label,
          message: "Bound document no longer exists in the workspace.",
          severity: "error",
        })
      }
      ...
      if (node.data.kind === "variable-assigner" && !node.data.variableName?.trim()) {
        nodeIssues.push({
          nodeId: node.id,
          label,
          message: "Variable assignment needs a variable name.",
          severity: "error",
        })
      }
      ...
      if (node.data.kind === "loop" && !node.data.loopExpression?.trim()) {
        nodeIssues.push({
          nodeId: node.id,
          label,
          message: "Loop node needs a collection expression.",
          severity: "error",
        })
      }
      ...
      const branchExpressions =
        node.data.kind === "if-else"
          ? (node.data.branches ?? []).filter(
              (branch): branch is { id: string; label: string; expression: string } =>
                typeof branch?.expression === "string",
            )
          : []
      if (
        node.data.kind === "if-else" &&
        !(node.data.runIf?.trim() || branchExpressions.some((branch) => branch.expression.trim()))
      ) {
        nodeIssues.push({
          nodeId: node.id,
          label,
      ...
      for (const [value, fieldLabel] of [
        [node.data.headersJson, "Headers"],
        [node.data.queryJson, "Query parameters"],
        [node.data.bodyJson, "Body"],
        [node.data.inputSchemaJson, "Input schema"],
        [node.data.outputSchemaJson, "Output schema"],
      ] as Array<[string | undefined, string]>) {
        if (value?.trim()) {
          const issue =
            fieldLabel === "Input schema" || fieldLabel === "Output schema"
              ? getWorkflowSchemaIssue(value, fieldLabel)
              : getWorkflowJsonIssue(value, fieldLabel)
          if (issue) nodeIssues.push({ nodeId: node.id, label, message: issue, severity: "error" })
        }
      }

      const expressionFields: Array<[string | undefined, string, boolean]> = [
        [node.data.prompt, "Prompt", false],
        [node.data.systemPrompt, "System instructions", false],
        [node.data.url, "URL", false],
        [node.data.headersJson, "Headers", false],
        [node.data.queryJson, "Query parameters", false],
        [node.data.bodyJson, "Body", false],
        [node.data.queryExpression, "Search question", false],
        [node.data.variableValue, "Variable value", false],
        [node.data.template, "Template body", false],
        [node.data.inputTemplate, "Result template", false],
        [node.data.runIf, "Condition expression", false],
        [node.data.loopExpression, "Collection expression", false],
        [node.data.emailTo, "Email recipient", false],
        [node.data.emailSubject, "Email subject", false],
        [node.data.emailBody, "Email message", false],
      ]
      if (node.data.kind === "if-else") {
        for (const branch of branchExpressions)
          expressionFields.push([branch.expression, `Branch "${branch.label}" expression`, false])
      }
      try {
        const outputSchema = JSON.parse(node.data.outputSchemaJson || "{}") as {
          properties?: Record<string, { default?: unknown }>
        }
        for (const [name, property] of Object.entries(outputSchema.properties ?? {}))
        ...
        /* JSON validation above reports malformed output schemas. */
      }
      for (const [value, fieldLabel, allowCurrent] of expressionFields) {
        for (const message of getExpressionIssues(activeNodes, validEdges, node.id, value, fieldLabel, allowCurrent))
          nodeIssues.push({ nodeId: node.id, label, message, severity: "error" })
      }
...
const WORKFLOW_SCHEMA_TYPES = new Set(["string", "number", "integer", "boolean", "object", "array", "null"])

export function getWorkflowSchemaIssue(value: string, label: string) {
  let root: unknown
  try {
  ...
  if (!root || typeof root !== "object" || Array.isArray(root)) return `${label} must be a JSON Schema object.`
  const visit = (schema: unknown, path: string): string | null => {
    if (!schema || typeof schema !== "object" || Array.isArray(schema)) return `${path} must be an object.`
    const record = schema as Record<string, unknown>
    if (record.type !== undefined && (typeof record.type !== "string" || !WORKFLOW_SCHEMA_TYPES.has(record.type)))
    ...
    if (record.properties !== undefined) {
      ...
      for (const [name, child] of Object.entries(record.properties)) {
        const issue = visit(child, `${path}.properties.${name}`)
        if (issue) return issue
      }
    }
    if (record.items !== undefined) {
      const issue = visit(record.items, `${path}.items`)
      if (issue) return issue
    }
    if (
      record.required !== undefined &&
      (!Array.isArray(record.required) || record.required.some((name) => typeof name !== "string"))
    )
      return `${path}.required must be an array of field names.`
    const properties = record.properties as Record<string, unknown> | undefined
    if (properties && Array.isArray(record.required) && record.required.some((name) => !(name in properties)))
      return `${path}.required can only contain declared properties.`
    return null
  }
  return visit(root, label)
}

export function getAutoLayoutedWorkflowNodes(input: { nodes: Node<WorkflowNodeData>[]; edges: Edge[] }) {
  ...
  for (const depth of [...orderedDepths].reverse()) {
    for (const nodeId of levels.get(depth) ?? []) {
      const children = (adjacency.get(nodeId) ?? [])
        .map((childId) => ({ node: nodeById.get(childId), position: positions.get(childId) }))
        .filter((item): item is { node: Node<WorkflowNodeData>; position: { x: number; y: number } } =>
          Boolean(item.node && item.position),
        )
      const position = positions.get(nodeId)
      if (!position || children.length === 0) continue
      const left = Math.min(...children.map((item) => item.position.x))
      const right = Math.max(
        ...children.map((item) => item.position.x + (item.node.measured?.width ?? item.node.width ?? 256)),
      )
    ...
    let minimumX = 120
    for (const nodeId of [...(levels.get(depth) ?? [])].sort(
      (left, right) => (positions.get(left)?.x ?? 0) - (positions.get(right)?.x ?? 0),
    )) {
      const position = positions.get(nodeId)
      const node = nodeById.get(nodeId)
