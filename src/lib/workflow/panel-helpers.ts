...
export function hasOutgoingWorkflowConnection(edges: Edge[], nodeId: string, sourceHandle: string | null) {
  return edges.some(
    (edge) =>
      edge.source === nodeId && (sourceHandle === null ? !edge.sourceHandle : edge.sourceHandle === sourceHandle),
  )
}
...
export function buildConnectedWorkflowNode(input: {
  ...
  let nextId = `${kind}-${nextIndex}`
  let suffix = nextIndex
  while (nodes.some((node) => node.id === nextId)) nextId = `${kind}-${++suffix}`
  ...
  const branch =
    sourceNode.data.kind === "if-else" ? sourceNode.data.branches?.find((item) => item.id === sourceHandle) : undefined
