import type { WorkflowNodeData, WorkflowNotificationSettings } from "@/types/workflow"
...
export const workflowPresetNodes: Array<{
  ...
  { kind: "condition", label: "Condition", summary: "Continue only when an expression resolves to true." },
  { kind: "if-else", label: "If / Else", summary: "Route execution based on expressions." },
  { kind: "fork", label: "Fork", summary: "Split control flow into named branches." },
  { kind: "join", label: "Join", summary: "Merge branch results before continuing." },
  ...
  { kind: "parallel", label: "Parallel", summary: "Run downstream work as a grouped concurrent stage." },
  { kind: "serial", label: "Serial", summary: "Group a deterministic sequential stage." },
...
export const defaultWorkflowNotifications: WorkflowNotificationSettings = {
  enabled: false,
  to: "",
  ...
  switch (kind) {
    ...
    case "end":
      return {
        ...base,
        label: "End",
        task: "Finalize and return the workflow result.",
    ...
    case "agent":
      return {
        ...base,
        task: "Run the selected agent with the current workflow context.",
    ...
    case "template":
      return {
        ...base,
        label: "Template",
        task: "Render a text template from workflow context.",
        outputKey: "rendered",
      }
    case "ai-response":
      return {
        ...base,
        label: "AI response",
        task: "Generate a model response from the current context.",
        outputKey: "response",
      }
    case "loop":
    ...
    case "webhook":
      return {
        ...base,
        label: "Webhook",
        task: "Deliver the current context to an HTTP endpoint.",
        outputKey: "webhook_result",
      }
    case "wiki-retrieval":
