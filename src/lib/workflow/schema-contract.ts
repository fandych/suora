import type { WorkflowInputParameter } from "@/types/workflow"
...
export function readWorkflowSchemaParameters(value?: string): WorkflowSchemaParameter[] {
  ...
  return Object.entries(schema.properties ?? {}).map(([name, property], index) => ({
    ...
    type: types.has(property.type as WorkflowInputParameter["type"])
      ? (property.type as WorkflowInputParameter["type"])
      : "string",
    defaultValue:
