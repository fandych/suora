...
export function getWorkflowExpressionSuggestions(
  nodes: Node<WorkflowNodeData>[],
  edges: Edge<WorkflowEdgeData>[],
  selectedNodeId: string,
  options: ExpressionSuggestionOptions = {},
  ...
  if (options.includeCurrent) {
    suggestions.push({
      ...
      description: "The complete raw result of this node.",
    })
    suggestions.push(
      ...getSchemaSuggestions(
        selectedNode?.data.outputSchemaJson,
        "current",
        "Current result",
  ...
  suggestions.push(
    ...getSchemaSuggestions(
      startNode?.data.inputSchemaJson,
      "input",
      "Workflow input",
      "Supplied when the workflow starts.",
    ),
  )
  ...
  for (const node of nodes) {
    ...
    suggestions.push({
      expression: `\${steps.${node.id}}`,
      group: "Prior steps",
      description: `Complete result from ${node.id}.`,
    })
    suggestions.push(
      ...getSchemaSuggestions(node.data.outputSchemaJson, `steps.${node.id}`, "Prior steps", `From ${node.id}.`),
    )
    if (node.data.outputKey)
      suggestions.push({
        expression: `\${vars.${node.data.outputKey}}`,
        group: "Variables",
        description: `Result alias from ${node.id}.`,
      })
    if (node.data.kind === "variable-assigner" && node.data.variableName)
      suggestions.push({
        expression: `\${vars.${node.data.variableName}}`,
        group: "Variables",
        description: `Variable assigned by ${node.id}.`,
      })
  }
...
function hasDeclaredPath(schemaJson: string | undefined, path: string) {
  ...
  return (
    ...
    fields.some(
      (field) =>
        field === path || field.startsWith(`${path}.`) || path.startsWith(`${field}.`) || field.startsWith(`${path}[`),
    )
  )
}

export function getExpressionIssues(
  nodes: Node<WorkflowNodeData>[],
  edges: Edge<WorkflowEdgeData>[],
  nodeId: string,
  ...
  for (const rawExpression of expressions) {
    ...
    if (parts[0] === "steps") {
      ...
      else if (!upstreamNodeIds.has(target.id))
        issues.push(`${fieldLabel}: step "${target.id}" is not upstream of this node.`)
      else if (parts.length > 2 && !hasDeclaredPath(target.data.outputSchemaJson, parts.slice(2).join(".")))
        issues.push(`${fieldLabel}: "${parts.slice(2).join(".")}" is not declared by step "${target.id}".`)
    } else if (
      parts[0] === "input" &&
      parts.length > 1 &&
      !hasDeclaredPath(inputNode?.data.inputSchemaJson, parts.slice(1).join("."))
    ) {
