...
export const chatIpc = {
  list: async () => (await getProjectBridge().chats.list() as RawChatSummaryRow[]).map(parseSummary),
  get: async (chatId: string) => parseDetail(await getProjectBridge().chats.get(chatId) as ChatPayload),
  create: async () => parseDetail(await getProjectBridge().chats.create() as ChatPayload) as ChatDetail,
  ensure: async (payload: { chatId: string; title: string; chatbotId: string; summary?: string; sourceType?: "manual" | "channel"; sourceRef?: string | null }) => parseDetail(await getProjectBridge().chats.ensure(payload) as ChatPayload),
  delete: async (chatId: string) => getProjectBridge().chats.delete(chatId) as Promise<boolean>,
  appendUser: async (chatId: string, content: string, parts?: ChatMessagePart[]) => parseDetail(await getProjectBridge().chats.appendUser({ chatId, content, parts }) as ChatPayload | null),
  appendAssistant: async (chatId: string, content: string, parts?: ChatMessagePart[]) => parseDetail(await getProjectBridge().chats.appendAssistant({ chatId, content, parts }) as ChatPayload | null),
  updateMessageParts: async (chatId: string, messageId: string, parts: ChatMessagePart[]) => parseDetail(await getProjectBridge().chats.updateMessageParts({ chatId, messageId, parts }) as ChatPayload | null),
  getSettings: async () => getProjectBridge().chats.getSettings(),
  saveSettings: async (payload: unknown) => getProjectBridge().chats.saveSettings(payload),
}
