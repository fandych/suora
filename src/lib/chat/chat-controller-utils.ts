...
import type { AssistantResponsePart, ChatAttachment, ChatRuntimeSettings } from "@/types/chat"
...
export async function fileToChatAttachment(file: File): Promise<ChatAttachment> {
  ...
  if (
    file.type &&
    !["image/", "text/", "application/pdf", "application/json"].some((prefix) => file.type.startsWith(prefix))
  ) {
...
export function createPersistedUserPayload(draft: string, attachments: ChatAttachment[]) {
  ...
  return {
    content:
      trimmedDraft ||
      attachments.map((attachment) => buildAttachmentSummary(toPersistedAttachmentRecord(attachment))).join("\n"),
    parts,
  }
}
...
export function buildChatTranscript(selectedChat: ChatDetail | null, assistantResponseParts: AssistantResponsePart[]) {
  const messageBlocks = (selectedChat?.messages ?? [])
    .map((message) => `${message.role.toUpperCase()}\n${buildChatMessageDisplayText(message)}`)
    .join("\n\n")
