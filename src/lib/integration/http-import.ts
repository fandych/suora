import type { HttpEndpointConfig, HttpEndpointParameter, HttpIntegrationAuthType } from "@/types/integration"
import {
  createHttpEndpoint,
...
export function parseCurlImport(command: string): CurlImportResult {
  ...
  for (let index = 1; index < tokens.length; index += 1) {
    ...
    if ((token === "-X" || token === "--request") && next) {
      ...
      index += 1
    } else if (["--data", "--data-raw", "--data-binary", "-d"].includes(token) && next) {
      bodyText = next
  ...
  const bodyMode = bodyText
    ? headers["Content-Type"]?.includes("form") || headers["content-type"]?.includes("form")
      ? "form-data"
      : "json"
    : "none"
  return {
...
export function parseOpenApiImport(source: string): OpenApiImportResult {
      ...
      Record<
        string,
        {
          ...
          requestBody?: {
            content?: Record<
              string,
              { schema?: { properties?: Record<string, { type?: string; description?: string }>; type?: string } }
            >
          }
        }
      >
  ...
  const firstSecurityScheme = parsed.components?.securitySchemes
    ? Object.values(parsed.components.securitySchemes)[0]
    : undefined
  const authType: HttpIntegrationAuthType =
    firstSecurityScheme?.type === "apiKey"
      ? "api-key"
  ...
  const endpoints = Object.entries(parsed.paths ?? {}).flatMap(([path, operations]) =>
    Object.entries(operations ?? {}).map(([method, operation]) => {
      ...
      return createHttpEndpoint({
        ...
        bodyJson: bodySchema?.properties
          ? toPrettyJson(Object.fromEntries(Object.keys(bodySchema.properties).map((key) => [key, ""])))
          : "{}",
      })
    }),
  )
