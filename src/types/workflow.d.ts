...
import type { ChatMessageRecord, ChatRuntimeSettings } from "@/types/chat"
import type { DocumentDetail } from "@/types/document"
import type { IntegrationConfig, IntegrationDetail, IntegrationExecutionResult } from "@/types/integration"
...
export type WorkflowNodeData = {
  ...
  trueLabel?: string
  falseLabel?: string
  branches?: Array<{ id: string; label: string; expression: string }>
  variableName?: string
...
export type WorkflowRunMode = "dry-run" | "manual"
export type WorkflowRunStatus = "idle" | "queued" | "running" | "completed" | "failed" | "cancelled"
...
export type WorkflowRunAccepted = {
  accepted: true
  requestId: string
  mode: WorkflowRunMode
  state: "queued"
}
export type WorkflowRunCancelResult = { cancelled: boolean; requestId: string }
...
export type WorkflowChatAgentEvent = {
  type: "text-delta" | "error" | string
  text?: string
  error?: string
  [key: string]: unknown
}
...
export type WorkflowRuntimePorts = {
  ...
  ) => AsyncIterable<WorkflowChatAgentEvent>
