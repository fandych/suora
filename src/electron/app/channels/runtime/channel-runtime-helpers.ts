...
} from "@/electron/app/channels/runtime/channel-runtime-messages"
export type { TokenCacheEntry, WeChatWebhookPayload } from "@/types/channel-runtime"

export * from "@/electron/app/channels/runtime/channel-runtime-http"
export * from "@/electron/app/channels/runtime/channel-runtime-tokens"
export * from "@/electron/app/channels/runtime/channel-runtime-email"
...
export function normalizeWeChatPersonalQrCodeUrl(value?: string) {
  ...
  if (/^[A-Za-z0-9+/=]+$/.test(compactValue)) return `data:image/png;base64,${compactValue}`
...
function extractWeChatPersonalMessageContent(
  items: WeChatPersonalMessageItem[] | undefined,
): { content: string; messageType: RuntimeChannelMessage["messageType"] } | null {
...
export function weChatPersonalMessageToChannelMessage(
