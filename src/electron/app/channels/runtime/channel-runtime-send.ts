...
import { sendCustomMessage, sendTelegramMessage } from "@/electron/app/channels/runtime/channel-runtime-outbound"
import {
...
async function sendDingTalkWebhook(channel: ChannelConfigRecord, content: string) {
  try {
    ...
    const data = response.data as { errcode?: number; errmsg?: string }
    return data.errcode == null || data.errcode === 0
      ? { success: true }
      : { success: false, error: data.errmsg || "Send failed" }
  } catch (error) {
    return { success: false, error: error instanceof Error ? error.message : String(error) }
  }
}
...
async function sendWeChatOfficial(
  channel: ChannelConfigRecord,
  chatId: string,
  content: string,
  tokenCache: Map<string, TokenCacheEntry>,
...
async function sendWeChatMiniProgram(
  channel: ChannelConfigRecord,
  chatId: string,
  content: string,
  tokenCache: Map<string, TokenCacheEntry>,
...
async function sendTeams(
  ...
  if (!channel.teamsAppId || !channel.teamsAppPassword)
    return { success: false, error: "Missing Teams app ID or password" }
  try {
