...
import { fetchNewEmails, isValidWeChatPersonalToken } from "@/electron/app/channels/runtime/channel-runtime-helpers"
...
import {
  pollWeChatPersonalChannel,
  readWeChatPersonalSyncCursor,
  writeWeChatPersonalSyncCursor,
      ...
      if (!context.streamClients.has(id)) {
        ...
        client.onMessage(async (runtimeChannel, message, sessionWebhook) => {
          if (sessionWebhook && message.chatId) {
            context.sessionWebhooks.set(`${runtimeChannel.id}:${message.chatId}`, {
              url: sessionWebhook,
        ...
        void client
          ...
          .catch((error) =>
            appendDebugLog(
              id,
              "error",
              `DingTalk stream connect failed: ${error instanceof Error ? error.message : String(error)}`,
            ),
          )
      } else {
      ...
      if (!context.customSocketClients.has(id)) {
        ...
        try {
          ...
          appendDebugLog(
            id,
            "error",
            `Custom WebSocket connect failed: ${error instanceof Error ? error.message : String(error)}`,
          )
        }
      } else {
...
export async function syncWeChatPersonalPollers(context: SyncContext, emitMessage: EmitMessage) {
  ...
  for (const [id, channel] of context.channels) {
    if (
      ...
      isValidWeChatPersonalToken(channel.wechatPersonalBotToken)
    ) {
      activeIds.add(id)
...
async function runWeChatPersonalPoller(
  context: SyncContext,
  emitMessage: EmitMessage,
  initialChannel: ChannelConfigRecord,
  ...
  try {
    while (!controller.signal.aborted) {
      ...
      try {
        const result = await pollWeChatPersonalChannel(
          channel,
          cursor,
          context.weChatPersonalContextTokens,
          controller.signal,
        )
        ...
        if (
          ...
          (result.response.errcode && result.response.errcode !== 0)
        ) {
          throw new Error(
            result.response.errmsg ||
              `ret=${result.response.ret ?? "n/a"}, errcode=${result.response.errcode ?? "n/a"}`,
          )
        }
...
async function pollEmailChannel(context: SyncContext, emitMessage: EmitMessage, channel: ChannelConfigRecord) {
  try {
    ...
    appendDebugLog(
      channel.id,
      "error",
      `Email polling failed: ${error instanceof Error ? error.message : String(error)}`,
    )
  }
}
