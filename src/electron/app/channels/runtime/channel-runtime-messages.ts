...
import type { WeChatWebhookPayload } from "@/types/channel-runtime"
...
function parseWeChatRecord(get: (tag: string) => unknown): WeChatWebhookPayload {
  return {
    ...
    MediaId: stringValue(get("MediaId")),
    Format: stringValue(get("Format")),
    Recognition: stringValue(get("Recognition")),
    ThumbMediaId: stringValue(get("ThumbMediaId")),
    Location_X: stringValue(get("Location_X")),
    Location_Y: stringValue(get("Location_Y")),
    Label: stringValue(get("Label")),
    Title: stringValue(get("Title")),
    Description: stringValue(get("Description")),
    Url: stringValue(get("Url")),
    Event: stringValue(get("Event")),
    EventKey: stringValue(get("EventKey")),
...
export function buildWeChatSignature(token: string, timestamp: string, nonce: string, encrypted?: string) {
  return crypto
    .createHash("sha1")
    .update((encrypted ? [token, timestamp, nonce, encrypted] : [token, timestamp, nonce]).sort().join(""))
    .digest("hex")
}
...
export function weChatWebhookToChannelMessage(
  ...
  if (payload.MsgType === "image") {
    ...
    messageType = "image"
  } else if (payload.MsgType === "voice") {
    content = payload.Recognition || payload.MediaId || "[Voice]"
    messageType = "voice"
  } else if (payload.MsgType === "video") {
  ...
  else if (payload.MsgType === "link")
    content = `[Link] ${payload.Title || "Link"}${payload.Url ? `: ${payload.Url}` : ""}${payload.Description ? `\n${payload.Description}` : ""}`
  else if (payload.MsgType === "event")
