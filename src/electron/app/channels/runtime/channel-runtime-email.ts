...
import {
  ...
  sendMail,
} from "@/electron/app/channels/mail-service"
export type { ParsedEmail } from "@/types/mail"
...
export async function sendEmailMessage(
  ...
  if (!profile) {
    return {
      success: false,
      error: channel.emailUseGlobalMailService
        ? "Missing global mail service configuration"
...
function decodeEmailSubject(subject: string) {
  return subject.replace(/=\?([^?]+)\?([BQ])\?([^?]+)\?=/gi, (_match, _charset, encoding, text) => {
    ...
    return String(text)
      .replace(/_/g, " ")
      .replace(/=([0-9A-F]{2})/gi, (_inner, hex) => String.fromCharCode(Number.parseInt(hex, 16)))
  })
}
  ...
  for (const action of actions) {
    ...
    try {
      switch (action.type) {
        case "forward":
          if (action.value)
            await sendEmailMessageWithAttachments(
              channel,
              action.value,
              `Fwd: ${email.subject}`,
        ...
        case "auto_reply":
          if (!action.useAgent && action.value) {
            const replyContent = action.value
...
async function sendEmailMessageWithAttachments(
  channel: ChannelConfigRecord,
  toAddress: string,
  subject: string,
  content: string,
