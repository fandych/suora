...
import { replyViaDingTalkSessionWebhook } from "@/electron/app/channels/providers/dingtalk-stream"
import {
...
} from "@/electron/app/channels/runtime/channel-runtime-persistence"
import { sendMessageForChannel } from "@/electron/app/channels/runtime/channel-runtime-send"
import type {
  ChannelHealthStatus,
  RuntimeChannelEvent,
  RuntimeMessageHandler,
} from "@/electron/app/channels/runtime/channel-runtime-types"
import { getChannelDetail, listEnabledChannelDetails } from "@/electron/app/channels/repositories/channel-store"
import { CustomWebSocketClient } from "@/electron/app/channels/providers/custom-websocket-client"
import { DingTalkStreamClient } from "@/electron/app/channels/providers/dingtalk-stream"
import { ChannelMessageQueue } from "@/electron/app/channels/runtime/channel-message-queue"
import {
  stopAllRuntimeClients,
  syncCustomSocketClients,
  syncEmailPollers,
  syncStreamClients,
  syncWeChatPersonalPollers,
} from "@/electron/app/channels/runtime/channel-service-sync"
import {
...
export class ChannelService {
  ...
  async sendMessage(channelId: string, chatId: string, content: string) {
    ...
    if (channel.platform === "dingtalk" && channel.connectionMode === "stream") {
      ...
      if (session && session.expiresAt > Date.now()) {
        ...
        recordOutgoingMessage(channelId, {
          chatId,
          senderId: channel.replyAgentId || "suora",
          senderName: channel.replyAgentId || "SUORA",
          content,
    ...
    const result = await sendMessageForChannel(
      channel,
      chatId,
      content,
      tokenCache,
      this.weChatPersonalContextTokens.get(`${channelId}:${chatId}`),
    )
    recordOutgoingMessage(channelId, {
      chatId,
      senderId: channel.replyAgentId || "suora",
    ...
    try {
      if (channel.platform === "wechat_personal") {
        if (channel.wechatPersonalBotToken) {
          ...
          const result = {
            ...
            error: healthy ? undefined : "Personal WeChat QR binding is not complete",
          }
      ...
      if (channel.platform === "feishu") {
        await getFeishuAccessToken(
          channel.feishuAppId || channel.appId || "",
          channel.feishuAppSecret || channel.appSecret || "",
          tokenCache,
        )
      } else if (channel.platform === "dingtalk") {
        await getDingTalkAccessToken(
          channel.dingtalkClientId || channel.appId || "",
          channel.dingtalkClientSecret || channel.appSecret || "",
          tokenCache,
        )
        ...
        await getWeChatOfficialAccessToken(
          channel.wechatOfficialAppId || channel.appId || "",
          channel.wechatOfficialAppSecret || channel.appSecret || "",
          tokenCache,
        )
      } else if (channel.platform === "wechat_miniprogram") {
        await getWeChatMiniProgramAccessToken(
          channel.wechatMiniProgramAppId || channel.appId || "",
          channel.wechatMiniProgramAppSecret || channel.appSecret || "",
          tokenCache,
        )
  ...
  async waitForWeChatPersonalLogin(
    channelId: string | undefined,
    sessionKey: string,
    verifyCode?: string,
    timeoutMs?: number,
  ) {
    return waitForWeChatPersonalLogin(
      this.weChatPersonalLoginSessions,
