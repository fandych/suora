...
export async function sendCustomMessage(channel: ChannelConfigRecord, chatId: string, content: string) {
  ...
  if (!endpoint)
    return {
      success: false,
      error: channel.connectionMode === "stream" ? "Missing custom WebSocket URL" : "Missing custom webhook URL",
    }
  if (channel.connectionMode === "stream")
    return sendCustomWebSocketMessage(
      endpoint,
      channel.customWebsocketProtocol,
...
async function sendCustomWebSocketMessage(
  endpoint: string,
  ...
  try {
    ...
    const payload = applyMessageTemplate(
      template?.trim() || '{"chatId":"{{chatId}}","content":"{{content}}"}',
      chatId,
      content,
    )
