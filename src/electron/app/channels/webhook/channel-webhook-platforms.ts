...
import type { EmitChannelMessage } from "@/electron/app/channels/webhook/channel-webhook-dispatch"
  ...
  if (body.header && (body.header as Record<string, unknown>).event_type === "im.message.receive_v1") {
    ...
    await emitMessage(
      channel,
      {
        ...
        platform: "feishu",
        senderId: String(
          ((event.sender as Record<string, unknown>)?.sender_id as Record<string, unknown>)?.user_id || "",
        ),
        senderName: String(
          ((event.sender as Record<string, unknown>)?.sender_id as Record<string, unknown>)?.union_id || "",
        ),
        content,
...
export async function handleWeChatWebhook(
  ...
  if (req.query.echostr) {
    if (
      ...
      timestamp &&
      nonce &&
      !verifyWeChatSignature(token, timestamp, nonce, signature, encrypted)
    ) {
