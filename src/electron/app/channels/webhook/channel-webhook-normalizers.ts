...
export function readGenericMessageFields(body: UnknownRecord) {
  const senderId = String(
    body.senderId || body.sender_id || body.user_id || readNested(body, ["from", "id"]) || "unknown",
  )
  return {
    senderId,
    senderName: String(
      body.senderName || body.sender_name || body.user_name || readNested(body, ["from", "name"]) || senderId,
    ),
