...
export function fetchWeChatPersonalQrCode(localTokenList: string[]) {
  return requestJson<WeChatPersonalQrCodeResponse>(
    WECHAT_PERSONAL_DEFAULT_BASE_URL,
    "ilink/bot/get_bot_qrcode",
    "POST",
    { local_token_list: localTokenList },
    { query: { bot_type: WECHAT_PERSONAL_QR_BOT_TYPE }, timeoutMs: WECHAT_PERSONAL_API_TIMEOUT_MS },
  )
}
export async function pollWeChatPersonalQrStatus(baseUrl: string, qrcode: string, verifyCode?: string) {
  const endpoint = `ilink/bot/get_qrcode_status?qrcode=${encodeURIComponent(qrcode)}${verifyCode?.trim() ? `&verify_code=${encodeURIComponent(verifyCode.trim())}` : ""}`
  try {
    const response = await requestJson<WeChatPersonalQrStatusResponse>(baseUrl, endpoint, "GET", undefined, {
    ...
    return response.status
      ...
      : {
          ...
          diagnosticEvent: "invalid_response" as const,
    ...
    return {
      status: "wait" as const,
      diagnosticEvent:
        error instanceof Error && error.name === "AbortError" ? ("timeout" as const) : ("error" as const),
...
export function fetchWeChatPersonalUpdates(channel: ChannelConfigRecord, cursor: string, signal?: AbortSignal) {
  return requestJson<WeChatPersonalUpdatesResponse>(
    getWeChatPersonalBaseUrl(channel),
    "ilink/bot/getupdates",
    "POST",
    { get_updates_buf: cursor, base_info: { channel_version: "suora", bot_agent: "Suora" } },
    { token: channel.wechatPersonalBotToken, timeoutMs: WECHAT_PERSONAL_LONG_POLL_TIMEOUT_MS, signal },
  )
}
export async function sendWeChatPersonalNativeMessage(
  channel: ChannelConfigRecord,
  chatId: string,
  content: string,
  contextToken?: string,
) {
  if (!channel.wechatPersonalBotToken) return { success: false, error: "Missing personal WeChat bot token" }
  try {
    await requestJson(
      getWeChatPersonalBaseUrl(channel),
      ...
      "POST",
      {
        msg: {
          from_user_id: "",
          to_user_id: chatId,
          client_id: `suora-wechat-${crypto.randomUUID()}`,
          message_type: 2,
          message_state: 2,
