...
export async function startWeChatPersonalLogin(
  ...
  if (!force && existing) {
    logWeChatPersonalSession(
      channelId,
      "info",
      `QR login reused existing session. sessionKey=${existing.sessionKey} startedAt=${existing.startedAt} baseUrl=${existing.currentApiBaseUrl}`,
    )
    return {
      success: true,
  ...
  try {
    ...
    logWeChatPersonalSession(
      channelId,
      "info",
      `Starting WeChat QR login. force=${String(force)} localTokenCount=${localTokens.length} localTokens=${localTokens.map((token) => summarizeToken(token)).join(", ") || "none"}`,
    )
    ...
    if (!qr.qrcode || !qrCodeUrl) {
      logWeChatPersonalSession(
        channelId,
        "error",
        `QR generation returned incomplete payload. hasQrcode=${String(Boolean(qr.qrcode))} hasImage=${String(Boolean(qr.qrcode_img_content))}`,
      )
    ...
    sessions.set(sessionKey, {
      ...
      qrcodeUrl: qrCodeUrl,
      startedAt: Date.now(),
      currentApiBaseUrl: getWeChatPersonalBaseUrl({
        id: "",
        title: "",
    ...
    logWeChatPersonalSession(
      channelId,
      "info",
      `QR session created. sessionKey=${sessionKey} qrcodeLength=${qr.qrcode.length} qrcodeUrlKind=${qrCodeUrl.startsWith("data:") ? "data-url" : "remote-url"} baseUrl=${sessions.get(sessionKey)?.currentApiBaseUrl || "unknown"}`,
    )
    ...
    logWeChatPersonalSession(
      channelId,
      "error",
      `Failed to start WeChat QR login: ${error instanceof Error ? error.message : String(error)}`,
    )
...
export async function waitForWeChatPersonalLogin(
  ...
  if (!session) {
    logWeChatPersonalSession(
      channelId,
      "error",
      `QR login wait requested without active session. sessionKey=${sessionKey}`,
    )
  ...
  if (!isFreshWeChatPersonalLogin(session)) {
    ...
    logWeChatPersonalSession(
      channelId,
      "error",
      `QR login session expired before polling. sessionKey=${sessionKey} startedAt=${session.startedAt}`,
    )
  ...
  if (verifyCode?.trim()) {
    ...
    logWeChatPersonalSession(
      channelId,
      "info",
      `Verification code submitted for QR login. sessionKey=${sessionKey} codeLength=${verifyCode.trim().length}`,
    )
  }
  ...
  logWeChatPersonalSession(
    channelId,
    "info",
    `Begin polling WeChat QR login. sessionKey=${sessionKey} timeoutMs=${timeoutMs} baseUrl=${session.currentApiBaseUrl} hasPendingVerifyCode=${String(Boolean(session.pendingVerifyCode))}`,
  )
  while (Date.now() < deadline) {
    try {
      const status = await pollWeChatPersonalQrStatus(
      ...
      switch (status.status) {
        ...
        case "scaned_but_redirect":
          if (status.redirect_host) {
            ...
            logWeChatPersonalSession(
              channelId,
              "info",
              `QR poll redirected to new host. sessionKey=${sessionKey} redirectHost=${status.redirect_host} nextBaseUrl=${session.currentApiBaseUrl}`,
            )
          }
          return {
            success: true,
        ...
        case "expired": {
          ...
          if (qr.qrcode && qrCodeUrl) {
            ...
            session.currentApiBaseUrl = getWeChatPersonalBaseUrl({
              id: "",
              title: "",
              platform: "wechat_personal",
              enabled: false,
            ...
            logWeChatPersonalSession(
              channelId,
              "info",
              `QR login expired and refreshed. sessionKey=${sessionKey} newBaseUrl=${session.currentApiBaseUrl}`,
            )
            return {
              success: true,
              status: "expired",
          ...
          logWeChatPersonalSession(
            channelId,
            "error",
            `QR login expired and refresh failed. sessionKey=${sessionKey} hasQrcode=${String(Boolean(qr.qrcode))} hasImage=${String(Boolean(qr.qrcode_img_content))}`,
          )
          return {
            success: false,
            status: "expired",
        ...
        case "confirmed":
          ...
          logWeChatPersonalSession(
            channelId,
            "success",
            `QR login confirmed. sessionKey=${sessionKey} accountId=${status.ilink_bot_id || "none"} userId=${status.ilink_user_id || "none"} baseUrl=${status.baseurl || session.currentApiBaseUrl}`,
          )
          return {
            success: true,
            status: "connected",
        ...
        default:
          logWeChatPersonalSession(
            channelId,
            "error",
            `QR poll returned unhandled status. sessionKey=${sessionKey} upstreamStatus=${status.status || "missing"}`,
          )
          break
      }
    } catch (error) {
      logWeChatPersonalSession(
        channelId,
        "error",
        `QR login poll threw exception. sessionKey=${sessionKey} error=${error instanceof Error ? error.message : String(error)}`,
      )
      return {
        success: false,
        status: "error",
  ...
  logWeChatPersonalSession(
    channelId,
    "info",
    `QR login wait timed out without terminal state. sessionKey=${sessionKey} baseUrl=${session.currentApiBaseUrl}`,
  )
  return {
    success: true,
    status: "timeout",
    ...
    diagnosticMessage: `No terminal QR status received within ${timeoutMs}ms.`,
    pollBaseUrl: session.currentApiBaseUrl,
  }
}
...
export async function pollWeChatPersonalChannel(
  ...
  return {
    nextCursor:
      response.get_updates_buf && response.get_updates_buf !== currentCursor ? response.get_updates_buf : currentCursor,
    timeoutMs:
      response.longpolling_timeout_ms && response.longpolling_timeout_ms > 0 ? response.longpolling_timeout_ms : 35_000,
    response,
    messages,
  }
}
...
function weChatPersonalItemToMessage(
  message: WeChatPersonalInboundMessage,
  channelId: string,
): RuntimeChannelMessage | null {
  ...
  const timestamp =
    typeof message.create_time_ms === "number" && Number.isFinite(message.create_time_ms)
