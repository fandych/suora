...
export type { FileAccessDecision, PreferenceEnvironmentVariable, PreferenceSettings }
...
export function sanitizePreferenceSettings(
...
export function resolvePreferenceSettings(raw: string | null | undefined) {
  if (!raw) return createDefaultPreferenceSettings()
  try {
...
export function shouldConfirmWorkspaceCommand(
  mode: PreferenceCommandConfirmationMode,
  lastConfirmedDate: string | null | undefined,
  today = getTodayPreferenceDateKey(),
) {
...
export function getFileAccessDecision(
  settings: Pick<PreferenceSettings, "fileAccessPolicy" | "fileAccessDirectories">,
  ...
  if (rules.length === 0) return { allowed: true, matchedRule, normalizedPath }
  if (settings.fileAccessPolicy === "allowlist") return { allowed: Boolean(matchedRule), matchedRule, normalizedPath }
...
export function validateSystemMailSettings(
  settings: Pick<
    PreferenceSettings,
    | "mailServiceEnabled"
    | "mailServerHost"
...
export function hasValidSystemMailSettings(
  settings: Pick<
    PreferenceSettings,
    | "mailServiceEnabled"
    | "mailServerHost"
