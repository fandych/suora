...
import { agentVersions, agents, appMeta } from "@/drizzle/schema"
...
export async function getAgent(agentId: string, selectedVersionId?: string) {
  const database = getDrizzleDatabase()
  ...
  const versions = await database
    ...
    .from(agentVersions)
    .where(eq(agentVersions.agentId, agentId))
    .orderBy(desc(agentVersions.major), desc(agentVersions.minor), desc(agentVersions.createdAt))
...
export async function saveAgent(payload: {
  ...
  await database
    ...
    .onConflictDoUpdate({
      target: agents.id,
      set: { title: payload.title, kind: payload.kind || "custom", summary: payload.summary, updatedAt: now },
    })
  const [draft] = payload.selectedVersionId
    ? await database
        .select()
        .from(agentVersions)
        .where(and(eq(agentVersions.agentId, payload.id), eq(agentVersions.id, payload.selectedVersionId)))
        .limit(1)
    : []
  const [latest] = await database
    .select()
    .from(agentVersions)
    .where(eq(agentVersions.agentId, payload.id))
    .orderBy(desc(agentVersions.major), desc(agentVersions.minor), desc(agentVersions.createdAt))
    .limit(1)
...
export async function getAgentSetting(key: string) {
  const database = getDrizzleDatabase()
  const [row] = await database.select({ value: appMeta.value }).from(appMeta).where(eq(appMeta.key, key)).limit(1)
  return row?.value ?? null
}

export async function saveAgentSetting(key: string, value: unknown) {
  const database = getDrizzleDatabase()
