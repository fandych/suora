...
function applyPipeFilters(value: unknown, pipes: string[]) {
  ...
  for (const pipe of pipes) {
    ...
    else if (trimmed.startsWith("default(") && (current === undefined || current === null || current === ""))
      current = trimmed.slice(8, -1).replace(/^['"]|['"]$/g, "")
  }
  return current
}

export function readPath(context: WorkflowExecutionContext, expression: string): unknown {
  ...
  else if (first === "current" || first === "$current") value = navigateObject(context.current, parts.slice(1))
  else
    value =
      navigateObject(context, parts) ??
      navigateObject(context.vars, parts) ??
      navigateObject(context.steps, parts) ??
      navigateObject(context.input, parts)
...
export function interpolate(value: string | undefined | null, context: WorkflowExecutionContext): string {
  if (!value) return ""
  return value
...
export function mapNodeOutput(
  outputSchemaJson: string | undefined,
  context: WorkflowExecutionContext,
): Record<string, unknown> | undefined {
  try {
    const schema = JSON.parse(outputSchemaJson || "{}") as {
      properties?: Record<string, { default?: unknown; properties?: Record<string, unknown> }>
    }
    ...
    const mapProperties = (
      ...
      const mapped = Object.fromEntries(
        entries.flatMap(([name, property]) => {
          const nested = property.properties
            ? mapProperties(
                property.properties as Record<string, { default?: unknown; properties?: Record<string, unknown> }>,
              )
            : undefined
...
export function combineNodeOutput(rawOutput: unknown, mappedOutput: Record<string, unknown> | undefined): unknown {
  ...
  return rawOutput !== null && typeof rawOutput === "object" && !Array.isArray(rawOutput)
    ? { ...(rawOutput as Record<string, unknown>), ...mappedOutput }
    : { value: rawOutput, ...mappedOutput }
}
