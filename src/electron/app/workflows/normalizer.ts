type WorkflowNodeData = Record<string, unknown> & { kind?: string; label?: string; prompt?: string }

type WorkflowDefinition = {
  nodes: Array<{ type?: string; data: WorkflowNodeData; [key: string]: unknown }>
  edges: Array<{ data?: { condition?: string; successOnly?: boolean }; [key: string]: unknown }>
...
export function normalizeWorkflowDefinition(value: unknown): WorkflowDefinition {
  ...
  return {
    ...
    edges: definition.edges.map((edge) => ({
      ...edge,
      data: { condition: edge.data?.condition ?? "", successOnly: edge.data?.successOnly ?? false },
    })),
