...
export const executeAiResponseNode: WorkflowNodeExecutor = async (node, context) => {
  const runtime = (
    context as typeof context & {
      runtime?: {
        executeAgent: (input: { prompt: string; systemPrompt?: string; modelId?: string }) => Promise<unknown>
      }
    }
  ).runtime
  if (!runtime) throw new Error("Workflow runtime is missing AI execution capability.")
