...
export function startAiFetch(target: WebContents, payload: AiFetchStartPayload) {
  ...
  const headers = { "User-Agent": "SUORA/1.0 (Desktop AI Workbench)", Connection: "close", ...(payload.headers ?? {}) }
  let attemptCount = 0
  let responseBytes = 0
  ...
  const executeRequest = () => {
    ...
    logAiFetch(requestId, "attempt", {
      attempt: attemptCount,
      viaProxy: Boolean(proxyAgent),
      agent: requestAgent.constructor.name,
    })
    const request = transport.request(
      ...
      (response) => {
        ...
        response.on("data", (chunk: Buffer) => {
          ...
          sendAiEvent(target, { requestId, type: "data", chunkBase64: chunk.toString("base64") })
        })
        response.on("end", () => {
