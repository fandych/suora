...
type DiscoverProviderModelsResult = { models: ProviderModelRecord[]; source: string }
...
function inferCapabilities(model: Record<string, unknown>): ProviderModelCapability[] {
  ...
  if (modalities.includes("image") || modalities.includes("vision")) capabilities.add("vision")
  ...
  if (
    ...
    supportedParameters.includes("function_calling") ||
...
function normalizeOpenAiCompatibleModels(data: unknown) {
  const rows = Array.isArray((data as { data?: unknown[] } | undefined)?.data) ? (data as { data: unknown[] }).data : []
  return rows
...
export async function discoverProviderModels(
  ...
  if (payload.providerType === "ollama") {
    ...
    return { models: normalizeOllamaModels(await requestJson(source, { method: "GET" })), source }
  }
  ...
  if (!openAiCompatibleProviders.has(payload.providerType))
    throw new Error(`Model discovery is not supported for provider type: ${payload.providerType}`)
  const baseUrl = normalizeBaseUrl(payload.baseUrl)
  if (!baseUrl) throw new Error("A base URL is required before refreshing the remote model catalog.")
  const source = `${baseUrl}/models`
  const headers: Record<string, string> = { Accept: "application/json" }
  if (payload.apiKey.trim()) headers.Authorization = `Bearer ${payload.apiKey.trim()}`
  return { models: normalizeOpenAiCompatibleModels(await requestJson(source, { method: "GET", headers })), source }
}
