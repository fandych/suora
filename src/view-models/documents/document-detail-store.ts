...
export const useDocumentDetailStore = create<DocumentDetailState>((set, get) => ({
  ...
  updatePage: (pageId, patch) => set((state) => state.draft ? { draft: { ...state.draft, pages: state.draft.pages.map((page) => page.id === pageId ? { ...page, ...patch } : page) } } : state),
  addPage: (page) => set((state) => state.draft ? { draft: { ...state.draft, pages: [...state.draft.pages, page] } } : state),
  removePageTree: (pageId) => set((state) => {
    if (!state.draft) return state
    const descendants = new Set<string>()
    const visit = (parentId: string) => {
      state.draft?.pages.forEach((page) => {
        if (page.parentId === parentId) { descendants.add(page.id); visit(page.id) }
      })
    }
    visit(pageId)
