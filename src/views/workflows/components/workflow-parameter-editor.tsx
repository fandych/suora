...
export function WorkflowKeyValueEditor({ label, value, onChange, placeholder = "key: value", suggestions }: { label: string; value: string; onChange: (value: string) => void; placeholder?: string; suggestions?: WorkflowExpressionSuggestion[] }) {
  const entries = (() => {
    try {
  ...
  return (
    <WorkflowField label={label} hint="Values support workflow interpolation such as {{input.requestId}}.">
      <div className="flex flex-col gap-2">
        {entries.map((entry, index) => <div key={`${entry.key}-${index}`} className="grid grid-cols-[1fr_1fr_auto] gap-2"><Input value={entry.key} placeholder="Name" onChange={(event) => writeEntries(entries.map((item, itemIndex) => itemIndex === index ? { ...item, key: event.target.value } : item))} />{suggestions ? <WorkflowExpressionInput value={entry.value} onChange={(entryValue) => writeEntries(entries.map((item, itemIndex) => itemIndex === index ? { ...item, value: entryValue } : item))} suggestions={suggestions} placeholder="Value" /> : <Input value={entry.value} placeholder="Value" onChange={(event) => writeEntries(entries.map((item, itemIndex) => itemIndex === index ? { ...item, value: event.target.value } : item))} />}<Button type="button" size="icon-xs" variant="ghost" onClick={() => writeEntries(entries.filter((_item, itemIndex) => itemIndex !== index))} aria-label={`Remove ${entry.key || "entry"}`}>×</Button></div>)}
...
export function WorkflowParameterEditor({ items, nameLabel, valueLabel, onNameChange, onValueChange, onAdd, addLabel = "Add item", suggestions }: WorkflowParameterEditorProps) {
      ...
      {items.map((item) => (
        <div key={item.id} className="rounded-xl border p-2.5">
          {editingId === item.id ? <div className="flex flex-col gap-2">
            ...
            <WorkflowField label={valueLabel}>{item.valueDisabled ? <Input value={item.value} disabled /> : suggestions ? <WorkflowExpressionInput multiline rows={3} value={item.value} onChange={(value) => onValueChange(item.id, value)} suggestions={suggestions} placeholder={item.valuePlaceholder} /> : <Textarea value={item.value} onChange={(event) => onValueChange(item.id, event.target.value)} placeholder={item.valuePlaceholder} />}</WorkflowField>
