...
export function useWorkflowValidation(input: WorkflowValidationInput) {
  ...
  }), [input.dryRunInput, input.edges, input.notifications, input.nodes, input.resourceBindings, input.savedDefinition?.budget, input.savedDefinition?.variables, input.viewport])
  ...
  }), [input.availableAgentIds, input.availableDocumentIds, input.availableIntegrationIds, input.availableModelIds, input.edges, input.nodes])
  ...
  const currentFingerprint = useMemo(() => buildWorkflowFingerprint({ title: input.title, summary: input.summary, definition: currentDefinition }), [currentDefinition, input.summary, input.title])
