...
export async function discoverSkillsFromGitHub(
  ...
  try {
    const fetchResult = (await electron.invoke('web:fetchJson', apiUrl)) as { content?: string; error?: string }
    ...
    for (const item of result) {
      ...
      try {
        const content = (await electron.invoke('web:fetchText', skillMdUrl)) as { content?: string; error?: string }
        if (content.error || !content.content) continue

        const skill = parseSkillMarkdown(content.content, `${item.name}/SKILL.md`, 'registry', { silent: true })
        if (!skill) continue
  ...
  for (const skill of installedSkills) {
    ...
    try {
      ...
      if (repo && skill.installInfo.manifestHash) {
        ...
        const result = (await electron.invoke('web:fetchText', contentUrl)) as {
          content?: string
          error?: string
        }
...
export async function browseRegistrySkills(
  ...
  const allEntries = getFeaturedRegistrySkills(installedSkillNames)
...
export function getFeaturedRegistrySkills(installedSkillNames: Set<string>): RegistrySkillEntry[] {
  return getFeaturedSkills().map((skill) => ({
    ...skill,
    installed: installedSkillNames.has(skill.name),
    trustedSource: TRUSTED_SOURCE_IDS.has(skill.sourceId),
  }))
}
...
async function fetchGitHubDirectory(
  ...
  const result = (await electron.invoke('web:fetchJson', apiUrl)) as { content?: string; error?: string }
