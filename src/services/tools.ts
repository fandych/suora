...
const pipelineToolStepSchema = z.object({
  ...
  node_type: z.enum(['start', 'end', 'condition', 'agent', 'pipeline', 'script', 'code', 'template', 'variable', 'iteration', 'http', 'email', 'parallel', 'join', 'webhook', 'toolset', 'rag', 'wiki']).optional().describe('Optional workflow node type'),
  ...
  code_language: z.enum(['javascript']).optional().describe('Inline code language'),
  code_source: z.string().optional().describe('Inline code source; define main(inputs)'),
  code_input_mapping: z.string().optional().describe('Code input mapping using key={{reference}} lines'),
  code_output_schema: z.string().optional().describe('Comma-separated code output variable names'),
  template_body: z.string().optional().describe('Template body for template nodes'),
  template_input_mapping: z.string().optional().describe('Template input mapping using key={{reference}} lines'),
  variable_assignments: allowJsonStringInput(z.array(z.object({
    variable: z.string(),
    mode: z.enum(['overwrite', 'append', 'extend', 'clear']),
    value: z.string().optional(),
  }))).optional().describe('Workflow variable assignment rows'),
  iteration_source: z.string().optional().describe('Array source for iteration nodes'),
  iteration_pipeline_target_id: z.string().optional().describe('Child pipeline id or name for iteration nodes'),
  iteration_input_mapping: z.string().optional().describe('Iteration child variable mapping using key={{reference}} lines'),
  iteration_item_var: z.string().optional().describe('Workflow variable name for the current iteration item'),
  iteration_index_var: z.string().optional().describe('Workflow variable name for the current iteration index'),
  iteration_mode: z.enum(['sequential', 'parallel']).optional().describe('Whether iteration runs sequentially or in parallel'),
  iteration_error_mode: z.enum(['terminate', 'continue', 'remove-failed']).optional().describe('How iteration handles failed child runs'),
  http_method: z.enum(['GET', 'POST', 'PUT', 'PATCH', 'DELETE']).optional().describe('HTTP method for request nodes'),
  http_url: z.string().optional().describe('HTTP URL for request nodes'),
  http_headers: z.string().optional().describe('HTTP headers as newline-separated Key: Value pairs'),
  http_auth_type: z.enum(['none', 'bearer', 'basic', 'api-key', 'custom-header']).optional().describe('HTTP auth type'),
  http_auth_header: z.string().optional().describe('HTTP auth header name'),
  http_auth_value: z.string().optional().describe('HTTP auth value'),
  http_body: z.string().optional().describe('HTTP request body'),
  http_body_type: z.enum(['json', 'form', 'raw']).optional().describe('HTTP request body type'),
  http_async: z.boolean().optional().describe('Queue the HTTP request asynchronously'),
  http_capture_response: z.boolean().optional().describe('Capture the HTTP response as step output'),
  http_treat_non_2xx_as_error: z.boolean().optional().describe('Treat non-2xx statuses as failures'),
  http_success_statuses: z.string().optional().describe('Explicit successful HTTP statuses'),
  http_response_body_var: z.string().optional().describe('Workflow variable for HTTP response body'),
  http_response_status_var: z.string().optional().describe('Workflow variable for HTTP status code'),
  http_response_headers_var: z.string().optional().describe('Workflow variable for HTTP response headers'),
  http_response_size_var: z.string().optional().describe('Workflow variable for HTTP response size'),
})
...
function mapPipelineToolStep(step: PipelineToolStepInput): AgentPipelineStep {
  return {
    ...
    task: step.task,
    ...(step.node_type !== undefined ? { nodeType: step.node_type } : {}),
    ...
    ...(step.code_language !== undefined ? { codeLanguage: step.code_language } : {}),
    ...(sanitizeOptionalText(step.code_source) ? { codeSource: sanitizeOptionalText(step.code_source) } : {}),
    ...(sanitizeOptionalText(step.code_input_mapping) ? { codeInputMapping: sanitizeOptionalText(step.code_input_mapping) } : {}),
    ...(sanitizeOptionalText(step.code_output_schema) ? { codeOutputSchema: sanitizeOptionalText(step.code_output_schema) } : {}),
    ...(sanitizeOptionalText(step.template_body) ? { templateBody: sanitizeOptionalText(step.template_body) } : {}),
    ...(sanitizeOptionalText(step.template_input_mapping) ? { templateInputMapping: sanitizeOptionalText(step.template_input_mapping) } : {}),
    ...(step.variable_assignments !== undefined ? { variableAssignments: step.variable_assignments } : {}),
    ...(sanitizeOptionalText(step.iteration_source) ? { iterationSource: sanitizeOptionalText(step.iteration_source) } : {}),
    ...(sanitizeOptionalText(step.iteration_pipeline_target_id) ? { iterationPipelineTargetId: sanitizeOptionalText(step.iteration_pipeline_target_id) } : {}),
    ...(sanitizeOptionalText(step.iteration_input_mapping) ? { iterationInputMapping: sanitizeOptionalText(step.iteration_input_mapping) } : {}),
    ...(sanitizeOptionalText(step.iteration_item_var) ? { iterationItemVar: sanitizeOptionalText(step.iteration_item_var) } : {}),
    ...(sanitizeOptionalText(step.iteration_index_var) ? { iterationIndexVar: sanitizeOptionalText(step.iteration_index_var) } : {}),
    ...(step.iteration_mode !== undefined ? { iterationMode: step.iteration_mode } : {}),
    ...(step.iteration_error_mode !== undefined ? { iterationErrorMode: step.iteration_error_mode } : {}),
    ...(step.http_method !== undefined ? { httpMethod: step.http_method } : {}),
    ...(sanitizeOptionalText(step.http_url) ? { httpUrl: sanitizeOptionalText(step.http_url) } : {}),
    ...(sanitizeOptionalText(step.http_headers) ? { httpHeaders: sanitizeOptionalText(step.http_headers) } : {}),
    ...(step.http_auth_type !== undefined ? { httpAuthType: step.http_auth_type } : {}),
    ...(sanitizeOptionalText(step.http_auth_header) ? { httpAuthHeader: sanitizeOptionalText(step.http_auth_header) } : {}),
    ...(sanitizeOptionalText(step.http_auth_value) ? { httpAuthValue: sanitizeOptionalText(step.http_auth_value) } : {}),
    ...(sanitizeOptionalText(step.http_body) ? { httpBody: sanitizeOptionalText(step.http_body) } : {}),
    ...(step.http_body_type !== undefined ? { httpBodyType: step.http_body_type } : {}),
    ...(step.http_async !== undefined ? { httpAsync: step.http_async } : {}),
    ...(step.http_capture_response !== undefined ? { httpCaptureResponse: step.http_capture_response } : {}),
    ...(step.http_treat_non_2xx_as_error !== undefined ? { httpTreatNon2xxAsError: step.http_treat_non_2xx_as_error } : {}),
    ...(sanitizeOptionalText(step.http_success_statuses) ? { httpSuccessStatuses: sanitizeOptionalText(step.http_success_statuses) } : {}),
    ...(sanitizeOptionalText(step.http_response_body_var) ? { httpResponseBodyVar: sanitizeOptionalText(step.http_response_body_var) } : {}),
    ...(sanitizeOptionalText(step.http_response_status_var) ? { httpResponseStatusVar: sanitizeOptionalText(step.http_response_status_var) } : {}),
    ...(sanitizeOptionalText(step.http_response_headers_var) ? { httpResponseHeadersVar: sanitizeOptionalText(step.http_response_headers_var) } : {}),
    ...(sanitizeOptionalText(step.http_response_size_var) ? { httpResponseSizeVar: sanitizeOptionalText(step.http_response_size_var) } : {}),
  }
}
