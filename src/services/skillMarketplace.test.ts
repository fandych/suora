...
import { browseRegistrySkills, getFeaturedRegistrySkills, installSkillFromRegistry, previewSkillInstall, uninstallSkill } from './skillMarketplace'
...
describe('skillMarketplace', () => {
  ...
  it('provides featured skills before remote registry discovery completes', async () => {
    const featured = getFeaturedRegistrySkills(new Set(['skill-creator']))

    expect(featured.length).toBeGreaterThan(0)
    expect(featured.find((entry) => entry.name === 'skill-creator')?.installed).toBe(true)
  })

  it('keeps featured skills when GitHub registry discovery fails', async () => {
    vi.mocked(window.electron.invoke).mockResolvedValue({ error: 'network unavailable' })

    const entries = await browseRegistrySkills([
      {
        id: 'broken-source',
        name: 'Broken Source',
        type: 'github',
        url: 'https://github.com/example/missing-skills',
        enabled: true,
      },
    ], new Set())

    expect(entries.length).toBeGreaterThan(0)
    expect(entries.some((entry) => entry.sourceId === 'skills-sh')).toBe(true)
  })
  ...
  it('installs all files from a GitHub skill directory', async () => {
    vi.mocked(window.electron.invoke).mockImplementation(async (channel, firstArg) => {
      ...
      if (channel === 'web:fetchJson' && firstArg === 'https://api.github.com/repos/anthropics/skills/contents/skills/skill-creator') {
        return {
      ...
      if (channel === 'web:fetchJson' && firstArg === 'https://api.github.com/repos/anthropics/skills/contents/skills/skill-creator/references') {
        return {
          content: JSON.stringify([
    ...
    expect(window.electron.invoke).toHaveBeenCalledWith('web:fetchJson', 'https://api.github.com/repos/anthropics/skills/contents/skills/skill-creator')
  })
  ...
  it('previews registry installation with resources, size, hash, and script warnings', async () => {
    vi.mocked(window.electron.invoke).mockImplementation(async (channel, firstArg) => {
      ...
      if (channel === 'web:fetchJson' && firstArg === 'https://api.github.com/repos/anthropics/skills/contents/skills/skill-creator/scripts') {
        return {
          content: JSON.stringify([
  ...
  it('rejects registry previews for too many files, too large skills, and overly deep directories', async () => {
    ...
    vi.mocked(window.electron.invoke).mockReset()
    vi.mocked(window.electron.invoke).mockImplementation(async (channel, firstArg) => {
      if (channel !== 'web:fetchJson') return { error: `unexpected ${channel}:${firstArg}` }
