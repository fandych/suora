...
import type { SkillDetail } from "@/types/document"
...
export const useSkillDetailStore = create<SkillDetailState>((set, get) => ({
  ...
  updateSkill: (patch) =>
    set((state) => (state.draft ? { draft: { ...state.draft, skill: { ...state.draft.skill, ...patch } } } : state)),
    ...
    set((state) =>
      state.draft
        ? {
            draft: {
              ...state.draft,
              files: state.draft.files.map((file) => (file.path === path ? { ...file, ...patch } : file)),
            },
          }
        : state,
    ),
  addFiles: (files) =>
    set((state) => (state.draft ? { draft: { ...state.draft, files: [...state.draft.files, ...files] } } : state)),
  removePathTree: (path) =>
    set((state) =>
      state.draft
        ? {
            draft: {
              ...state.draft,
              files: state.draft.files.filter((file) => file.path !== path && !file.path.startsWith(`${path}/`)),
            },
          }
        : state,
    ),
  save: async () => {
    ...
    try {
      const saved = await SkillApi.saveDraft(draft.skill.id, {
