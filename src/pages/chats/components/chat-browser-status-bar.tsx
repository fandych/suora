...
export function ChatBrowserStatusBar({
  browserState,
  onContinue,
  onRetry,
  sessionId,
  pendingContinue = false,
}: ChatBrowserStatusBarProps) {
  ...
  const handleReadPage = async () => {
    ...
    try {
      const result = (await ToolApi.browserPage({
  ...
  return (
    <div className="flex flex-wrap items-center gap-2 rounded-lg border border-sky-200 bg-sky-50/80 px-2.5 py-2 text-xs text-sky-900">
      ...
      ) : browserState.status === "working" || browserState.status === "navigating" ? (
      ...
      <span className="font-medium">
        {browserState.status === "error"
          ...
          : browserState.status === "navigating"
            ? "正在打开网页"
            : browserState.status === "working"
              ? "正在读取或操作网页"
              : "浏览器已打开，等待你的操作"}
      </span>
      {browserState.error ? <span className="basis-full text-destructive">{browserState.error}</span> : null}
      {browserState.url ? (
        <span className="min-w-0 max-w-full truncate text-[11px] text-sky-800/90" title={browserState.url}>
          {browserState.url}
        </span>
      ) : null}
      {browserState.status === "awaiting-user" ? (
        <span className="basis-full text-[11px] text-sky-800/80">
          完成登录、验证码或网页操作后，点击“我已完成”继续。
        </span>
      ) : null}
      {browserState.status === "awaiting-user" ? (
        <Button
          ...
          onClick={onContinue}
          disabled={pendingContinue}
        >
          {pendingContinue ? <Loader2Icon className="animate-spin" /> : <CheckIcon />}
          {pendingContinue ? "正在继续..." : "我已完成，继续"}
        </Button>
      ) : null}
      {browserState.status === "error" && onRetry ? (
        <Button size="sm" variant="ghost" type="button" className="h-7 px-2 text-[11px]" onClick={onRetry}>
          <RotateCcwIcon />
          重试
        </Button>
      ) : null}
      {browserState.url ? (
        <Button
          ...
          onClick={() => {
            void copyTextToClipboard(browserState.url).then(() =>
              showToast({
                title: "链接已复制",
                description: "浏览器页面链接已复制到剪贴板。",
                type: "success",
                timeout: 1800,
              }),
            )
          }}
          aria-label="复制浏览器页面链接"
        >
          <ClipboardIcon />
          复制链接
        </Button>
      ) : null}
      {browserState.status !== "navigating" && browserState.status !== "working" ? (
        <Button
          size="sm"
          variant="ghost"
          type="button"
      ...
      <Button
        ...
        className="h-7 px-2 text-[11px]"
        onClick={() => {
          void ToolApi.browserNavigate({ sessionId: sessionId ?? undefined, visible: !browserState.visible })
        }}
        disabled={browserState.status === "navigating" || browserState.status === "working"}
        aria-label={browserState.visible ? "隐藏浏览器窗口" : "打开浏览器窗口"}
      >
        {browserState.visible ? <EyeOffIcon className="size-3.5" /> : <EyeIcon className="size-3.5" />}
        {browserState.visible ? "隐藏浏览器" : "打开浏览器"}
      </Button>
      {showPageText ? (
        <div className="basis-full rounded-md border border-sky-200 bg-white/70 p-2 text-[11px] text-sky-950">
