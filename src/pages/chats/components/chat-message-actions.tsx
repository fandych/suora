...
export function ChatMessageActions({ baseName, className, content, createdAt }: ChatMessageActionsProps) {
  ...
  const handleCopy = async () => {
    try {
      ...
      showToast({
        ...
        description: "The message content was copied to clipboard.",
        type: "success",
        timeout: 2000,
      })
    } catch (error) {
  ...
  return (
    <div
      className={`flex min-w-0 items-center justify-end gap-1 px-1 text-[11px] text-muted-foreground ${className ?? ""}`}
    >
      {createdAt ? (
        <div className="mr-auto truncate">{new Date(createdAt).toLocaleString()}</div>
      ) : (
