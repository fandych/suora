...
import { ChannelLogoBadge } from "@/pages/channels/components/channel-logo-badge"
import PageHeader from "@/pages/components/page-header"
import { ErrorCard, LoadingCard } from "@/pages/components/resource-state"
import { ChannelEditorForm } from "@/pages/channels/components/channel-editor-form"
import {
  getChannelBindingLabel,
  getChannelBindingVariant,
  getChannelStatusLabel,
  getChannelStatusVariant,
} from "@/pages/channels/components/channel-utils"
...
const ChannelDetailPage = () => {
  ...
  const handleSave = async () => {
    ...
    showToast({
      title: "Channel saved",
      description: "Channel settings were persisted.",
      type: "success",
      timeout: 2000,
    })
  }
  ...
  const handleUnbind = async () => {
    ...
    try {
      ...
      updateDraft(next)
      showToast({
        title: "Channel unbound",
        description: "The current channel binding was removed.",
        type: "success",
        timeout: 2500,
      })
    } finally {
  ...
  const monitorWeChatBinding = async (initialDetail: ChannelDetail, verificationCode?: string) => {
    ...
    while (activeWeChatMonitorSessionRef.current === monitorSessionKey) {
      const waitResult = await waitForWeChatPersonalBinding(
        current,
        nextVerificationCode,
        nextVerificationCode ? 60_000 : 35_000,
      )
      ...
      if (waitResult.status === "connected") {
        ...
        showToast({
          ...
          description: waitResult.message || "Personal WeChat binding completed.",
          type: "success",
          timeout: 3000,
        })
        return
      ...
      if (waitResult.status === "already_bound") {
        ...
        showToast({
          title: "Already bound",
          description: waitResult.message || "This WeChat account is already bound.",
          type: "info",
          timeout: 3000,
        })
        return
      ...
      if (waitResult.status === "need_verifycode") {
        ...
        showToast({
          ...
          description: waitResult.message || "Enter the digits shown in WeChat to continue.",
          type: "info",
          timeout: 3000,
        })
        return
      ...
      if (waitResult.status === "scaned") {
        if (!hasShownScannedMessage) {
          ...
          showToast({
            title: "QR scanned",
            description: waitResult.message || "Waiting for confirmation in WeChat.",
            type: "info",
            timeout: 2500,
          })
        }
      ...
      if (waitResult.status === "expired") {
        showToast({
          title: "QR refreshed",
          description: waitResult.message || "The QR code expired and was refreshed.",
          type: "info",
          timeout: 3000,
        })
        continue
      ...
      activeWeChatMonitorSessionRef.current = null
      showToast({
        title: "WeChat binding failed",
        description: waitResult.message || "The QR login session could not be completed.",
        type: "error",
        timeout: 3000,
      })
      return
        ...
        showToast({
          ...
          description:
            ...
            (started
              ...
              : "The runtime did not return a usable QR code."),
      ...
      showToast({
        ...
        description:
          ...
          (bound
            ...
            : "Check the required provider fields and try again."),
  ...
  const handleConfirmWeChatBinding = async () => {
    ...
    try {
      ...
      if (next.channel.wechatPersonalBindingStatus === "bound") {
        ...
        showToast({
          title: "WeChat connected",
          description: "Personal WeChat bridge binding completed.",
          type: "success",
          timeout: 2500,
        })
        return
      ...
      showToast({
        title:
          next.channel.wechatPersonalBindingStatus === "pending" ? "Verification required" : "WeChat binding failed",
        description:
          latestMessage ||
          (next.channel.wechatPersonalBindingStatus === "pending"
            ? "Enter the verification code shown on the device to continue."
            : "The QR login session could not be completed."),
  ...
  return (
    <div className="flex min-h-full flex-col bg-background">
      <PageHeader
        ...
        leading={
          draft ? <ChannelLogoBadge channel={draft.channel} className="size-9" iconClassName="size-4.5" /> : null
        }
        actions={
          draft ? (
            <>
              <Badge variant={getChannelStatusVariant(draft.channel.status)}>
                {getChannelStatusLabel(draft.channel.status)}
              </Badge>
              <Badge variant={getChannelBindingVariant(draft.channel.bindingState)}>
                {getChannelBindingLabel(draft.channel.bindingState)}
              </Badge>
            </>
          ) : null
        }
