...
export function ChannelRuntimePanel({
  detail,
  agents,
  onHealthCheck,
  onSendMock,
  onClearDebug,
}: ChannelRuntimePanelProps) {
  ...
  const debugEntries = useMemo(
    () => [...runtime.debugLog].sort((left, right) => right.timestamp - left.timestamp),
    [runtime.debugLog],
  )
    ...
    <Card className="min-h-0">
      <CardHeader className="pb-3">
        ...
        <CardDescription className="text-xs">
          Overview, message traffic, health, and mock-mode controls aligned with main branch behavior.
        </CardDescription>
      </CardHeader>
        ...
        <Tabs defaultValue="overview" className="flex min-h-0 flex-col gap-3">
          ...
          <TabsContent value="overview" className="mt-0 space-y-3">
            <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
              ...
              <StatCard
                label="Health"
                value={
                  runtime.health.isHealthy == null ? "Not checked" : runtime.health.isHealthy ? "Healthy" : "Unhealthy"
                }
              />
            </div>
          </TabsContent>
                  ...
                  runtime.messages.map((message) => (
                    <div key={message.id} className="rounded-xl border px-3 py-2 text-xs">
                      ...
                      <div className="mt-1 text-[11px] text-muted-foreground">
                        {message.direction} · {message.status} · {message.senderId}
                      </div>
                      <div className="mt-2 whitespace-pre-wrap text-[12px] leading-5 text-foreground">
                        {message.content}
                      </div>
                    </div>
                  ))
                ...
                {runtime.users.length ? (
                  runtime.users.map((user) => (
                    <div key={user.id} className="rounded-xl border px-3 py-2 text-xs">
                      ...
                      {user.conversationHistory.length ? (
                        <div className="mt-2 space-y-1">
                          {user.conversationHistory.slice(-3).map((entry, index) => (
                            <div key={`${entry.timestamp}-${index}`} className="text-[11px] text-muted-foreground">
          ...
          <TabsContent value="health" className="mt-0 space-y-3">
            <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
              ...
              <StatCard
                label="Latency"
                value={runtime.health.latencyMs == null ? "Pending" : `${runtime.health.latencyMs}ms`}
              />
              <StatCard label="Errors" value={String(runtime.health.errorCount)} />
              <StatCard
                label="Last check"
                value={runtime.health.lastCheckAt ? new Date(runtime.health.lastCheckAt).toLocaleString() : "Never"}
              />
            </div>
          ...
          <TabsContent value="debug" className="mt-0 space-y-3">
            <div className="grid gap-2 sm:grid-cols-[minmax(0,1fr)_auto_auto]">
              <Input
                ...
                value={mockMessage}
                onChange={(event) => setMockMessage(event.target.value)}
                placeholder="Send a mock inbound message"
              />
              <Button
                size="sm"
                onClick={() => {
                  onSendMock(mockMessage)
                  setMockMessage("")
                }}
              ...
              <Button size="sm" variant="outline" onClick={onClearDebug} disabled={!debugEntries.length}>
                Clear log
              </Button>
            </div>
              ...
              <div className="space-y-2 p-3">
                {debugEntries.length ? (
                  debugEntries.map((entry) => (
                    <div
                      key={entry.id}
                      className={`rounded-xl border px-3 py-2 text-xs ${entry.tone === "error" ? "border-destructive/30 bg-destructive/5" : entry.tone === "success" ? "border-emerald-500/30 bg-emerald-500/5" : "bg-muted/20"}`}
                    >
                      <div className="flex items-center justify-between gap-2">
