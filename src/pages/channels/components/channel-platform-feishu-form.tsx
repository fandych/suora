...
export function ChannelPlatformFeishuForm({ channel, onPatch }: ChannelPlatformFeishuFormProps) {
  return (
    <div className="space-y-3">
      <FormGroupSection
        title="Feishu verification"
        description="Use the verification token and encrypt key from the Feishu bot console so webhook traffic can be validated correctly."
      >
        <div className="grid gap-3 md:grid-cols-2">
          ...
          <Field label="App webhook URL" className="md:col-span-2">
            <CompactInput
              value={channel.feishuWebhookUrl ?? ""}
              onChange={(event) => onPatch({ feishuWebhookUrl: event.target.value })}
              placeholder="Optional outgoing webhook"
            />
          </Field>
        </div>
      </FormGroupSection>
