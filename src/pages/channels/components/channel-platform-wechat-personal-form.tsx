    ...
    <div className="space-y-3">
      <div className="flex flex-wrap items-center justify-between gap-2 rounded-xl border border-primary/15 bg-primary/5 p-3">
        <div>
          ...
          <Hint>
            Use the built-in WeChat QR login flow. A verification code is only needed if the runtime explicitly requests
            it.
          </Hint>
        </div>
        <Button size="sm" onClick={handleOpenBindDialog} disabled={isBinding}>
          {isBinding ? "Binding..." : bindButtonLabel}
        </Button>
      </div>
      {channel.wechatPersonalBindingStatus === "bound" && !normalizedWeChatQrSource ? (
        ...
        <div className="rounded-xl border border-primary/20 bg-primary/5 px-3 py-3 text-xs text-foreground">
          QR code scanned. Waiting for confirmation in WeChat.
        </div>
      ) : channel.wechatPersonalQrStatus === "need_verifycode" ? (
        <div className="rounded-xl border border-primary/20 bg-primary/5 px-3 py-3 text-xs text-foreground">
        ...
        <div className="rounded-xl border border-dashed px-3 py-5 text-xs text-muted-foreground">
          Start or rebind WeChat to open the QR code dialog.
        </div>
      )}
