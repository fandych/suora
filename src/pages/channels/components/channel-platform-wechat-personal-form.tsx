...
import { CompactInput, Field, Hint } from "@/pages/channels/components/channel-form-fields"
...
function toDataUrl(base64: string, format?: string) {
  const mime =
    format?.trim().toLowerCase() === "jpeg" || format?.trim().toLowerCase() === "jpg" ? "image/jpeg" : "image/png"
...
async function buildWeChatQrScreenshotPreview(
  base64: string,
  format?: string,
): Promise<{ dataUrl: string; shouldRetry: boolean }> {
...
export function ChannelPlatformWeChatPersonalForm({
  ...
  const bindButtonLabel =
    channel.wechatPersonalBindingStatus === "bound"
      ? t("channels.wechatPersonal.rebind", "Rebind WeChat")
      : t("channels.wechatPersonal.startBinding", "Start QR binding")
  const isWeChatConnected = channel.wechatPersonalBindingStatus === "bound" || channel.bindingState === "connected"
  const qrImageSource =
    qrPreviewFallback?.source === normalizedWeChatQrSource
  ...
  const qrStatusClassName =
    channel.wechatPersonalQrStatus === "scaned" || channel.wechatPersonalQrStatus === "need_verifycode"
  ...
  useEffect(() => {
    if (
      ...
      !isBinding &&
      normalizedWeChatQrSource &&
      sourceSnapshotOnOpen !== normalizedWeChatQrSource
    ) {
    ...
    isWeChatConnected,
    channel.wechatPersonalQrStatus,
    isBinding,
    normalizedWeChatQrSource,
    requestedQrFlow,
    sourceSnapshotOnOpen,
  ])
  ...
  const handleQrPreviewError = useCallback(async () => {
    ...
    try {
      ...
      for (const delayMs of WECHAT_QR_SCREENSHOT_RETRY_DELAYS_MS) {
        ...
        const result = (await window.electron?.invoke("channel:wechatPersonalQrPreview", source, delayMs)) as
          { image?: string; format?: string; error?: string } | undefined
    ...
    <div className="space-y-3">
        ...
        <div>
          ...
          <Hint>
            {t(
              ...
              "Use the built-in WeChat QR login flow. A verification code is only needed if the runtime explicitly requests it.",
            )}
          </Hint>
        </div>
        <Button size="sm" onClick={handleOpenBindDialog} disabled={isBinding}>
          {isBinding ? t("channels.wechatPersonal.binding", "Binding...") : bindButtonLabel}
        </Button>
      </div>
      {channel.wechatPersonalBindingStatus === "bound" && !normalizedWeChatQrSource ? (
        <div className="rounded-xl border border-emerald-500/25 bg-emerald-500/5 px-3 py-3 text-xs text-emerald-700">
          {t(
            "channels.wechatPersonal.alreadyBound",
            "WeChat is already bound. Use Rebind WeChat to request a fresh QR code.",
          )}
        </div>
      ) : channel.wechatPersonalQrStatus === "scaned" ? (
        <div className="rounded-xl border border-primary/20 bg-primary/5 px-3 py-3 text-xs text-foreground">
          {t("channels.wechatPersonal.scannedHint", "QR code scanned. Waiting for confirmation in WeChat.")}
        </div>
      ) : channel.wechatPersonalQrStatus === "need_verifycode" ? (
        <div className="rounded-xl border border-primary/20 bg-primary/5 px-3 py-3 text-xs text-foreground">
          {t(
            "channels.wechatPersonal.needCodeHint",
            "WeChat requested a verification code. Enter the digits shown on the device below.",
          )}
        </div>
      ) : (
        <div className="rounded-xl border border-dashed px-3 py-5 text-xs text-muted-foreground">
          {t("channels.wechatPersonal.startHint", "Start or rebind WeChat to open the QR code dialog.")}
        </div>
      )}

      {showVerification ? (
        <div className="grid gap-3 rounded-xl border p-3 md:grid-cols-[minmax(0,1fr)_auto]">
          <Field label={t("channels.wechatPersonal.verificationCode", "Verification code")}>
            <CompactInput
              value={verificationCode}
              onChange={(event) => onVerificationCodeChange(event.target.value)}
              placeholder={t("channels.wechatPersonal.verificationCodePlaceholder", "Enter code from bridge or device")}
            />
          </Field>
      ...
      <Dialog open={isQrDialogVisible} onOpenChange={setIsQrDialogOpen}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle>{t("channels.wechatPersonal.scanToBind", "Scan to bind")}</DialogTitle>
          </DialogHeader>
          {shouldWaitForFreshQr ? (
            ...
            <div className="space-y-2">
              <div className="relative overflow-hidden rounded-xl border bg-white p-3">
                {!qrImageLoaded || qrPreviewResolvingSource === normalizedWeChatQrSource ? (
            ...
            <div className="flex min-h-80 items-center justify-center rounded-xl border bg-muted/10 text-sm text-muted-foreground">
              {t("channels.wechatPersonal.waitingForQr", "Waiting for QR code...")}
            </div>
          )}
        </DialogContent>
      </Dialog>
