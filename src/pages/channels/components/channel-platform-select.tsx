...
const groupedChannelPlatformOptions: Array<{ label: string; values: ChannelConfigRecord["platform"][] }> = [
  {
    ...
    values: ["feishu", "dingtalk", "wechat", "wechat_personal", "wechat_official", "wechat_miniprogram"],
  },
...
export function ChannelPlatformSelect({ channel, onChange }: ChannelPlatformSelectProps) {
  ...
  const selectedLabel =
    ...
    platformOptions.find((item) => item.value === channel.platform)?.label ||
    channel.platform
    ...
    <Select value={channel.platform} onValueChange={(value) => onChange(value as ChannelConfigRecord["platform"])}>
      <SelectTrigger className="w-full justify-start">
        <span className="flex min-w-0 flex-1 items-center gap-2">
          <SelectedLogo
            className={cn("size-4 shrink-0", getChannelPlatformBrandClassName(channel.platform, channel))}
          />
        ...
        {groupedChannelPlatformOptions.map((group) => (
          <SelectGroup key={group.label}>
            ...
            {group.values.map((value) => {
              ...
              const OptionLogo = getChannelPlatformLogo({
                platform: item.value,
                customPlatformName: item.label === "QQ" ? "QQ" : undefined,
              })
              return (
                <SelectItem key={`${group.label}-${item.label}`} value={item.value}>
                  <OptionLogo
                    className={cn(
                      "size-4 shrink-0",
                      getChannelPlatformBrandClassName(item.value, {
                        customPlatformName: item.label === "QQ" ? "QQ" : undefined,
                      }),
                    )}
                  />
