...
import type { HttpIntegrationConfig, IntegrationConfig, McpIntegrationConfig } from "@/types/integration"
...
function getIntegrationIssues(config: IntegrationConfig | null) {
  ...
  if (config.kind === "http") {
    return [
      !config.baseUrl.trim()
        ? { severity: "error" as const, message: "HTTP toolset requires a base URL before it can run." }
        : null,
      !config.endpoints.every((endpoint) => endpoint.path.trim())
        ? { severity: "error" as const, message: "Every HTTP endpoint needs a path." }
        : null,
      !config.description.trim()
        ? { severity: "warning" as const, message: "HTTP toolset is missing a description for operators." }
        : null,
  ...
  return [
    ...
    config.scripts.length === 0
      ...
      : null,
    !selectedScript?.code.trim() ? { severity: "error" as const, message: "Selected script has no code body." } : null,
  ].filter((issue): issue is { severity: "warning" | "error"; message: string } => Boolean(issue))
}
    ...
    <div className="flex min-h-full flex-col bg-background">
      <PageHeader
        ...
        actions={
          data ? (
            <DropdownMenu>
              <DropdownMenuTrigger
                render={<Button size="icon-sm" variant="outline" aria-label="Integration actions" />}
              >
                <EllipsisIcon />
              </DropdownMenuTrigger>
      ...
      <div className="flex-1 overflow-x-hidden p-4">
        <div className="flex w-full min-w-0 flex-col gap-4">
          ...
          {error && !(error.message.startsWith("Integration ") && error.message.endsWith(" was not found.")) ? (
