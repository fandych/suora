...
function buildCurlPreview(config: HttpIntegrationConfig, values: InputValues) {
  ...
  for (const parameter of endpoint.parameters.filter((item) => item.in === "path"))
    url = url
      .replaceAll(
        `{${parameter.name}}`,
        encodeURIComponent(String(values[parameter.name] ?? parameter.defaultValue ?? "")),
      )
      .replaceAll(
        `:${parameter.name}`,
        encodeURIComponent(String(values[parameter.name] ?? parameter.defaultValue ?? "")),
  ...
  try {
    Object.entries(JSON.parse(endpoint.queryJson) as Record<string, unknown>).forEach(([key, value]) =>
      query.set(key, String(value)),
    )
  } catch {
    /* invalid defaults are validated on save */
  }
  endpoint.parameters
    .filter((item) => item.in === "query")
    .forEach((parameter) => query.set(parameter.name, String(values[parameter.name] ?? parameter.defaultValue ?? "")))
  const queryText = query.toString()
  if (queryText) url += `${url.includes("?") ? "&" : "?"}${queryText}`
  const parts = ["curl", "-X", endpoint.method, curlQuote(url)]
  try {
    Object.entries(JSON.parse(endpoint.headersJson) as Record<string, unknown>).forEach(([key, value]) =>
      parts.push("-H", curlQuote(`${key}: ${String(value)}`)),
    )
  } catch {
    /* invalid defaults are validated on save */
  }
  ...
  for (const parameter of endpoint.parameters.filter((item) => item.in === "header"))
    parts.push("-H", curlQuote(`${parameter.name}: ${String(values[parameter.name] ?? parameter.defaultValue ?? "")}`))
  ...
  if (canSendBody && endpoint.bodyMode === "json") {
    const body = Object.fromEntries(
      bodyParameters
        .filter((item) => item.in === "json")
        .map((item) => [item.name, values[item.name] ?? item.defaultValue ?? ""]),
    )
  ...
  if (canSendBody && (endpoint.bodyMode === "form-data" || endpoint.bodyMode === "x-www-form-urlencoded")) {
    for (const parameter of bodyParameters.filter((item) => item.in === "form-data")) {
      ...
      parts.push(
        endpoint.bodyMode === "form-data" ? "-F" : "--data-urlencode",
        curlQuote(
          file
            ? `${parameter.name}=@${file.name}`
            : `${parameter.name}=${String(value ?? parameter.defaultValue ?? "")}`,
        ),
      )
    }
  }
...
export function IntegrationHttpTryRunForm({ config, input, onChangeInput }: HttpTryRunFormProps) {
  ...
  const updateValue = (parameter: HttpEndpointParameter, value: unknown) =>
    onChangeInput(JSON.stringify({ ...values, [parameter.name]: value }, null, 2))
  ...
  return (
    <div className="flex flex-col gap-4">
            ...
            <div key={parameter.id} className="flex flex-col gap-2">
              ...
              {parameter.type === "file" ? (
                <div className="flex items-center gap-2">
                  <input
                    ref={(element) => {
                      fileInputRefs.current[parameter.id] = element
                    }}
                    className="hidden"
                    type="file"
                    onChange={(event) => void uploadFile(parameter, event.target.files?.[0])}
                  />
                ...
                <NativeSelect
                  value={String(values[parameter.name] ?? parameter.defaultValue ?? false)}
      ...
      <div className="flex flex-col gap-2 rounded-xl border p-3">
        ...
        <pre className="overflow-x-auto whitespace-pre-wrap text-xs text-muted-foreground">{curlPreview}</pre>
      </div>
    </div>
  )
}
