...
import type { HttpIntegrationConfig, IntegrationExecutionRecord, ScriptIntegrationConfig } from "@/types/integration"
    ...
    <Sheet open={isOpen} onOpenChange={onOpenChange}>
      <SheetContent className="w-full sm:max-w-2xl">
        <SheetHeader>
          ...
          <SheetDescription>
            {scriptConfig
              ? "Provide values defined by this script's input contract and inspect recent outputs."
              : httpConfig
                ? "Provide values for the selected REST operation and inspect the generated cURL request."
                : "Execute the current draft with custom JSON input and inspect recent outputs."}
          </SheetDescription>
        </SheetHeader>
                    ...
                    <div key={field.key} className="flex flex-col gap-2">
                      ...
                      {field.type === "boolean" ? (
                        ...
                        <Input
                          ...
                          placeholder={
                            field.type === "array" || field.type === "object" ? `Valid JSON ${field.type}` : field.type
                          }
                          onChange={(event) => updateScriptInput(field, event.target.value)}
                        />
                      )}
