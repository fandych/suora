...
import type { HttpIntegrationConfig, IntegrationConfig, ScriptIntegrationConfig } from "@/types/integration"
  ...
  return JSON.stringify(
    {
      ...
      properties: Object.fromEntries(
        fields
          ...
          .map((field) => [field.key, { type: field.type, description: field.description }]),
      ),
    },
    null,
    2,
    ...
    <div className="space-y-3 rounded-xl border p-3">
        ...
        {nextFields.map((field, index) => (
          <div key={`${field.key}-${index}`} className="grid gap-2 md:grid-cols-[1fr_10rem_1.4fr_auto]">
            ...
            <Button
              ...
              title="Delete field"
              onClick={() => updateFields(nextFields.filter((_, itemIndex) => itemIndex !== index))}
            >
      ...
      <div className="flex gap-2">
        <Button
          size="sm"
          variant="outline"
          onClick={() => updateFields([...nextFields, { key: "", type: "string", description: "" }])}
        >
          Add field
        </Button>
      </div>
      ...
      <CardContent className="space-y-4">
        {config.kind === "http" ? (
          <SchemaEditor
            ...
            schemaText={config.parameterSchemaJson}
            onSchemaChange={(value) => onChange({ ...(config as HttpIntegrationConfig), parameterSchemaJson: value })}
          />
        ) : null}
        {config.kind === "scripts" ? (
          <>
            <SchemaEditor
              ...
              schemaText={config.inputSchemaJson}
              onSchemaChange={(value) => onChange({ ...(config as ScriptIntegrationConfig), inputSchemaJson: value })}
            />
            <SchemaEditor
              label="Output contract"
              schemaText={config.outputSchemaJson}
              onSchemaChange={(value) => onChange({ ...(config as ScriptIntegrationConfig), outputSchemaJson: value })}
            />
          </>
        ) : null}
      </CardContent>
