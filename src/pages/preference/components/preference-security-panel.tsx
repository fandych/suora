      ...
      <FieldGroup className="grid gap-4 md:grid-cols-2">
        ...
        <Field>
          ...
          <NativeSelect
            ...
            className="w-full"
            value={draft.commandConfirmationMode}
            onChange={(event) =>
        ...
        <Field className="md:col-span-2">
          ...
          <Textarea
            ...
            value={joinLines(draft.fileAccessDirectories)}
            onChange={(event) => onChange({ fileAccessDirectories: splitLines(event.target.value) })}
            placeholder={draft.fileAccessPolicy === "allowlist" ? "src\ndocs" : "node_modules\nsecrets"}
          />
        </Field>
        <Field className="md:col-span-2">
          <FieldContent>
          ...
          <Textarea
            rows={6}
            className="font-mono text-xs"
            value={joinLines(draft.commandAllowlist)}
            onChange={(event) => onChange({ commandAllowlist: splitLines(event.target.value) })}
            placeholder="git\nnode\nnpm\nrg"
          />
        </Field>
        <Field className="md:col-span-2">
          ...
          <Textarea
            rows={6}
            className="font-mono text-xs"
            value={joinLines(draft.commandBlacklist)}
            onChange={(event) => onChange({ commandBlacklist: splitLines(event.target.value) })}
            placeholder="rm -rf\nshutdown\nformat"
          />
        </Field>
      </FieldGroup>
