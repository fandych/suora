...
const PreferenceAboutPanel = ({
  ...
  return (
    <PreferenceSectionCard
      id="about"
      title="About"
      description="Version and update status."
      actions={
    ...
    >
      <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
        <div className="rounded-lg border px-4 py-3">
          <div className="text-xs uppercase tracking-[0.12em] text-muted-foreground">Product</div>
          <div className="mt-2 text-base font-medium text-foreground">{systemInfo?.productName ?? "SUORA"}</div>
        </div>
        <div className="rounded-lg border px-4 py-3">
          <div className="text-xs uppercase tracking-[0.12em] text-muted-foreground">Version</div>
          <div className="mt-2 text-base font-medium text-foreground">{systemInfo?.version ?? "-"}</div>
        </div>
        <div className="rounded-lg border px-4 py-3">
          <div className="text-xs uppercase tracking-[0.12em] text-muted-foreground">Platform</div>
          <div className="mt-2 text-base font-medium text-foreground">{systemInfo?.platform ?? "-"}</div>
        </div>
        <div className="rounded-lg border px-4 py-3">
          <div className="text-xs uppercase tracking-[0.12em] text-muted-foreground">Mode</div>
          <div className="mt-2 flex items-center gap-2 text-base font-medium text-foreground">
            {systemInfo?.isDev ? "Development" : "Packaged"}
            {systemInfo ? <Badge variant="outline">PID runtime</Badge> : null}
          </div>
        </div>
      </div>
      <div className="grid gap-3 md:grid-cols-3">
        <div className="rounded-lg border px-4 py-3">
          <div className="text-xs uppercase tracking-[0.12em] text-muted-foreground">Node.js</div>
          <div className="mt-2 text-sm font-medium text-foreground">{systemInfo?.nodeVersion ?? "-"}</div>
        </div>
        <div className="rounded-lg border px-4 py-3">
          <div className="text-xs uppercase tracking-[0.12em] text-muted-foreground">Electron</div>
          <div className="mt-2 text-sm font-medium text-foreground">{systemInfo?.electronVersion ?? "-"}</div>
        </div>
        <div className="rounded-lg border px-4 py-3">
          <div className="text-xs uppercase tracking-[0.12em] text-muted-foreground">Chrome</div>
      ...
      <div className="flex items-center gap-3 text-sm text-muted-foreground">
        <Badge variant={updaterState?.enabled ? "secondary" : "outline"}>
          {updaterState?.enabled ? "Updater enabled" : "Updater unavailable"}
        </Badge>
