        ...
        <div className="mx-auto flex max-w-7xl flex-col gap-4">
          ...
          {!isLoading && !error && data?.length ? (
            <SummaryCardGrid
              ...
              emptyDescription="Create the first workflow to start building a reusable execution graph."
              items={data}
              renderItem={(workflow) => (
                <WorkflowCard
                  key={workflow.id}
          ...
          {!isLoading && !error && data?.length === 0 ? (
            <EmptyCard
              ...
              description="Create the first workflow to start building a reusable execution graph."
            />
          ) : null}
        </div>
      </div>
