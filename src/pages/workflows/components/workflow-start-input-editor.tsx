...
import {
  getNestedSchemaFragment,
  mergeNestedSchemaFragment,
  readWorkflowSchemaParameters,
  type WorkflowSchemaParameter,
...
const EMPTY_DRAFT: InputParameterDraft = {
  name: "",
  description: "",
  type: "string",
  defaultValue: "",
...
export function WorkflowStartInputEditor({
  value,
  onChange,
  title = "Parameters",
  variableSupport = false,
  suggestions = [],
}: WorkflowStartInputEditorProps) {
    ...
    const next =
      editingId === "new"
        ...
        : parameters.map((parameter) =>
            parameter.id === editingId ? { ...nextParameter, id: parameter.id } : parameter,
          )
    ...
    <div className="flex flex-col gap-2 pt-1">
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-1.5 text-[10px] font-semibold text-muted-foreground">
          {title}
          <Popover>
            <PopoverTrigger aria-label="About Parameters" className="text-muted-foreground hover:text-foreground">
            ...
            <PopoverContent align="start" className="w-72">
              ...
              <PopoverDescription>
                {variableSupport
                  ? "Use {{current.field}} for this node's raw result or ${steps.node-id.field} for an earlier node. Mapped fields join this node's result."
                  : "Define the fields accepted by this workflow node."}
              </PopoverDescription>
            </PopoverContent>
          </Popover>
        </div>
      ...
      {editingId ? (
        <div className="flex flex-col gap-2 rounded-xl bg-muted/20 p-2">
          <div className="grid gap-2 sm:grid-cols-2">
            <WorkflowField label="Name" hint="The input field name used by the workflow.">
              <Input
                value={draft.name}
            ...
            <WorkflowField label="Type" hint="The value type accepted by this field.">
              <NativeSelect
                value={draft.type}
                onChange={(event) => setDraft({ ...draft, type: event.target.value as InputParameterDraft["type"] })}
              >
          ...
          <WorkflowField label="Description" hint="Explain what this field represents.">
            <Textarea
          ...
          <WorkflowField
            label={variableSupport ? "Value" : "Default Value"}
            hint={
              variableSupport
                ? "Type ${ to choose current results, input fields, prior steps, or variables."
                : "Value used when no input is provided."
            }
          >
            {variableSupport ? (
              ...
              <Input
                value={draft.defaultValue}
                onChange={(event) => setDraft({ ...draft, defaultValue: event.target.value })}
          ...
          {draft.type === "object" || draft.type === "array" ? (
            <WorkflowField
              ...
              hint="Optional JSON Schema properties or items. This is preserved when editing the field."
              error={nestedContractError}
            >
              <Textarea
                aria-invalid={Boolean(nestedContractError)}
                value={nestedContractDraft}
                onChange={(event) => {
                  const result = mergeNestedSchemaFragment(draft.schema, event.target.value)
                  setNestedContractDraft(event.target.value)
                  setNestedContractError(result.error)
                  if (!result.error) setDraft({ ...draft, schema: result.schema })
                }}
                placeholder={
                  draft.type === "object"
          ...
          <WorkflowField label="Required" hint="Whether this field must be provided.">
      ...
      <div className="flex min-w-0 w-full max-w-full flex-col gap-2 overflow-hidden">
            ...
            >
              <div className="min-w-0 w-full max-w-full overflow-hidden">
                <PopoverTrigger className="block w-full min-w-0 max-w-full overflow-hidden text-left" render={<div />}>
                  <div
                    className="grid w-full min-w-0 max-w-full grid-cols-[minmax(0,1fr)_auto] items-start gap-1 overflow-hidden rounded-lg border px-2.5 py-2 text-xs"
                  ...
                  >
                    <div className="min-w-0 overflow-hidden wrap-anywhere">
                      ...
                      <p className="truncate text-muted-foreground" title={parameter.description || "No description"}>
              ...
              >
                ...
                <div className="flex flex-col gap-1 text-xs text-muted-foreground">
                  ...
                  <span>Required: {parameter.required ? "Yes" : "No"}</span>
                  {parameter.defaultValue ? (
                    <span>
                      {variableSupport ? "Value" : "Default Value"}: {parameter.defaultValue}
                    </span>
                  ) : null}
                </div>
              </PopoverContent>
        ...
        {parameters.length === 0 ? (
          <p className="rounded-lg border border-dashed px-3 py-4 text-center text-xs text-muted-foreground">
            No parameters yet. Add the inputs accepted by this workflow.
          </p>
        ) : null}
      </div>
    </div>
