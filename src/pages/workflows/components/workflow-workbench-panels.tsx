...
import { WORKFLOW_NODE_ICONS } from "@/pages/workflows/components/workflow-canvas-node"
...
const libraryGroups: Array<{ title: string; kinds: WorkflowNodeData["kind"][] }> = [
  { title: "Core", kinds: ["start", "end", "condition", "if-else", "fork", "join", "parallel", "loop", "serial"] },
...
export function WorkflowLibraryPanel({
    ...
    .map((group) => ({
      ...
      items: presets.filter(
        (item) =>
          ...
          (!normalizedQuery || `${item.label} ${item.summary}`.toLowerCase().includes(normalizedQuery)),
      ),
    }))
  ...
  return (
    <div className="flex max-h-[calc(100vh-10rem)] min-h-0 w-48 max-w-[calc(100vw-6rem)] flex-col gap-2 overflow-hidden rounded-xl border bg-background/95 p-2 shadow-xl">
      ...
      {!canEdit ? (
        <p className="rounded-lg border border-amber-500/30 bg-amber-500/5 px-2 py-1.5 text-[10px] text-muted-foreground">
      ...
      <ScrollArea className="min-h-0 flex-1">
        <div className="flex flex-col gap-3 p-2.5 pb-5">
          ...
          {groupedPresets.length === 0 ? (
            <p className="rounded-lg border border-dashed px-2 py-5 text-center text-xs text-muted-foreground">
              No node types match.
            </p>
          ) : null}
        </div>
