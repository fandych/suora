  ...
  if (start >= 0) {
    ...
    if (closing === -1 || closing >= cursor - (syntax === "template" ? 1 : 2)) {
      ...
      return {
        ...
        query: value
          .slice(start + (syntax === "template" ? 2 : 2), cursor)
          .replace(/}\}?\s*$/, "")
          .trim(),
        syntax,
      }
    ...
    <div className="relative">
      <Control
        ...
        onKeyDown={(event) => {
          ...
          if ((event.key === "Enter" || event.key === "Tab") && visibleSuggestions[activeIndex]) {
            event.preventDefault()
      ...
      {open ? (
        <div
          id={menuId}
          role="listbox"
          className="absolute z-50 mt-1 max-h-60 w-full overflow-y-auto rounded-lg bg-popover p-1 shadow-md ring-1 ring-foreground/10"
        >
          {groups.map((group) => {
                ...
                {groupSuggestions.map((suggestion) => {
                  ...
                  return (
                    <button
                      ...
                      type="button"
                      className="flex w-full flex-col gap-0.5 rounded-md px-2 py-1.5 text-left text-sm hover:bg-accent data-[active=true]:bg-accent"
                    ...
                    >
                      <span className="font-mono text-xs">
                        {formatSuggestion(suggestion.expression, expressionRange?.syntax)}
                      </span>
