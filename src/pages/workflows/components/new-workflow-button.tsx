...
export function NewWorkflowButton({ className, iconOnly = false }: { className?: string; iconOnly?: boolean }) {
  ...
  return (
    <Button
      ...
      disabled={isCreating}
      aria-label={t("newWorkflow.button", "New workflow")}
      title={t("newWorkflow.button", "New workflow")}
    >
      {isCreating ? <Spinner /> : <PlusIcon />}
      {iconOnly ? null : isCreating ? t("newWorkflow.creating", "Creating...") : t("newWorkflow.button", "New workflow")}
    </Button>
  )
}
