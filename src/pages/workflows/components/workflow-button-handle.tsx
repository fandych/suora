...
import { WorkflowNodeHandle } from "@/pages/workflows/components/workflow-node-primitives"
...
export function WorkflowButtonHandle({
  ...
  onAdd: (kind: WorkflowNodeData["kind"]) => void
  getIcon: (kind: WorkflowNodeData["kind"]) => LucideIcon
}) {
  if (!canEdit || hasConnection)
    return (
      <WorkflowNodeHandle
          ...
          <DropdownMenuTrigger
            render={
              <Button
                ...
                variant="secondary"
                size="icon-sm"
                className="pointer-events-auto nodrag nopan rounded-full border border-border/70 bg-background/95 shadow-sm"
          ...
          <DropdownMenuContent side="bottom" align="center" className="w-48">
            {workflowPresetNodes
              ...
              .map((item) => {
                ...
                return (
                  <DropdownMenuItem key={item.kind} onClick={() => onAdd(item.kind)}>
                    <ItemIcon />
                    {item.label}
                  </DropdownMenuItem>
                )
