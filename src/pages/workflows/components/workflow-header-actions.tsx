...
import VersionSelect from "@/pages/components/version-select"
    ...
    <div className="flex w-full items-center justify-between gap-3 rounded-2xl border bg-background/95 px-3 py-2 shadow-sm backdrop-blur">
      <div className="flex min-w-0 items-center gap-2">
        <Button
          ...
          aria-label={showLibrary ? "Collapse node library" : "Expand node library"}
        >
      ...
      <div className="flex items-center gap-2">
        ...
        <TooltipProvider>
          ...
          <Tooltip>
            <TooltipTrigger
              render={
                <Button size="icon-sm" onClick={onPublish} disabled={!canPublish} aria-label="Publish workflow" />
              }
            >
              <UploadIcon />
            </TooltipTrigger>
...
export function WorkflowRevisionActions({
  ...
  "versions" | "selectedVersionId" | "onVersionChange" | "canSave" | "canPublish" | "onSave" | "onPublish"
> & { canTryRun: boolean; onOpenTryRun: () => void }) {
  return (
