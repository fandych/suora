...
export function WorkflowNodeSearchControl({
  ...
  const results = useMemo(() => {
    ...
    return nodes
      .filter((node) =>
        `${node.data.label} ${node.data.task ?? ""} ${node.data.description ?? ""} ${node.data.kind}`
          .toLowerCase()
          .includes(keyword),
      )
      .slice(0, 8)
      ...
      <div className="mt-2 max-h-56 overflow-y-auto">
        {results.length > 0 ? (
          ...
          <div className="px-2 py-3 text-xs text-muted-foreground">
            {query.trim() ? "No matching nodes." : "Type to search nodes."}
          </div>
        )}
      </div>
    </div>
...
export function WorkflowIssuesControl({
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger
        render={<Button type="button" variant="outline" size="sm" className="bg-background/95 shadow-lg" />}
      >
        {issues.length > 0 ? (
      ...
      <DropdownMenuContent align="start" className="max-h-72 w-80 overflow-y-auto">
        {issues.length === 0 ? (
          <DropdownMenuItem disabled>No design issues.</DropdownMenuItem>
        ) : (
