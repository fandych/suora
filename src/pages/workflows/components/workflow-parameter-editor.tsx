...
export function WorkflowKeyValueEditor({
  label,
  value,
  onChange,
  ...
  const entries = (() => {
    try {
      ...
      return parsed && typeof parsed === "object" && !Array.isArray(parsed)
        ? Object.entries(parsed).map(([key, item]) => ({ key, value: String(item) }))
        : []
    } catch {
      return []
    }
  ...
  const writeEntries = (nextEntries: Array<{ key: string; value: string }>) => {
    onChange(
      JSON.stringify(
        Object.fromEntries(
          nextEntries.filter((entry) => entry.key.trim()).map((entry) => [entry.key.trim(), entry.value]),
        ),
        null,
        2,
      ),
    ...
    <WorkflowField label={label} hint="Values support workflow interpolation such as {{input.requestId}}.">
      <div className="flex flex-col gap-2">
        {entries.map((entry, index) => (
          <div key={`${entry.key}-${index}`} className="grid grid-cols-[1fr_1fr_auto] gap-2">
            <Input
              ...
              onChange={(event) =>
                writeEntries(
                  entries.map((item, itemIndex) => (itemIndex === index ? { ...item, key: event.target.value } : item)),
                )
              }
            />
            {suggestions ? (
              <WorkflowExpressionInput
                ...
                onChange={(entryValue) =>
                  writeEntries(
                    entries.map((item, itemIndex) => (itemIndex === index ? { ...item, value: entryValue } : item)),
                  )
                }
            ...
            <Button
              ...
              size="icon-xs"
              variant="ghost"
              onClick={() => writeEntries(entries.filter((_item, itemIndex) => itemIndex !== index))}
        ...
        <Button
          ...
          size="sm"
          variant="outline"
          onClick={() => writeEntries([...entries, { key: "", value: "" }])}
        >
          <PlusIcon /> Add entry
        </Button>
...
export function WorkflowParameterEditor({
  ...
  return (
    <div className="flex flex-col gap-2">
            ...
            <div className="flex items-center gap-2">
              <div className="min-w-0 flex-1">
                <p className="truncate text-sm font-medium">
                  {item.name || item.namePlaceholder || "Unnamed parameter"}
                </p>
                <p className="truncate text-xs text-muted-foreground">
                  {item.value || item.valuePlaceholder || "No value"}
                </p>
              </div>
              <Button
                type="button"
                ...
                aria-label={`Edit ${item.name || "parameter"}`}
              >
                <PencilIcon />
              </Button>
            </div>
      ...
      {onAdd ? (
        <Button type="button" variant="outline" size="sm" aria-label={addLabel} title={addLabel} onClick={onAdd}>
          <PlusIcon />
          {addLabel}
        </Button>
      ) : null}
