...
import type { WorkflowInvocationRecord } from "@/types/workflow"
import { downloadJson } from "@/lib/browser/file-exports"
import { WorkflowTraceJsonPreview } from "@/pages/workflows/components/workflow-trace-json-preview"
import {
  WorkflowTraceCopyButton,
  WorkflowTraceValueSection,
} from "@/pages/workflows/components/workflow-trace-sections"
import { WorkflowPanelResizeHandle } from "@/pages/workflows/components/workflow-panel-resize-handle"
import { WorkflowTraceSnapshotViewer } from "@/pages/workflows/components/workflow-trace-snapshot-viewer"
...
function getInvocationMetadata(invocation: WorkflowInvocationRecord) {
  try {
    return JSON.parse(invocation.output) as {
      durationMs?: number
      requestId?: string
...
export function WorkflowTryPanel({
  invocation,
  input,
  isRunning,
  error,
  ...
  return (
    <div
      className="relative flex h-full max-w-[calc(100vw-1.5rem)] min-h-0 flex-col overflow-hidden rounded-xl border bg-background/95 p-2 shadow-xl"
      style={{ width }}
    >
      <WorkflowPanelResizeHandle
        label="Resize try panel"
        onResize={(deltaX) => onWidthChange(Math.min(520, Math.max(260, width - deltaX)))}
      />
      <div className="flex items-center justify-between gap-2 border-b px-1 pb-2 text-xs font-semibold">
        ...
        <div className="flex items-center gap-1">
          <Button
            ...
            size="icon-xs"
            disabled={!invocation}
            onClick={() => invocation && downloadJson(`workflow-try-${invocation.id}.json`, invocation)}
            aria-label="Export try result"
            title="Export try result"
          >
            <DownloadIcon />
          </Button>
      ...
      <div className="min-h-0 flex-1 space-y-3 overflow-y-auto p-1 pb-5">
        ...
        <p className="rounded-lg border border-amber-500/30 bg-amber-500/5 px-2.5 py-2 text-[11px] text-muted-foreground">
          Safe dry runs do not call AI, HTTP, scripts, integrations, or email. Use Run release for real side effects.
        </p>
        ...
        {invocation ? (
          <div className="space-y-3 border-t pt-3">
            ...
            <div className="grid grid-cols-2 gap-2 text-[11px] text-muted-foreground">
              <div className="rounded-lg border px-2.5 py-2">
                ...
                <span className="font-medium text-foreground">{invocation.trigger}</span>
              </div>
              {(() => {
                ...
                return (
                  <div className="rounded-lg border px-2.5 py-2">
                    ...
                    <span className="font-medium text-foreground">
                      {metadata.durationMs ??
                        Math.max(0, ...invocation.traces.map((trace) => trace.finishedAt - trace.startedAt))}{" "}
                      ms
                    </span>
                  </div>
                )
            ...
            {(() => {
              ...
              return metadata.requestId ? (
                <p className="truncate text-[10px] text-muted-foreground">
                  Request {metadata.requestId} · {metadata.executionTarget ?? "desktop"}
                </p>
              ) : null
            })()}
            ...
            {invocation.traces.map((trace) => (
              <Collapsible
                key={trace.traceId ?? `${trace.nodeId}-${trace.startedAt}`}
              ...
              >
                <CollapsibleTrigger className="group/trace flex w-full items-center gap-2 px-3 py-2.5 text-left hover:bg-muted/60">
                  ...
                  <span className="text-[10px] tabular-nums text-muted-foreground">
                    {Math.max(0, trace.finishedAt - trace.startedAt)} ms
                  </span>
            ...
            <Collapsible className="rounded-xl border border-emerald-200 bg-emerald-50/70 dark:border-emerald-900 dark:bg-emerald-950/30">
              <div className="flex items-center gap-1">
                ...
                <WorkflowTraceCopyButton
                  copied={copiedKey === "final-output"}
                  onClick={() => void copyPreview("final-output", invocation.output)}
                  label="Copy final output"
                />
              </div>
              <CollapsibleContent className="border-t border-emerald-200 px-3 pb-3 pt-2 dark:border-emerald-900">
                <WorkflowTraceJsonPreview
                  value={invocation.output || "No output"}
          ...
          <div className="rounded-xl border border-dashed px-3 py-4 text-xs text-muted-foreground">
            No try run yet. Run the current draft to inspect node-level inputs and outputs.
          </div>
        )}
      </div>
    </div>
