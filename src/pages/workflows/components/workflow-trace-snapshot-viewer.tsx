...
import type { WorkflowTraceSnapshot } from "@/types/workflow"
      ...
      <div className="flex items-center justify-between gap-2">
        <div className="flex gap-1">
          <Button
            type="button"
            size="xs"
            variant={phase === "before" ? "secondary" : "ghost"}
          ...
          <Button
            type="button"
            size="xs"
            variant={phase === "after" ? "secondary" : "ghost"}
        ...
        >
          ...
          <NativeSelectOption value="vars">$vars</NativeSelectOption>
          <NativeSelectOption value="steps">$steps</NativeSelectOption>
          <NativeSelectOption value="input">$input</NativeSelectOption>
        </NativeSelect>
      </div>
      {snapshot.redactedPaths.length > 0 || snapshot.truncated ? (
        <p className="text-[10px] text-muted-foreground">
          {snapshot.redactedPaths.length ? `${snapshot.redactedPaths.length} values redacted` : ""}
          {snapshot.redactedPaths.length && snapshot.truncated ? " · " : ""}
