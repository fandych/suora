...
import { DocumentEditorPanel } from "@/pages/documents/components/document-editor-panel"
import { DocumentTreePanel } from "@/pages/documents/components/document-tree-panel"
...
const DocumentsDetailPage = () => {
  ...
  const { draft, error, isLoading, load, updateDraft, updatePages, updatePage, updateDocument, reload } =
  ...
  const commitAddOrRename = () => {
    ...
    if (entryDialogMode.kind === "rename") {
      ...
      const siblingTitles = draft.pages
        .filter((page) => page.id !== target.id && page.parentId === target.parentId)
        .map((page) => page.title)
      if (
        siblingTitles.includes(nextTitle) ||
        ((target.type ?? "document") === "document" &&
          siblingTitles.includes(getUniqueDocumentTitle(siblingTitles, nextTitle)))
      ) {
        ...
        pages: draft.pages.map((page) =>
          page.id === target.id
            ? {
                ...page,
                title:
                  (page.type ?? "document") === "document"
                    ? getUniqueDocumentTitle(
                        draft.pages.filter((candidate) => candidate.id !== page.id).map((candidate) => candidate.title),
                        nextTitle,
                      )
                    : nextTitle,
              }
    ...
    const nodeTitle =
      type === "folder"
        ? getUniqueDocumentTitle(siblingTitles, rawValue)
        : getUniqueDocumentTitle(siblingTitles, rawValue)
    const nextNode = {
  ...
  const handleUploadChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    for (const file of files) {
      nextPages.push({
        id: globalThis.crypto.randomUUID(),
        title: getUniqueDocumentTitle(
          nextPages.map((page) => page.title),
          file.name,
        ),
  ...
  const toggleDocumentEnabled = () => {
    if (draft) updateDocument({ enabled: !draft.document.enabled })
  }
  ...
  return (
    <div className="flex min-h-full flex-col bg-background">
          ...
          >
            <ResizablePanel defaultSize={28} minSize={18} className="min-w-0">
              <DocumentTreePanel
                ...
                onAddFile={(parentId) => {
                  setEntryDialogMode({ kind: "add-file", parentId })
                  setEntryDialogValue("new-document.md")
                  setEntryDialogError("")
                }}
                onDelete={(nodeId) => setDeleteTargetId(nodeId)}
                onExport={exportNode}
                onRename={(nodeId) => {
                  const target = draft.pages.find((page) => page.id === nodeId)
                  setEntryDialogMode({ kind: "rename", targetId: nodeId })
                  setEntryDialogValue(target?.title ?? "")
                  setEntryDialogError("")
                }}
      ...
      <DocumentCreateDialog
        dialogDescription="Update the document name and description for this workspace."
        dialogTitle="Edit document details"
        onDescriptionChange={(value) =>
          setDraft((current) => (current ? { ...current, document: { ...current.document, summary: value } } : current))
        }
        onOpenChange={setIsMetadataDialogOpen}
        onSubmit={() => setIsMetadataDialogOpen(false)}
        onTitleChange={(value) =>
          setDraft((current) => (current ? { ...current, document: { ...current.document, title: value } } : current))
        }
      ...
      />
      <ConfirmDeleteDialog
        description="This permanently deletes the document and all of its content. This action cannot be undone."
        onConfirm={() => void handleDeleteDocument()}
        onOpenChange={setIsDocumentDeleteDialogOpen}
      ...
      <ResourceEntryDialog
        description={
          entryDialogMode?.kind === "rename"
            ? "Rename the selected page or folder."
            : "Create a new document or folder inside the selected parent."
        }
        errorMessage={entryDialogError}
        fieldLabel={entryDialogMode?.kind === "rename" ? "New name" : "Name"}
        onOpenChange={(open) => {
          if (!open) {
            setEntryDialogMode(null)
            setEntryDialogError("")
        ...
        placeholder={entryDialogMode?.kind === "add-file" ? "new-document.md" : "new-folder"}
      ...
      />
      <ConfirmDeleteDialog
        description={deleteTargetId ? `Delete this item and all nested content under it?` : "Delete this item?"}
        onConfirm={() => (deleteTargetId ? deleteNode(deleteTargetId) : undefined)}
        onOpenChange={(open) => {
