...
import { getDocumentSourceLanguage, isMarkdownDocumentTitle } from "@/pages/documents/document-tree"
...
export function DocumentEditorPanel({
  ...
  const effectiveEditorMode =
    selectedIsDocument && selectedPage && !isMarkdownDocumentTitle(selectedPage.title) ? "source" : editorMode
  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
        ...
        {selectedIsDocument && selectedPage ? (
          previewKind === "markdown" || previewKind === "data" || previewKind === "script" ? (
            <DocumentContentEditor
              ...
              onChange={onChange}
              sourceLanguage={
                effectiveEditorMode === "source" ? getDocumentSourceLanguage(selectedPage.title) : undefined
              }
            />
          ) : (
          ...
          <div className="flex h-full items-center justify-center rounded-lg border border-dashed text-sm text-muted-foreground">
            Create or upload files into this folder.
          </div>
        )}
      </div>
      <div className="flex items-center justify-between gap-3 border-t px-3 py-2 text-xs text-muted-foreground">
        ...
        {selectedIsDocument && (previewKind === "markdown" || previewKind === "data" || previewKind === "script") ? (
          <NativeSelect
            className="w-24 shrink-0"
            value={effectiveEditorMode}
            onChange={(event) => onEditorModeChange(event.target.value as "rich" | "source")}
            size="sm"
          >
            <NativeSelectOption
              value="rich"
              disabled={selectedPage ? !isMarkdownDocumentTitle(selectedPage.title) : false}
            >
              Rich
            </NativeSelectOption>
