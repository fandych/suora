...
import {
  ...
  SKILL_ROOT_PATH,
} from "@/lib/skill/editor-utils"
import PageHeader from "@/pages/components/page-header"
import { ErrorCard, LoadingCard } from "@/pages/components/resource-state"
import { SkillEditorPanel } from "@/pages/skills/components/skill-editor-panel"
import { SkillTreePanel } from "@/pages/skills/components/skill-tree-panel"
...
const SkillsDetailPage = () => {
  ...
  const [entryDialogMode, setEntryDialogMode] = useState<{
    kind: "add-file" | "add-directory" | "rename"
    parentPath?: string
    targetPath?: string
  } | null>(null)
  ...
  const persistDraft = async (nextDraft: SkillDetail) => {
    ...
    const saved = await SkillApi.saveDraft(skillId ?? "", {
    ...
    const hasSelectedFile = next.files.some(
  ...
  const autosave = useAutosaveStatus({
    enabled: !isExternalSkill && hasLoadedInitialState && Boolean(skillId && draft),
  ...
  const treeEntries = useMemo(
    ...
    [draft?.files, draft?.skill.title],
  )
  const visibleEntries = useMemo(
    () =>
      treeEntries.filter((entry) => {
  ...
  const commitAddOrRename = () => {
    ...
    if (entryDialogMode.kind === "rename") {
      ...
      if (
        draft.files.some(
          (file) => normalizeSkillPath(file.path) === normalizedNext && normalizeSkillPath(file.path) !== sourcePath,
        )
      ) {
      ...
      const nextFiles = draft.files.map((file) => {
        ...
        if (currentPath.startsWith(`${sourcePath}/`))
          return { ...file, path: `${normalizedNext}/${currentPath.slice(sourcePath.length + 1)}` }
        return file
      })
    ...
    const nextFile: SkillFileRecord =
      entryDialogMode.kind === "add-directory"
        ? { path: nextPath, content: "", language: "plaintext", kind: "directory", executable: false }
        : {
            path: nextPath,
            content: nextPath.startsWith("scripts/")
              ? "export async function main(input: unknown) {\n  return { ok: true, input }\n}\n"
              : "",
            language: getSkillSourceLanguage(nextPath),
            kind: "file",
            executable: nextPath.startsWith("scripts/"),
          }
  ...
  const applyUpload = (files: SkillFileRecord[], conflictPaths: string[], overwrite: boolean) => {
    ...
    const uploadedPaths = files
      .filter((file) => overwrite || !conflictSet.has(normalizeSkillPath(file.path)))
  ...
  const handleUploadChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    for (const file of inputFiles) {
      ...
      const existingPath = draft.files.some(
        (candidate) => normalizeSkillPath(candidate.path) === normalizeSkillPath(preferredPath),
      )
      const nextPath = existingPath
        ? normalizeSkillPath(preferredPath)
        : getUniqueSkillPath(draft.files.concat(nextFiles), normalizeSkillPath(preferredPath))
    ...
    const conflictPaths = nextFiles
      ...
      .filter(
        (path, index, paths) =>
          paths.indexOf(path) !== index || draft.files.some((candidate) => normalizeSkillPath(candidate.path) === path),
      )
  ...
  const handleSkillAction = async (actionId: "disable" | "delete") => {
    ...
    try {
      ...
      await SkillApi.saveDraft(skillId, {
  ...
  return (
    <div className="flex min-h-full flex-col bg-background">
      <PageHeader
        title={draft?.skill.title ?? "Skill"}
        description={draft?.skill.summary}
        actions={draft && !isExternalSkill ? skillActions : null}
      />
      ...
      <div className="min-h-0 flex-1 p-3">
        ...
        {isExternalSkill && draft ? (
          <div className="mb-3 rounded-lg border border-amber-500/30 bg-amber-500/10 px-3 py-2 text-sm text-muted-foreground">
            External Skill is read-only. Changes are not saved to the source directory.
          </div>
        ) : null}
        {!isLoading && !error && draft ? (
          <ResizablePanelGroup
            orientation="horizontal"
            className="min-h-[calc(100vh-9.5rem)] overflow-hidden rounded-xl border bg-background"
          >
            <ResizablePanel defaultSize={28} minSize={18} className="min-w-0">
              <SkillTreePanel
                collapsedPaths={collapsedPaths}
                entries={visibleEntries}
                onAddDirectory={(parentPath) => {
                  if (isExternalSkill) return
                  setEntryDialogMode({ kind: "add-directory", parentPath })
                  setEntryDialogValue("new-folder")
                  setEntryDialogError("")
                }}
                onAddFile={(parentPath) => {
                  if (isExternalSkill) return
                  setEntryDialogMode({ kind: "add-file", parentPath })
                  setEntryDialogValue(getDefaultSkillFileName(parentPath))
                  setEntryDialogError("")
                }}
                onDelete={(path) => {
                  if (!isExternalSkill) setDeleteTargetPath(path)
                }}
                onExport={exportEntry}
                onRename={(path) => {
                  if (isExternalSkill) return
                  setEntryDialogMode({ kind: "rename", targetPath: path })
                  setEntryDialogValue(path)
                  setEntryDialogError("")
                }}
            ...
            <ResizablePanel defaultSize={72} minSize={30} className="min-w-0">
              <SkillEditorPanel
                ...
                onChange={isExternalSkill ? () => undefined : handleFileContentChange}
              />
            </ResizablePanel>
          </ResizablePanelGroup>
        ) : null}
      ...
      <ResourceEntryDialog
        description={
          entryDialogMode?.kind === "rename"
            ? "Rename the selected file or folder. Descendant paths will move with the folder."
            : "Create a new item inside the selected folder."
        }
        errorMessage={entryDialogError}
        fieldLabel={entryDialogMode?.kind === "rename" ? "New path" : "File or folder name"}
        onOpenChange={(open) => {
          if (!open) {
            setEntryDialogMode(null)
            setEntryDialogError("")
        ...
        onSubmit={commitAddOrRename}
        onValueChange={setEntryDialogValue}
        open={Boolean(entryDialogMode)}
        placeholder={entryDialogMode?.kind === "rename" ? "references/guide.md" : "guide.md"}
        submitLabel={entryDialogMode?.kind === "rename" ? "Rename" : "Create"}
        title={
          entryDialogMode?.kind === "rename"
            ? "Rename item"
            : entryDialogMode?.kind === "add-directory"
      ...
      />
      <ConfirmDeleteDialog
        description={deleteTargetPath ? `Delete ${deleteTargetPath} and any nested items?` : "Delete this item?"}
        onConfirm={() => (deleteTargetPath ? deleteEntry(deleteTargetPath) : undefined)}
        onOpenChange={(open) => {
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {pendingUpload?.conflictPaths.length === 1
                ? `The file ${pendingUpload.conflictPaths[0]} already exists.`
                : `${pendingUpload?.conflictPaths.length ?? 0} files already exist.`}{" "}
              Choose whether to overwrite them.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              onClick={() =>
                pendingUpload ? applyUpload(pendingUpload.files, pendingUpload.conflictPaths, true) : undefined
              }
            >
              Overwrite
            </AlertDialogAction>
