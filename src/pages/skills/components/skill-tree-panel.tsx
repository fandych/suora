...
import { SKILL_ROOT_PATH, type SkillTreeEntry } from "@/lib/skill/editor-utils"
      ...
      {entries.map((entry) => {
        ...
        return (
          <ContextMenu key={entry.path}>
              ...
              <div className={`${rowClass} ${entry.path === selectedPath ? "bg-muted" : ""}`}>
                ...
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end" className="w-44 min-w-44">
                    {isDirectory ? (
                      <DropdownMenuItem onClick={() => onAddFile(entry.path)}>
                        <FilePlus2Icon className="size-4" />
                        New file
                      </DropdownMenuItem>
                    ) : null}
                    {isDirectory ? (
                      <DropdownMenuItem onClick={() => onAddDirectory(entry.path)}>
                        <FolderPlusIcon className="size-4" />
                        New folder
                      </DropdownMenuItem>
                    ) : null}
                    {isDirectory ? (
                      <DropdownMenuItem onClick={() => onUpload(entry.path)}>
                        <UploadIcon className="size-4" />
                        Upload
                      </DropdownMenuItem>
                    ) : null}
                    <DropdownMenuItem onClick={() => onExport(entry.path)}>
            ...
            <ContextMenuContent>
              {isDirectory ? (
                <ContextMenuItem onClick={() => onAddFile(entry.path)}>
                  <FilePlus2Icon className="size-4" />
                  New file
                </ContextMenuItem>
              ) : null}
              {isDirectory ? (
                <ContextMenuItem onClick={() => onAddDirectory(entry.path)}>
                  <FolderPlusIcon className="size-4" />
                  New folder
                </ContextMenuItem>
              ) : null}
              {isDirectory ? (
                <ContextMenuItem onClick={() => onUpload(entry.path)}>
                  <UploadIcon className="size-4" />
                  Upload
                </ContextMenuItem>
              ) : null}
              <ContextMenuItem onClick={() => onExport(entry.path)}>
                <DownloadIcon className="size-4" />
                Export
              </ContextMenuItem>
              {!isProtectedPath(entry.path) ? (
