...
function inlineMarkdown(value: string) {
  ...
  let result = escapeHtml(tokenized)
    ...
    .replace(
      /!\[([^\]]*)\]\(([^)]+)\)/g,
      (_, alt: string, src: string) => `<img src="${escapeAttr(src)}" alt="${escapeAttr(alt)}">`,
    )
    .replace(
      /\[\[([^\]\n]+)\]\]/g,
      (_, target: string) => `<a href="#doc:${escapeAttr(target)}">${escapeHtml(target)}</a>`,
    )
    .replace(
      /\[([^\]\n]+)\]\(([^)]+)\)/g,
  ...
  for (const line of lines) {
    ...
    if (tablePending.length > 0) {
      if (isTableSeparator(line)) {
        ...
        html.push(
          `<table><thead><tr>${headerCells.map((cell) => `<th>${inlineMarkdown(cell)}</th>`).join("")}</tr></thead><tbody>`,
        )
        inTable = true
        tablePending = []
        continue
    ...
    if (taskUnchecked) {
      ...
      html.push(
        `<li data-type="taskItem" data-checked="false"><label><input type="checkbox" /></label><div><p>${inlineMarkdown(taskUnchecked[1])}</p></div></li>`,
      )
      continue
    }
    ...
    if (taskChecked) {
      ...
      html.push(
        `<li data-type="taskItem" data-checked="true"><label><input type="checkbox" checked /></label><div><p>${inlineMarkdown(taskChecked[1])}</p></div></li>`,
      )
      continue
    }
