...
import { InlineMath, MathBlock, MermaidBlock } from "@/pages/components/document-extensions"
import { markdownToTiptapHtml, tiptapJsonToMarkdown } from "@/pages/components/document-markdown"
...
const DocumentContentEditor = ({ mode, value, onChange, sourceLanguage = "html" }: DocumentContentEditorProps) => {
  const editor = useEditor({
    ...
    editorProps: {
      attributes: {
        class:
          "document-prose h-full min-h-full rounded-xl border border-border bg-background px-4 py-3 text-sm outline-none",
      },
    },
  })
      ...
      <TooltipProvider>
        <div className="flex flex-nowrap items-center gap-2 overflow-x-auto px-1 pt-3">
          <NativeSelect
            ...
            value={editor?.getAttributes("heading").level ? `h${editor.getAttributes("heading").level}` : "paragraph"}
            onChange={(event) => {
          ...
          >
            ...
            {([1, 2, 3, 4, 5, 6] as const).map((level) => (
              <NativeSelectOption key={level} value={`h${level}`}>
                H{level}
              </NativeSelectOption>
            ))}
          </NativeSelect>
          <ButtonGroup>
            ...
            <ToolbarButton label="Thin" onClick={() => editor?.chain().focus().unsetBold().run()}>
              <TypeIcon />
            </ToolbarButton>
          </ButtonGroup>
          ...
          <ToolbarButton label="Ordered list" onClick={() => editor?.chain().focus().toggleOrderedList().run()}>
            <ListOrderedIcon />
          </ToolbarButton>
          <DropdownMenu>
            <Tooltip>
              <TooltipTrigger
                render={
                  <DropdownMenuTrigger
                    render={<Button size="icon-sm" variant="outline" aria-label="Table options" />}
                  />
                }
              >
                <Table2Icon />
          ...
          <ToolbarButton
            ...
            onClick={() =>
              editor
                ?.chain()
                .focus()
                .insertContent({ type: "mathBlock", attrs: { content: "x^2 + y^2 = z^2" } })
                .run()
            }
          >
            <SigmaIcon />
...
function ToolbarButton({
  ...
  return (
    <Tooltip>
      <TooltipTrigger render={<Button size="icon-sm" variant="outline" aria-label={label} />} onClick={onClick}>
        {children}
      </TooltipTrigger>
