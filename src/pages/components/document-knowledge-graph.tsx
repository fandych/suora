...
import type { DocumentGraphEdge, DocumentPageRecord } from "@/types/document"
...
const DocumentKnowledgeGraph = ({ pages, edges, filter }: DocumentKnowledgeGraphProps) => {
  const graph = useMemo(() => {
    ...
    const baseNodes: Node[] = pages.map((page, index) => ({
      ...
      style: normalized && !page.title.toLowerCase().includes(normalized) ? { opacity: 0.45 } : undefined,
    }))
