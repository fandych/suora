        ...
        <div className="flex w-full min-w-0 flex-col gap-4">
          ...
          {!isLoading && !error && orderedAgents.length ? (
            <SummaryCardGrid
              ...
              emptyDescription="Create the first agent to manage prompts, model bindings, and related skills."
              items={orderedAgents}
              renderItem={(agent) => (
          ...
          {!isLoading && !error && orderedAgents.length === 0 ? (
            <EmptyCard
              title="No agents yet"
              description="Create the first agent to manage prompts, model bindings, and related skills."
            />
          ) : null}
        </div>
      </div>
