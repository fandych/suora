        ...
        <div className="space-y-2 p-3">
          ...
          {items.map((item) => {
            ...
            return (
              <label
                key={item.id}
                className={`flex items-start gap-3 rounded-lg border bg-background px-3 py-2 transition-colors ${disabled ? "opacity-70" : "cursor-pointer hover:border-primary/30 hover:bg-muted/30"}`}
              >
                <Checkbox
                  checked={checked}
                  disabled={disabled}
...
export function AgentBindingsForm({
  documents,
  draft,
  integrations,
  isReadOnly = false,
  ...
  const buildItems = useMemo(
    () => (items: SelectableItem[], selectedIds: string[], query: string) => {
  ...
  const integrationItems = buildItems(
    ...
    integrationSelectedIds,
    queries.integrations,
  )
  const skillItems = buildItems(
    skills.map((skill) => ({ id: skill.id, title: skill.title, description: skill.summary })),
    skillSelectedIds,
    queries.skills,
  )
    ...
    <Card className="h-full min-h-0 min-w-0 overflow-hidden">
      ...
      <CardContent className="flex min-h-0 flex-1 flex-col overflow-hidden p-0">
        <Accordion
          ...
          onValueChange={(value) => setOpenSection(Array.isArray(value) ? String(value[0] ?? "") : String(value ?? ""))}
        >
          <AccordionItem
            value="workflows"
            className={`px-4 pt-2 ${openSection === "workflows" ? "flex min-h-0 flex-1 flex-col" : ""}`}
          >
          ...
          <AccordionItem
            value="integrations"
            className={`px-4 ${openSection === "integrations" ? "flex min-h-0 flex-1 flex-col" : ""}`}
          >
          ...
          <AccordionItem
            value="skills"
            className={`px-4 ${openSection === "skills" ? "flex min-h-0 flex-1 flex-col" : ""}`}
          >
          ...
          <AccordionItem
            value="documents"
            className={`px-4 pb-2 ${openSection === "documents" ? "flex min-h-0 flex-1 flex-col" : ""}`}
          >
