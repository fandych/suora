...
export function ModelSelector() {
    ...
    const handleTest = async () => {
        if (!selectedModel || testState.kind === 'pending')
            return;
        ...
        try {
            const result = await testConnection(selectedModel.providerType, selectedModel.apiKey || provider.apiKey || '', selectedModel.baseUrl || provider.baseUrl, selectedModel.modelId, selectedModel.provider);
            if (result.success) {
    ...
    return (<div className="p-4 border-b border-border-subtle">
      <div className="space-y-2">
        <label htmlFor="model-selector-select" className="text-xs font-medium text-text-muted uppercase tracking-wider">
          {t('chat.model', 'Model')}
        </label>
        <div className="flex items-stretch gap-2">
          <UiSelect id="model-selector-select" aria-label={t('chat.selectModelAria', 'Select model')} value={selectedModel?.id || ''} onChange={(e) => {
          ...
          <UiButton unstyled type="button" onClick={handleTest} disabled={!selectedModel || testState.kind === 'pending'} aria-label={t('models.testConnection', 'Test Connection')} {...(testState.kind === 'pending' ? { 'aria-busy': true } : {})} title={selectedModel ? t('models.testConnectionHint', 'Send a minimal request to verify this model works') : t('models.selectModelFirst', 'Select a model first')} className="px-3 rounded-xl bg-surface-2 hover:bg-surface-3 disabled:opacity-40 disabled:cursor-not-allowed text-text-primary text-xs font-medium border border-border transition-colors">
            {testState.kind === 'pending' ? (<span className="inline-flex items-center gap-1.5">
              ...
              </span>) : (t('common.test', 'Test'))}
          </UiButton>
        </div>

        {testState.kind === 'ok' && (<p role="status" className="text-[11px] text-emerald-500">
            ✓ {t('models.connectedWithLatency', 'Connected ({latency}ms)').replace('{latency}', String(testState.latency))}
          </p>)}
        {testState.kind === 'error' && (<p role="alert" className="text-[11px] text-red-400 wrap-break-word">
            ✗ {testState.message}
          </p>)}
      </div>
    </div>);
