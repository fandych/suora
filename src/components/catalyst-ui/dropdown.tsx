  ...
  return (
    <Headless.MenuItems
      ...
      className={clsx(
        ...
        // Base styles
        'isolate z-[110] w-max rounded-xl p-1',
        // Invisible border that is only visible in `forced-colors` mode for accessibility purposes
        'outline outline-transparent focus:outline-hidden',
        // Handle scrolling when menu won't fit in viewport
        'overflow-y-auto',
        // Popover background — use workbench theme tokens
        'bg-surface-2/95 backdrop-blur-xl',
        // Shadows and ring
        'shadow-xl ring-1 ring-border-subtle',
...
export function DropdownItem({
  ...
  let classes = clsx(
    ...
    // Focus
    'data-focus:bg-accent/15 data-focus:text-accent',
    // Disabled state
    ...
    '*:data-[slot=icon]:text-text-muted data-focus:*:data-[slot=icon]:text-accent',
    // Avatar
...
export function DropdownDivider({
  ...
  return (
    <Headless.MenuSeparator
      ...
      className={clsx(
        className,
        'col-span-full mx-3.5 my-1 h-px border-0 bg-border-subtle sm:mx-3 forced-colors:bg-[CanvasText]'
      )}
    />
  )
}
...
export function DropdownDescription({
  ...
  return (
    <Headless.Description
      ...
      className={clsx(
        className,
        'col-span-2 col-start-2 row-start-2 text-sm/5 text-text-muted group-data-focus:text-accent sm:text-xs/5 forced-colors:group-data-focus:text-[HighlightText]'
      )}
    />
  )
}
    ...
    >
      {(Array.isArray(keys) ? keys : keys.split('')).map((char, index) => (
        <kbd
          key={index}
          className={clsx([
            'min-w-[2ch] text-center font-sans text-text-muted capitalize group-data-focus:text-accent forced-colors:group-data-focus:text-[HighlightText]',
