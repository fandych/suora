...
export function DataSettings() {
    ...
    const handleImportFile = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        reader.onload = async () => {
            try {
                ...
                toast.error(t('settings.importFailed', 'Failed to import data'), err instanceof Error ? err.message : String(err));
                input.value = '';
            }
        };
        reader.onerror = () => {
            toast.error(t('settings.importFailed', 'Failed to import data'), reader.error?.message ?? t('settings.importReadFailed', 'Could not read file'));
            input.value = '';
        };
    ...
    return (<div className="space-y-6">
      <SettingsOverview description={t('settings.backupRetentionAndCleanupHint', 'Move data safely between workspaces, keep history under control, and make destructive actions unmistakably explicit.')} statsClassName="grid gap-2 sm:grid-cols-2 xl:grid-cols-4" stats={(<>
      ...
      <SettingsSection eyebrow={t('settings.backups', 'Backups')} title={t('settings.portableSnapshots', 'Portable Snapshots')} description={t('settings.portableSnapshotsHint', 'Export a clean workspace snapshot for migration or safekeeping, then restore it on another machine when needed.')}>
        <div className="grid gap-4 xl:grid-cols-2">
          <div className="rounded-3xl border border-border-subtle/55 bg-surface-0/45 p-5 shadow-[0_10px_24px_rgba(15,23,42,0.05)]">
            ...
            <UiButton unstyled type="button" onClick={handleExport} className="mt-5 inline-flex items-center gap-1.5 rounded-2xl border border-accent/30 bg-accent/15 px-5 py-3 text-sm font-semibold text-accent transition-colors hover:bg-accent/25">
          ...
          <div className="rounded-3xl border border-border-subtle/55 bg-surface-0/45 p-5 shadow-[0_10px_24px_rgba(15,23,42,0.05)]">
            ...
            <UiInput type="file" accept=".json" onChange={handleImportFile} className="hidden" id="import-data-file"/>
            <label htmlFor="import-data-file" className="mt-5 inline-flex cursor-pointer items-center gap-1.5 rounded-2xl bg-surface-3 px-5 py-3 text-sm font-semibold text-text-secondary transition-colors hover:bg-surface-4">
      ...
      <SettingsSection eyebrow={t('settings.historyRetention', 'History Retention')} title={t('settings.retentionPolicy', 'Retention Policy')} description={t('settings.historyRetentionDesc', 'Automatically delete conversations older than the specified number of days. Set to 0 to keep all history.')} action={historyRetentionDays > 0 ? (<UiButton unstyled type="button" onClick={() => void cleanOldSessions()} className="rounded-2xl border border-amber-500/18 bg-amber-500/10 px-4 py-3 text-sm font-semibold text-amber-400 transition-colors hover:bg-amber-500/18">
      ...
      <SettingsSection eyebrow={t('settings.clearData', 'Clear Data')} title={t('settings.dangerZone', 'Danger Zone')} description={t('settings.clearDataDesc', 'Permanently delete all chat history and sessions. This cannot be undone.')}>
        <div className="rounded-3xl border border-red-500/18 bg-red-500/8 p-5 shadow-[0_10px_24px_rgba(15,23,42,0.05)]">
          <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
            ...
            <UiButton unstyled type="button" onClick={() => void clearAllData()} className="inline-flex items-center gap-1.5 rounded-2xl bg-danger/10 px-5 py-3 text-sm font-semibold text-danger transition-colors hover:bg-danger/20">
