...
export function KnowledgeSettings() {
    ...
    const [kbResults, setKbResults] = useState<Array<{
        id: string;
        content: string;
        score: number;
    }>>([]);
    ...
    return (<div className="space-y-6">
      <SettingsSection eyebrow={t('settings.vectorMemoryIndex', 'Vector Memory Index')} title={t('settings.semanticMemoryLab', 'Semantic Memory Lab')} description={t('settings.rebuildIndexDesc', 'The index is automatically maintained when memories are added or removed. Use rebuild when you want to recompute the full TF-IDF corpus from scratch.')} action={<UiButton unstyled type="button" onClick={() => {
            ...
            }} disabled={kbRebuilding} className="rounded-2xl border border-accent/18 bg-accent/10 px-4 py-3 text-sm font-semibold text-accent transition-colors hover:bg-accent/18 disabled:opacity-50">
      ...
      <SettingsSection eyebrow={t('settings.testSemanticSearch', 'Test Semantic Search')} title={t('settings.queryWorkbench', 'Query Workbench')} description={t('settings.queryWorkbenchHint', 'Probe the in-browser semantic index with a sample query to see what memories would be recalled first.')} action={<UiButton unstyled type="button" onClick={runSearch} className="rounded-2xl bg-accent px-4 py-3 text-sm font-semibold text-white shadow-[0_10px_30px_rgba(var(--t-accent-rgb),0.22)] transition-colors hover:bg-accent-hover">
            {t('settings.search', 'Search')}
          </UiButton>}>
        <div className="flex gap-3">
          <UiInput value={kbQuery} onChange={(event) => setKbQuery(event.target.value)} onKeyDown={(event) => {
        ...
        <div className="mt-5 space-y-3">
          {kbResults.length > 0 ? (kbResults.map((result) => (<div key={result.id} className="rounded-3xl border border-border-subtle/55 bg-surface-0/45 p-4 shadow-[0_10px_24px_rgba(15,23,42,0.05)]">
              ...
              </div>))) : kbQuery ? (<div className="rounded-3xl border border-dashed border-border-subtle/60 bg-surface-0/35 px-4 py-10 text-center">
            ...
            </div>) : (<div className="rounded-3xl border border-dashed border-border-subtle/60 bg-surface-0/35 px-4 py-10 text-center">
              <div className="mx-auto mb-3 flex h-11 w-11 items-center justify-center rounded-2xl border border-border-subtle/45 bg-surface-2/65 text-text-muted/60">
                <IconifyIcon name="ui-search" size={18} color="currentColor"/>
              </div>
