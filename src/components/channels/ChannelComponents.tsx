...
export function ChannelCopyButton({ text, className = '' }: {
    ...
    return (<UiButton unstyled onClick={copy} title={copied ? t('common.copied', 'Copied') : t('common.copy', 'Copy')} className={`text-[11px] px-1.5 py-0.5 rounded-md transition-colors inline-flex items-center gap-1 ${copied ? 'text-success' : 'text-text-muted hover:text-text-secondary hover:bg-surface-3/60'} ${className}`}>
      {copied
...
function ChannelCodeBlock({ children, className, ...rest }: ComponentPropsWithoutRef<'code'>) {
    ...
    return (<div className="relative group/code my-3 rounded-xl overflow-hidden border border-border-subtle/80 bg-surface-0/60 shadow-sm">
...
function ChannelMarkdownContent({ content }: {
    content: string;
}) {
    return (<Markdown remarkPlugins={[remarkGfm]} components={CHANNEL_MD_COMPONENTS as Record<string, React.ComponentType<unknown>>}>
      {content}
    </Markdown>);
}
// ─── Relative Time ─────────────────────────────────────────────────
export function formatChannelRelativeTime(value: number | undefined, locale: string, emptyLabel = ''): string {
    ...
    if (diff < 3600000)
        return formatter.format(-Math.max(1, Math.floor(diff / 60000)), 'minute');
    if (diff < 86400000)
        return formatter.format(-Math.max(1, Math.floor(diff / 3600000)), 'hour');
    if (diff < 604800000)
        return formatter.format(-Math.max(1, Math.floor(diff / 86400000)), 'day');
...
export function normalizeChannelDirection(direction?: string): ChannelMessageDirection {
    ...
    if (normalized === 'outgoing' || normalized === 'send' || normalized === 'sent' || normalized === 'assistant')
        return 'outgoing';
    return 'incoming';
}
// ─── Channel Message Bubble ────────────────────────────────────────
export function ChannelMessageBubble({ msg, showChannel }: {
    msg: ChannelHistoryMessage;
    showChannel?: string;
}) {
    ...
    const statusCls = msg.status === 'sent' ? 'bg-green-500/15 text-green-400' :
    ...
    return (<div className={`group mb-4 flex animate-fade-in ${isOutgoing ? 'justify-end' : 'justify-start'}`}>
      {/* Incoming avatar */}
      {!isOutgoing && (<div className="mr-3 mt-1 flex h-9 w-9 shrink-0 items-center justify-center rounded-2xl border border-accent/10 bg-gradient-to-br from-accent/22 via-accent/8 to-transparent shadow-sm">
      ...
      <div className="flex max-w-[82%] flex-col">
        <div className={`px-4 py-3 text-[12.5px] leading-6 ${isOutgoing ? outgoingBubbleCls : incomingBubbleCls}`}>
          ...
          {!isOutgoing && (<div className="mb-2.5 flex min-w-48 flex-wrap items-center gap-2 border-b border-border-subtle/50 pb-2">
              <span className="max-w-48 truncate rounded-full bg-accent/10 px-2 py-0.5 text-[10px] font-semibold text-accent/85">
                {senderLabel}
              </span>
              {showChannel && (<span className="max-w-40 truncate rounded-full bg-surface-0/65 px-2 py-0.5 text-[10px] text-text-muted/70">
                  → {showChannel}
                </span>)}
          ...
          {!isOutgoing && (msg.status === 'failed' || msg.status === 'pending') && (<div className="mt-2 pt-1.5 border-t border-border-subtle/50">
        ...
        <div className={`mt-1.5 flex min-h-5 flex-wrap items-center gap-1.5 px-1 text-[9px] ${isOutgoing ? 'justify-end' : 'justify-start'}`}>
          ...
          {msg.content && (<ChannelCopyButton text={msg.content} className="opacity-0 group-hover:opacity-100 transition-opacity"/>)}
        </div>
      </div>

      {/* Outgoing avatar */}
      {isOutgoing && (<div className="ml-3 mt-1 flex h-9 w-9 shrink-0 items-center justify-center rounded-2xl border border-accent/10 bg-accent/15 text-[10px] font-semibold text-accent/80">
