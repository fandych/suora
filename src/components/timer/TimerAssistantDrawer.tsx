...
function describeVisibleSchedule(timer: ScheduledTask, locale: string, everyMinutesLabel: string) {
    if (timer.type === 'once')
        return new Date(timer.schedule).toLocaleString(locale);
    if (timer.type === 'interval')
        return everyMinutesLabel.replace('{minutes}', timer.schedule);
    return timer.schedule;
}
function describeVisibleAction(timer: ScheduledTask, labels: {
    pipeline: string;
    prompt: string;
    notify: string;
}) {
    if (timer.action === 'pipeline')
        return labels.pipeline;
    if (timer.action === 'prompt')
        return labels.prompt;
    return labels.notify;
}
function describeAssistantTimerType(timer: ScheduledTask, t: Translate) {
    if (timer.type === 'once')
        return t('timer.oneTime', 'One-time');
    if (timer.type === 'interval')
        return t('timer.interval', 'Interval');
    return t('timer.cronLabel', 'Cron');
}
function buildContextPrompt({ mode, timer, t, locale, everyMinutesLabel, actionLabels, }: {
    ...
    if (mode === 'edit' && timer) {
        ...
        lines.push(`${t('timer.assistantTargetTimerId', 'Target timer id')}: ${timer.id}`, `${t('timer.assistantCurrentName', 'Current name')}: ${timer.name}`, `${t('timer.assistantCurrentType', 'Current type')}: ${describeAssistantTimerType(timer, t)}`, `${t('timer.assistantCurrentSchedule', 'Current schedule')}: ${describeVisibleSchedule(timer, locale, everyMinutesLabel)}`, `${t('timer.assistantCurrentAction', 'Current action')}: ${describeVisibleAction(timer, actionLabels)}`, `${t('timer.assistantCurrentPrompt', 'Current prompt/body')}: ${timer.prompt || noneLabel}`, `${t('timer.assistantCurrentAgentId', 'Current agentId')}: ${timer.agentId || noneLabel}`, `${t('timer.assistantCurrentPipelineId', 'Current pipelineId')}: ${timer.pipelineId || noneLabel}`, `${t('timer.assistantCurrentTimezone', 'Current timezone')}: ${timer.timezone || Intl.DateTimeFormat().resolvedOptions().timeZone}`, `${t('timer.assistantCurrentMissedRunPolicy', 'Current missedRunPolicy')}: ${timer.missedRunPolicy || 'skip'}`, `${t('timer.assistantCurrentCalendarRule', 'Current calendarRule')}: ${timer.calendarRule || 'all-days'}`);
    }
    return lines.join('\n');
}
...
export function TimerAssistantDrawer({ mode, timer, onClose, onTimerMutated, }: {
    mode: TimerAssistantMode;
    timer?: ScheduledTask | null;
    onClose: () => void;
    onTimerMutated?: () => void;
}) {
    const { sessions, addSession, updateSession, selectedModel, models, } = useAppStore();
    ...
    const contextPrompt = useMemo(() => buildContextPrompt({ mode, timer, t, locale, everyMinutesLabel, actionLabels }), [actionLabels, everyMinutesLabel, locale, mode, t, timer]);
    useEffect(() => {
    ...
    useEffect(() => {
        ...
        for (const message of messages) {
            for (const toolCall of message.toolCalls ?? []) {
                if (!['timer_add', 'timer_update', 'timer_remove'].includes(toolCall.toolName))
                    continue;
    ...
    return (<div className="pointer-events-none absolute inset-y-0 right-0 z-40 flex w-full justify-end pl-16">
      <section role="dialog" aria-label={t('timer.assistantLabel', 'Timer assistant')} className="pointer-events-auto flex h-full w-full max-w-136 flex-col border-l border-border-subtle/55 bg-surface-0/94 shadow-[-24px_0_60px_rgba(15,23,42,0.22)] backdrop-blur-xl">
