...
const panelMonoInputClass = 'font-mono';
...
export function MarkdownEditor({ value, onChange, placeholder, rows = 10, disabled, fillHeight = false, }: {
    ...
    return (<div className={`overflow-hidden rounded-[28px] border border-border-subtle/60 bg-linear-to-br from-surface-2/82 via-surface-2/76 to-surface-3/58 shadow-[0_18px_40px_rgba(15,23,42,0.08)] ${fillHeight ? 'flex min-h-168 flex-col' : ''}`}>
      <div className="flex flex-wrap items-center justify-between gap-2 border-b border-border-subtle/55 bg-surface-3/30 px-3 py-2.5">
        <div className="inline-flex rounded-2xl border border-border-subtle bg-surface-0/65 p-1 text-[11px]">
          <UiButton unstyled type="button" onClick={() => setMode('edit')} className={`rounded-xl px-3 py-1.5 transition-colors ${mode === 'edit' ? 'bg-accent/15 text-accent font-medium' : 'text-text-muted hover:text-text-secondary'}`}>
            {t('skills.edit', 'Edit')}
          </UiButton>
          <UiButton unstyled type="button" onClick={() => setMode('preview')} className={`rounded-xl px-3 py-1.5 transition-colors ${mode === 'preview' ? 'bg-accent/15 text-accent font-medium' : 'text-text-muted hover:text-text-secondary'}`}>
            {t('skills.preview', 'Preview')}
          </UiButton>
        </div>

        <div className="flex items-center gap-2">
          {isPreview && (<span className="rounded-full border border-accent/20 bg-accent/10 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.16em] text-accent">
      ...
      {mode === 'edit' ? (<div className={`p-3 ${fillHeight ? 'min-h-0 flex-1' : ''}`}>
          ...
          {value.trim() ? (<div className={`rounded-[26px] border border-border-subtle/55 bg-surface-0/55 p-5 shadow-[inset_0_1px_0_rgba(255,255,255,0.02)] ${fillHeight ? 'min-h-full' : ''}`}>
            ...
            </div>) : (<div className="flex min-h-52 items-center justify-center rounded-[26px] border border-dashed border-border-subtle/60 bg-surface-0/35 px-5 text-center text-xs text-text-muted">
...
// ─── Skill Test Panel ─────────────────────────────────────────────
export function SkillTestPanel({ skill }: {
    skill: Skill;
}) {
    ...
    const toolNames = useMemo(() => {
        const names: string[] = (skill.tools ?? []).map((t: {
            name: string;
        }) => t.name);
    ...
    const handleRun = useCallback(async () => {
        ...
        setOutput(null);
        if (!selectedTool) {
            setError(t('skills.noToolSelected', 'No tool selected.'));
            return;
        }
        ...
        try {
            ...
            setError(t('skills.toolNoExecutableHandler', 'Tool "{name}" does not have an executable handler. It may be a reference-only tool.')
    ...
    return (<div className="space-y-4">
      ...
      {toolNames.length === 0 ? (<div className="rounded-xl border border-border p-6 text-center">
          ...
          {error && (<div className="rounded-xl border border-danger/30 bg-danger/5 p-4">
          ...
          {output !== null && (<div className="rounded-xl border border-success/30 bg-success/5 p-4">
              <div className="flex items-center gap-2 mb-1">
...
// ─── Skill Log Panel ──────────────────────────────────────────────
export function SkillLogPanel({ skill }: {
    skill: Skill;
}) {
    const [logs, setLogs] = useState<AuditLogEntry[]>([]);
    const [filter, setFilter] = useState<'all' | 'success' | 'error' | 'blocked'>('all');
    const toolNames = useMemo(() => {
        const names = new Set((skill.tools ?? []).map((t: {
            name: string;
        }) => t.name));
        if (skill.customCode) {
            try {
    ...
    return (<div className="space-y-3">
      <div className="flex items-center gap-2">
        ...
        <div className="ml-auto flex gap-1">
          {(['all', 'success', 'error', 'blocked'] as const).map((f) => (<UiButton unstyled key={f} type="button" onClick={() => setFilter(f)} className={`px-2 py-1 rounded-lg text-[11px] capitalize transition-colors ${filter === f ? 'bg-accent/15 text-accent font-medium' : 'text-text-muted hover:bg-surface-2'}`}>
              {filterLabel(f)}
            </UiButton>))}
        </div>
      </div>

      {displayed.length === 0 ? (<div className="text-center py-8 text-text-muted text-sm">
          {filter === 'all'
        ...
        </div>) : (<div className="space-y-1.5 max-h-100 overflow-y-auto">
          {displayed.map((entry) => (<div key={entry.id} className="px-3 py-2 rounded-lg bg-surface-1 border border-border/50 text-xs">
              <div className="flex items-center gap-2 mb-1">
                <span className={`w-1.5 h-1.5 rounded-full ${entry.status === 'success' ? 'bg-green-500' :
                    entry.status === 'error' ? 'bg-red-500' : 'bg-yellow-500'}`}/>
                <span className="font-mono font-medium text-text-primary">{entry.toolName}</span>
                {entry.duration != null && (<span className="text-text-muted">{entry.duration} {t('common.millisecondsUnit', 'ms')}</span>)}
                <span className="ml-auto text-text-muted">
                  {new Date(entry.timestamp).toLocaleString()}
                </span>
              </div>
              {entry.result && (<div className="text-text-muted truncate font-mono text-[10px] mt-0.5">
...
export function SecurityAuditPanel({ skill }: {
    skill: Skill;
}) {
    ...
    return (<div className="space-y-6">
      ...
      <div className="rounded-xl border border-border p-4 bg-surface-0/30">
        ...
        <div className="flex items-center gap-3 mb-3">
          <UiButton unstyled onClick={handleSign} disabled={signing} className="px-4 py-2 rounded-xl bg-accent/15 text-accent text-sm font-medium hover:bg-accent/25 transition-colors border border-accent/30 disabled:opacity-50 inline-flex items-center gap-1.5">
            {signing ? t('skills.signing', 'Signing...') : <><IconifyIcon name="ui-sign" size={14} color="currentColor"/> {t('skills.signSkill', 'Sign Skill')}</>}
          </UiButton>
          {signature && (<UiButton unstyled onClick={handleVerify} className="px-4 py-2 rounded-xl bg-surface-3 text-text-secondary text-sm font-medium hover:bg-surface-4 transition-colors inline-flex items-center gap-1.5">
              <IconifyIcon name="ui-search" size={14} color="currentColor"/> {t('skills.verify', 'Verify')}
            </UiButton>)}
        </div>
        {signature && (<div className="space-y-2 text-xs">
            <div className="flex items-center gap-2">
      ...
      <div className="rounded-xl border border-border p-4 bg-surface-0/30">
        ...
        {!skill.customCode?.trim() ? (<p className="text-xs text-text-muted">{t('skills.noCustomCode', 'No custom code to audit.')}</p>) : findings.length === 0 ? (<div className="flex items-center gap-2 text-xs">
            ...
            {findings.map((f, i) => (<div key={i} className={`flex items-start gap-2 text-xs rounded-lg border px-3 py-2 ${severityColors[f.severity]}`}>
...
export function CustomCodeEditor({ value, onChange, disabled }: {
    ...
    return (<div className="space-y-3">
      ...
      <div className="flex items-center gap-3">
        ...
        {testResult && (<span className={`text-xs ${testResult.error ? 'text-danger' : 'text-green-400'}`}>
            {testResult.error
      ...
      {testResult && !testResult.error && testResult.toolNames.length > 0 && (<div className="rounded-xl border border-border p-3 space-y-1">
          ...
          {testResult.toolNames.map((name) => (<div key={name} className="text-xs text-text-secondary font-mono flex items-center gap-2">
...
export function SkillDependenciesEditor({ dependencies, onChange, disabled = false }: {
    ...
    const addDep = () => {
        if (!newDepId)
            return;
        if (dependencies.some((d) => d.skillId === newDepId))
            return;
    ...
    return (<div>
      <div className="flex items-center justify-between mb-2">
        <label className="mb-1.5 block text-[12px] font-medium text-text-secondary">{t('skills.dependencies', 'Dependencies')}</label>
        {!disabled && (<UiButton unstyled type="button" onClick={() => setAdding(!adding)} className="text-xs text-accent hover:text-accent-hover">
            {adding ? t('common.cancel', 'Cancel') : t('common.add', '+ Add')}
          </UiButton>)}
      </div>
      {dependencies.length === 0 && !adding && (<p className="text-xs text-text-muted py-2">{t('skills.noDependencies', 'No dependencies declared.')}</p>)}
      {dependencies.length > 0 && (<div className="space-y-1 mb-2">
          {dependencies.map((dep) => {
                const depSkill = skills.find((s) => s.id === dep.skillId);
                return (<div key={dep.skillId} className="flex items-center justify-between p-2 rounded-lg bg-surface-2 border border-border-subtle text-xs">
                <div className="flex items-center gap-2">
                ...
                {!disabled && (<div className="flex items-center gap-1">
                    <UiButton unstyled type="button" onClick={() => toggleOptional(dep.skillId)} className="px-1.5 py-0.5 text-[10px] rounded bg-surface-3 text-text-muted hover:text-text-primary">
                      {dep.optional ? t('skills.required', 'Required') : t('skills.optional', 'Optional')}
                    </UiButton>
                      <UiButton unstyled type="button" onClick={() => removeDep(dep.skillId)} title={t('skills.removeDependency', 'Remove dependency')} className="px-1.5 py-0.5 text-[10px] text-red-400 hover:text-red-300"><IconifyIcon name="ui-close" size={12} color="currentColor"/></UiButton>
                  </div>)}
              </div>);
            })}
        </div>)}
...
export function SkillVersionsPanel({ skill, updateForm }: {
    skill: Skill;
    updateForm: (patch: Partial<Skill>) => void;
}) {
    ...
    return (<div className="space-y-4">
      ...
      <div className="rounded-xl border border-border p-4 bg-surface-0/30">
        <div className="flex items-center justify-between mb-3">
          ...
          {versions.length > 0 && (<UiButton unstyled type="button" onClick={async () => {
                ...
                if (ok)
                    removeSkillVersions(skill.id);
            }} className="text-xs text-red-400 hover:text-red-300">
              {t('skills.clearAll', 'Clear All')}
            </UiButton>)}
        </div>
        {versions.length === 0 ? (<p className="text-xs text-text-muted py-4 text-center">{t('skills.noVersions', 'No versions saved yet.')}</p>) : (<div className="space-y-2 max-h-80 overflow-y-auto">
            {versions.map((ver) => (<div key={ver.id} className="flex items-center justify-between p-3 rounded-lg bg-surface-2 border border-border-subtle">
                <div>
                  <div className="flex items-center gap-2">
                ...
                <UiButton unstyled type="button" onClick={() => restoreVersion(ver)} className={panelCompactSecondaryButtonClass}>
