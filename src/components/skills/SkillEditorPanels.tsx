...
export function MarkdownEditor({
  ...
  return (
    <div className="rounded-xl border border-border overflow-hidden bg-surface-2">
      <div className="flex items-center gap-1 px-2 py-1 border-b border-border bg-surface-3/40">
        <button
          type="button"
          onClick={() => setMode('edit')}
          className={`text-[11px] px-2 py-0.5 rounded transition-colors ${mode === 'edit' ? 'bg-accent/15 text-accent font-medium' : 'text-text-muted hover:text-text-secondary'}`}
        >
          {t('skills.edit', 'Edit')}
        </button>
        <button
          ...
          className={`text-[11px] px-2 py-0.5 rounded transition-colors ${mode === 'preview' ? 'bg-accent/15 text-accent font-medium' : 'text-text-muted hover:text-text-secondary'}`}
        >
          {t('skills.preview', 'Preview')}
        </button>
        <span className="ml-auto text-[10px] text-text-muted">Markdown</span>
      </div>
      {mode === 'edit' ? (
        <textarea
          value={value}
        ...
        <div className="px-3 py-2.5 min-h-20 text-sm prose prose-sm max-w-none text-text-primary overflow-auto">
          {value.trim() ? (
            ...
            <span className="text-text-muted italic">{t('skills.nothingToPreview', 'Nothing to preview.')}</span>
          )}
        </div>
      )}
    </div>
