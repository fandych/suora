...
function EditorSection({
  ...
  return (
    <section className={`rounded-[28px] border border-border-subtle/55 bg-linear-to-br from-surface-1/96 via-surface-1/88 to-surface-2/70 p-5 shadow-[0_18px_46px_rgba(15,23,42,0.08)] xl:p-6 ${className}`}>
      <div className="mb-5">
...
function ResourceTreePanel({
  ...
  // ── render ──────────────────────────────────────────────────────────
  return (
    <div className="grid gap-6 xl:grid-cols-[minmax(20rem,0.82fr)_minmax(0,1.18fr)] xl:items-stretch">
      <EditorSection
      ...
      >
        <div className="flex min-h-168 flex-1 flex-col">
          ...
          <div className="mb-3 flex flex-wrap items-center gap-2">
            ...
            <input ref={uploadInputRef} type="file" onChange={handleUpload} aria-label={t('skills.uploadResource', 'Upload')} className="hidden" />
            <button
              type="button"
                        ...
                        >
                          ...
                          {renamingPath === resource.path ? (
                            <input
                              ...
                              placeholder={t('common.rename', 'Rename')}
                              className="min-w-0 flex-1 rounded-xl border border-accent/20 bg-surface-0 px-2 py-1 font-mono text-[11px] text-text-primary outline-none"
                            />
                          ) : resource.type === 'file' ? (
                            <button
      ...
      >
        <div className="flex min-h-168 flex-1 flex-col">
          {!selectedPath ? (
            <div className="flex flex-1 items-center justify-center rounded-3xl border border-dashed border-border-subtle/55 bg-surface-0/45 p-8 text-center text-[12px] text-text-muted">
              {t('skills.noFileSelected', 'No file selected.')}
            </div>
          ) : editorError ? (
            <div className="flex flex-1 items-center rounded-3xl border border-danger/20 bg-danger/8 p-4 text-[12px] text-danger">
              {editorError}
            </div>
          ) : selectedResource && !isEditableSkillFile(selectedResource.path) ? (
            <div className="flex flex-1 flex-col rounded-3xl border border-border-subtle/55 bg-surface-2/60 p-5 text-[12px] text-text-secondary">
            ...
            <div className="flex min-h-0 flex-1 flex-col gap-3">
              <div className="flex flex-wrap items-center gap-2 rounded-2xl border border-border-subtle/55 bg-surface-0/55 px-3 py-2 text-[11px] text-text-muted">
                ...
                {selectedResource?.size !== undefined && (
                  <span className="rounded-full border border-border-subtle/45 bg-surface-2/75 px-2 py-1 text-[10px] font-mono text-text-secondary/80">
              ...
              <textarea
                ...
                className="min-h-112 flex-1 w-full resize-none rounded-3xl border border-border-subtle/55 bg-surface-2/75 p-4 font-mono text-[13px] leading-6 text-text-secondary outline-none focus:border-accent/50"
                spellCheck={false}
              />
    ...
    <form onSubmit={handleSubmit} className="module-canvas flex-1 overflow-y-auto">
      <div className="module-content mx-auto flex w-full max-w-432 flex-col gap-6 px-5 py-6 xl:px-8 xl:py-8">
        ...
        <div className="space-y-6">
          ...
          {activeTab === 'preview' && (
            <div className="grid gap-6 xl:grid-cols-[minmax(0,1.08fr)_minmax(24rem,0.92fr)] xl:items-stretch">
              <EditorSection
                eyebrow={t('skills.preview', 'Preview')}
                title={t('skills.renderedPreview', 'Rendered Preview')}
                description={t('skills.previewHelp', 'See how the skill instructions will read once rendered as Markdown.')}
                className="flex h-full flex-col"
              >
                        ...
                        <div className="mt-4 flex flex-wrap gap-2">
                          {previewMetadata.map((item) => (
                            <span
                              ...
                              className="rounded-full border border-border-subtle/45 bg-surface-2/75 px-3 py-1 text-[11px] text-text-secondary"
                            >
              ...
              <EditorSection
                eyebrow={t('skills.generatedSkillFile', 'Generated SKILL.md')}
                title={t('skills.generatedSource', 'Generated Source')}
                description={t('skills.generatedSourceHint', 'Raw source that will be saved to disk, including YAML frontmatter.')}
                className="flex h-full flex-col"
              >
                <div className="flex min-h-168 flex-1 flex-col">
