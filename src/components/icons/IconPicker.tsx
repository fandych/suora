...
export function IconPicker({ value, onSelect, onClose, presetFilter }: IconPickerProps) {
    ...
    const picker = (<div ref={backdropRef} className="fixed inset-0 z-1000 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm" onClick={handleBackdropClick} onKeyDown={handleKeyDown}>
      <div className="flex max-h-[min(580px,calc(100vh-2rem))] w-[min(600px,calc(100vw-2rem))] flex-col overflow-hidden rounded-xl border border-border bg-surface-1 shadow-2xl">
        ...
        <div className="flex items-center gap-1 px-4 pt-3">
          <UiButton unstyled type="button" onClick={() => setTab('presets')} className={`px-3 py-1.5 text-xs font-medium rounded-md transition-colors ${tab === 'presets'
            ? 'bg-accent/15 text-accent'
            : 'text-text-muted hover:text-text-secondary hover:bg-surface-2'}`}>
            {t('icons.presets', 'Presets')}
          </UiButton>
          <UiButton unstyled type="button" onClick={() => setTab('iconify')} className={`px-3 py-1.5 text-xs font-medium rounded-md transition-colors ${tab === 'iconify'
            ? 'bg-accent/15 text-accent'
            : 'text-text-muted hover:text-text-secondary hover:bg-surface-2'}`}>
        ...
        <div className="px-4 pt-3 pb-2">
          <UiInput ref={searchRef} type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder={t('icons.search', 'Search icons...')} className="w-full px-3 py-2 bg-surface-0 border border-border-subtle rounded-lg text-sm text-text-primary placeholder-text-muted focus:outline-none focus:ring-2 focus:ring-accent/20 focus:border-accent transition-colors"/>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4">
              {presetIcons.length === 0 ? (<p className="text-xs text-text-muted text-center py-8">
                  ...
                  {presetIcons.map((name) => (<UiButton unstyled key={name} type="button" title={name} onClick={() => handleIconClick(name)} className={`w-10 h-10 rounded-lg flex items-center justify-center transition-all ${selectedIcon === name
        ...
        <>
              ...
              <div className="w-42.5 border-r border-border-subtle overflow-y-auto py-2 px-2 shrink-0">
                {featuredCollections.length > 0 && (<>
                    ...
                    {featuredCollections.map((c) => (<UiButton unstyled key={c.prefix} type="button" onClick={() => setSelectedCollection(c.prefix)} className={`w-full text-left px-2 py-1.5 text-xs rounded-md mb-0.5 transition-colors truncate ${selectedCollection === c.prefix
                ...
                {otherCollections.length > 0 && (<>
                    ...
                    {otherCollections.map((c) => (<UiButton unstyled key={c.prefix} type="button" onClick={() => setSelectedCollection(c.prefix)} className={`w-full text-left px-2 py-1.5 text-xs rounded-md mb-0.5 transition-colors truncate ${selectedCollection === c.prefix
                        ? 'bg-accent/15 text-accent font-medium'
                        : 'text-text-secondary hover:bg-surface-2'}`}>
                        {c.name} <span className="text-text-muted">({c.total})</span>
                      </UiButton>))}
              ...
              <div className="flex-1 overflow-y-auto p-3">
                {collectionLoading ? (<div className="flex items-center justify-center py-12">
                    ...
                    return (<UiButton unstyled key={name} type="button" title={fullName} onClick={() => handleIconClick(fullName)} className={`w-10 h-10 rounded-lg flex items-center justify-center transition-all ${selectedIcon === fullName
                            ? 'bg-accent/20 ring-2 ring-accent scale-110'
                            : 'bg-surface-2 hover:bg-surface-3'}`}>
        ...
        <div className="border-t border-border-subtle bg-surface-0/50">
          ...
          {selectedIcon && (<div className="px-4 pt-3 pb-2">
              ...
              <div className="flex items-center gap-1.5 flex-wrap">
                {COLOR_PALETTE.map((c) => (<UiButton unstyled key={c || 'default'} type="button" title={c || t('icons.defaultColor', 'Default')} onClick={() => { setSelectedColor(c); setCustomColor(c); }} className={`w-6 h-6 rounded-full border-2 transition-all shrink-0 ${selectedColor === c
                    ? 'border-accent scale-110 shadow-md'
                    : 'border-transparent hover:border-border'} ${c ? COLOR_SWATCH_CLASSES[c] : 'bg-linear-to-br from-surface-3 to-surface-1'}`}>
                    {!c && selectedColor === '' && (<span className="block w-full h-full rounded-full border-2 border-accent"/>)}
                  </UiButton>))}
                {/* Custom color input */}
                <div className="flex items-center gap-1 ml-1">
                  <UiInput type="color" value={customColor || '#6366F1'} onChange={(e) => {
                setCustomColor(e.target.value);
                setSelectedColor(e.target.value);
            }} className="w-6 h-6 rounded-full border-0 cursor-pointer bg-transparent p-0" title={t('icons.customColor', 'Custom color')}/>
                </div>
              </div>
            </div>)}
          {/* Preview + confirm */}
