...
function buildContextPrompt({ mode, pipeline, t, enabledAgentSummary, }: {
    ...
    if (mode === 'edit' && pipeline) {
        ...
        lines.push(`${t('agents.pipelineAssistantTargetPipelineId', 'Target pipeline id')}: ${pipeline.id}`, `${t('agents.pipelineAssistantCurrentName', 'Current name')}: ${pipeline.name}`, `${t('agents.pipelineAssistantCurrentDescription', 'Current description')}: ${pipeline.description || noneLabel}`, `${t('agents.pipelineAssistantCurrentVariables', 'Current variables')}: ${summarizeVariables(pipeline, t)}`, `${t('agents.pipelineAssistantCurrentBudget', 'Current budget')}: ${summarizeBudget(pipeline, t)}`, `${t('agents.pipelineAssistantCurrentSteps', 'Current steps')}: ${pipeline.steps.length}`);
        pipeline.steps.forEach((step, index) => {
            const stepName = step.name?.trim() || `${t('agents.pipelineStepFallback', 'Step {number}').replace('{number}', String(index + 1))}`;
            lines.push(`Step ${index + 1}: name=${stepName}; agentId=${step.agentId}; task=${truncateValue(step.task, 180)}`);
        });
    }
    return lines.join('\n');
}
...
export function PipelineAssistantDrawer({ mode, pipeline, onClose, onPipelineMutated, }: {
    mode: PipelineAssistantMode;
    pipeline?: AgentPipeline | null;
    onClose: () => void;
    onPipelineMutated?: () => void;
}) {
    const { sessions, addSession, updateSession, selectedModel, models, agents, } = useAppStore();
    const { t } = useI18n();
    ...
    const contextPrompt = useMemo(() => buildContextPrompt({ mode, pipeline, t, enabledAgentSummary }), [enabledAgentSummary, mode, pipeline, t]);
    useEffect(() => {
    ...
    useEffect(() => {
        if (!onPipelineMutated)
            return;
        let hasMutation = false;
        for (const message of messages) {
            for (const toolCall of message.toolCalls ?? []) {
                if (!['pipeline_add', 'pipeline_update', 'pipeline_remove'].includes(toolCall.toolName))
                    continue;
    ...
    return (<div className="pointer-events-none absolute inset-y-0 right-0 z-40 flex w-full justify-end pl-16">
      <section role="dialog" aria-label={t('agents.pipelineAssistantLabel', 'Pipeline assistant')} className="pointer-events-auto flex h-full w-full max-w-136 flex-col border-l border-border-subtle/55 bg-surface-0/94 shadow-[-24px_0_60px_rgba(15,23,42,0.22)] backdrop-blur-xl">
