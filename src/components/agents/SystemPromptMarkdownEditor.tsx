...
export function SystemPromptMarkdownEditor({ value, onChange, placeholder, rows = 12 }: SystemPromptMarkdownEditorProps) {
    ...
    return (<div className="overflow-hidden rounded-3xl border border-border-subtle/60 bg-surface-2/72">
      <div className="flex flex-wrap items-center justify-between gap-2 border-b border-border-subtle/55 bg-surface-3/35 px-3 py-2">
        <div className="inline-flex rounded-2xl border border-border-subtle bg-surface-0/65 p-1 text-[11px]">
          <UiButton unstyled type="button" onClick={() => setMode('write')} className={`rounded-xl px-3 py-1.5 transition-colors ${mode === 'write' ? 'bg-accent/15 text-accent' : 'text-text-muted hover:text-text-secondary'}`}>
            {t('agents.markdownWrite', 'Write')}
          </UiButton>
          <UiButton unstyled type="button" onClick={() => setMode('preview')} className={`rounded-xl px-3 py-1.5 transition-colors ${mode === 'preview' ? 'bg-accent/15 text-accent' : 'text-text-muted hover:text-text-secondary'}`}>
            {t('skills.preview', 'Preview')}
          </UiButton>
        </div>

        <div className="flex flex-wrap items-center gap-1.5">
          {tools.map((tool) => (<UiButton unstyled key={tool.snippet} type="button" title={tool.label} aria-label={tool.label} onClick={() => insertSnippet(tool.snippet)} disabled={mode !== 'write'} className="inline-flex h-8 w-8 items-center justify-center rounded-xl text-text-muted transition-colors hover:bg-surface-2 hover:text-accent disabled:cursor-not-allowed disabled:opacity-35">
