...
function summarizeModels(models: ReturnType<typeof useAppStore.getState>['models'], t: Translate): string {
  ...
  if (enabledModels.length === 0) return t('common.none', 'None')
...
function buildContextPrompt({
  ...
  if (mode === 'edit' && agent) {
    ...
    lines.push(
      `${t('agents.agentAssistantTargetAgentId', 'Target agent id')}: ${agent.id}`,
      `${t('agents.agentAssistantCurrentName', 'Current name')}: ${agent.name}`,
      `${t('agents.agentAssistantCurrentModel', 'Current model')}: ${agent.modelId || noneLabel}`,
      `${t('agents.agentAssistantCurrentSkills', 'Current skills')}: ${summarizeList(agent.skills, t)}`,
      `${t('agents.agentAssistantCurrentAllowedTools', 'Current allowed tools')}: ${summarizeList(agent.allowedTools, t)}`,
      `${t('agents.agentAssistantCurrentDisallowedTools', 'Current disallowed tools')}: ${summarizeList(agent.disallowedTools, t)}`,
      `${t('agents.agentAssistantCurrentWhenToUse', 'Current when-to-use hint')}: ${agent.whenToUse || noneLabel}`,
      `${t('agents.agentAssistantCurrentResponseStyle', 'Current response style')}: ${agent.responseStyle || noneLabel}`,
      `${t('agents.agentAssistantCurrentPermissionMode', 'Current permission mode')}: ${agent.permissionMode || 'default'}`,
      `${t('agents.agentAssistantCurrentAutoLearn', 'Current auto-learn')}: ${agent.autoLearn ? t('common.enabled', 'Enabled') : t('common.off', 'Off')}`,
      `${t('agents.agentAssistantCurrentPrompt', 'Current system prompt')}: ${truncateValue(agent.systemPrompt, 260) || noneLabel}`,
    )
  }
...
export function AgentAssistantDrawer({
  ...
  const contextPrompt = useMemo(
    () => buildContextPrompt({ mode, agent, t, modelSummary, skillSummary }),
    [agent, mode, modelSummary, skillSummary, t],
  )
  ...
  useEffect(() => {
    if (!onAgentMutated) return

    let hasMutation = false
    for (const message of messages) {
      for (const toolCall of message.toolCalls ?? []) {
        if (!['agent_add', 'agent_update', 'agent_remove'].includes(toolCall.toolName)) continue
  ...
  return (
    <div className="pointer-events-none absolute inset-y-0 right-0 z-40 flex w-full justify-end pl-16">
      <section
        role="dialog"
        aria-label={t('agents.agentAssistantLabel', 'Agent assistant')}
      ...
      >
          ...
          <div className="flex items-start justify-between gap-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                aria-label={t('agents.agentAssistantClose', 'Close agent assistant')}
        ...
        <div className="border-b border-border-subtle/55 px-5 py-4">
          <div className="mb-3">
            ...
            <select
              id="agent-assistant-model"
              aria-label={t('agents.agentAssistantModelPicker', 'Assistant model')}
