...
const orchestrationInputClass = 'rounded-xl px-3 py-2 text-xs';
const orchestrationSelectClass = 'rounded-xl px-3 py-2 text-xs';
const orchestrationSecondaryButtonClass = 'relative inline-flex items-center justify-center gap-1.5 rounded-xl border border-zinc-950/10 bg-white px-3 py-2 text-xs font-semibold text-zinc-950 shadow-sm transition hover:bg-zinc-950/2.5 dark:border-white/15 dark:bg-white/5 dark:text-white dark:hover:bg-white/10 disabled:cursor-not-allowed disabled:opacity-50';
const orchestrationDangerButtonClass = 'relative inline-flex items-center justify-center gap-1.5 rounded-xl border border-red-700/90 bg-red-600 px-3 py-2 text-xs font-semibold text-white shadow-sm transition hover:bg-red-500 disabled:cursor-not-allowed disabled:opacity-50';
const orchestrationCardClass = 'rounded-3xl border border-border-subtle/55 bg-surface-0/60 p-4';
export function AgentOrchestrationPanel({ agents, onClose, title, allowedTabs = DEFAULT_TABS, initialTab, }: AgentOrchestrationPanelProps) {
    const { t } = useI18n();
    ...
    // Versions
    const { workspacePath, agentVersions, agentPerformance, clearAgentPerformance, agentPipeline: pipeline, setAgentPipeline, clearAgentPipeline, agentPipelineName, setAgentPipelineName, selectedAgentPipelineId, setSelectedAgentPipelineId, agentPipelines, setAgentPipelines, addNotification, } = useAppStore();
    ...
    useEffect(() => {
        if (!supportsPipeline || !workspacePath)
            return;
    ...
    const selectedSavedPipeline = selectedAgentPipelineId
        ...
        : null;
    const hydratePipelineDraft = (savedPipeline: {
        id: string;
        name: string;
        steps: typeof pipeline;
    }) => {
    ...
    const updateStep = (idx: number, updates: Partial<{
        agentId: string;
        task: string;
    }>) => {
    ...
    return (<div className="module-workspace flex-1 flex flex-col overflow-hidden">
      <div className="px-4 py-3 border-b border-border flex items-center justify-between shrink-0">
        <h2 className="text-sm font-semibold text-text-primary">{title ?? t('agents.agentHub', 'Agent Hub')}</h2>
        {onClose && <UiButton unstyled title={t('common.close', 'Close')} onClick={onClose} className="text-text-muted hover:text-text-primary text-xs"><IconifyIcon name="ui-close" size={14} color="currentColor"/></UiButton>}
      </div>

      {TABS.length > 1 && <div className="flex gap-1 px-4 py-2 border-b border-border-subtle shrink-0">
        {TABS.map((t) => (<UiButton unstyled key={t.id} onClick={() => setTab(t.id)} className={`inline-flex items-center gap-1.5 rounded-2xl border px-3 py-2 text-xs font-medium transition-all ${tab === t.id ? 'border-accent/20 bg-accent/10 text-accent' : 'border-transparent text-text-muted hover:border-border-subtle/55 hover:bg-surface-0/60 hover:text-text-secondary'}`}>
      ...
      <div className="module-canvas flex-1 overflow-y-auto p-4">
        ...
        {tab === 'orchestrate' && (<div className={`${orchestrationCardClass} space-y-3`}>
            ...
            <div className="grid gap-2 md:grid-cols-[minmax(0,1fr)_auto_auto_auto]">
              <UiSelect value={selectedAgentPipelineId ?? ''} aria-label={t('agents.savedPipelines', 'Saved pipelines')} onChange={(e) => {
              ...
              <UiButton unstyled onClick={resetPipelineEditor} className={orchestrationSecondaryButtonClass}>{t('common.new', 'New')}</UiButton>
            ...
            {pipelineResult.length > 0 && (<div className="space-y-2">
                <h4 className="text-xs font-semibold text-text-primary">{t('agents.results', 'Results')}</h4>
                {pipelineResult.map((r, i) => (<div key={i} className="p-2 rounded-lg bg-surface-2 border border-border-subtle text-xs text-text-secondary whitespace-pre-wrap max-h-40 overflow-y-auto">
            ...
            <div className="space-y-2">
              ...
              {!selectedSavedPipeline ? (<p className="text-xs text-text-muted">{t('agents.pipelineHistoryHint', 'Save the pipeline first to keep execution history and let timers reference it.')}</p>) : pipelineHistory.length === 0 ? (<p className="text-xs text-text-muted">{t('agents.noPipelineHistory', 'No pipeline executions recorded yet.')}</p>) : (pipelineHistory.slice(0, 10).map((execution) => (<div key={execution.id} className="p-3 rounded-xl border border-border bg-surface-0/30 space-y-1">
                    ...
                    {(execution.error || execution.finalOutput) && (<div className="text-xs text-text-secondary whitespace-pre-wrap">{execution.error || execution.finalOutput}</div>)}
                  </div>)))}
            </div>
            <div className="flex gap-2">
              <UiButton unstyled onClick={addStep} className={orchestrationSecondaryButtonClass}>{t('agents.addStep', '+ Add Step')}</UiButton>
              <UiButton unstyled onClick={() => clearAgentPipeline()} disabled={pipeline.length === 0 || running} className={orchestrationSecondaryButtonClass}>{t('common.clearAll', 'Clear All')}</UiButton>
              <UiButton color="blue" onClick={runPipeline} disabled={pipeline.length === 0 || running || pipeline.some((s) => !s.task.trim())} className="rounded-xl px-3 py-2 text-xs">
        ...
        {tab === 'communications' && (<div className={`${orchestrationCardClass} space-y-2`}>
            ...
            {msgLog.length === 0 ? (<p className="text-xs text-text-muted text-center py-8">{t('agents.noAgentMessages', 'No agent-to-agent messages yet.')}<br />{t('agents.agentMessagesAppear', 'Messages appear when agents delegate to each other.')}</p>) : ([...msgLog].reverse().map((msg) => {
                ...
                return (<div key={msg.id} className="p-3 rounded-xl border border-border bg-surface-0/30">
                    <div className="flex items-center gap-2 text-xs text-text-muted mb-1">
                      ...
                      <span className={`px-1.5 py-0.5 rounded text-[10px] ${msg.status === 'completed' ? 'bg-green-500/15 text-green-400' :
                        msg.status === 'failed' ? 'bg-red-500/15 text-red-400' :
                            'bg-yellow-500/15 text-yellow-400'}`}>{statusLabel}</span>
                    </div>
        ...
        {/* ── Agent Versions ── */}
        {tab === 'versions' && (<div className={`${orchestrationCardClass} space-y-2`}>
            ...
            {agentVersions.length === 0 ? (<p className="text-xs text-text-muted text-center py-8">{t('agents.noVersions', 'No versions recorded yet.')}</p>) : ([...agentVersions].reverse().slice(0, 50).map((v) => {
                const agent = agents.find((a) => a.id === v.agentId);
                return (<div key={v.id} className="p-3 rounded-xl border border-border bg-surface-0/30">
        ...
        {tab === 'performance' && (<div className={`${orchestrationCardClass} space-y-3`}>
            <div className="flex items-center justify-between">
              <p className="text-xs text-text-muted">{t('agents.analyticsHelp', 'Agent response times, token usage, and error rates.')}</p>
              <UiButton unstyled onClick={() => clearAgentPerformance()} className={orchestrationSecondaryButtonClass}>{t('common.clearAll', 'Clear All')}</UiButton>
            </div>
            {Object.keys(agentPerformance).length === 0 ? (<p className="text-xs text-text-muted text-center py-8">{t('agents.noPerformanceData', 'No performance data recorded yet.')}</p>) : (Object.entries(agentPerformance).map(([agentId, stats]) => {
                const agent = agents.find((a) => a.id === agentId);
                return (<div key={agentId} className="p-3 rounded-xl border border-border bg-surface-0/30">
                    ...
                    {stats.responseTimes.length > 1 && (<div className="mt-2 flex h-6 items-end gap-0.5">
