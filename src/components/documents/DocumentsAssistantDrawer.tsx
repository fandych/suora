...
function buildContextPrompt({ mode, document, group, folder, t, }: {
    ...
    if (mode === 'edit' && document) {
        lines.push(`${t('documents.assistantTargetDocumentId', 'Target document id')}: ${document.id}`, `${t('documents.assistantCurrentTitle', 'Current title')}: ${document.title}`, `${t('documents.assistantCurrentCharacters', 'Current characters')}: ${document.markdown.length}`, `${t('documents.assistantCurrentContent', 'Current content preview')}: ${truncateValue(document.markdown) || noneLabel}`);
    }
    return lines.join('\n');
}
...
export function DocumentsAssistantDrawer({ mode, document, group, folder, onClose, onDocumentMutated, }: {
    mode: DocumentsAssistantMode;
    document?: DocumentItem | null;
    group?: DocumentGroup | null;
    folder?: DocumentFolder | null;
    ...
    const contextPrompt = useMemo(() => buildContextPrompt({ mode, document, group, folder, t }), [document, folder, group, mode, t]);
    ...
    useEffect(() => {
        if (!onDocumentMutated)
            return;
        let hasMutation = false;
        for (const message of messages) {
            for (const toolCall of message.toolCalls ?? []) {
                if (!['create_document', 'update_document'].includes(toolCall.toolName))
                    continue;
    ...
    return (<div className="pointer-events-none absolute inset-y-0 right-0 z-40 flex w-full justify-end pl-16">
      <section role="dialog" aria-label={t('documents.assistantLabel', 'Document assistant')} className="pointer-events-auto flex h-full w-full max-w-136 flex-col border-l border-border-subtle/55 bg-surface-0/94 shadow-[-24px_0_60px_rgba(15,23,42,0.22)] backdrop-blur-xl">
        ...
        <div ref={messagesScrollRef} className="flex-1 overflow-y-auto px-5 py-4">
          {messages.length === 0 ? (<div className="space-y-4">
              ...
              {messages.map((message) => (<MessageBubble key={message.id} message={message} onRetry={message.isError || message.failedMidStream ? () => retryLastError() : undefined} onResume={message.failedMidStream ? () => resumeFromMessage(message.id) : undefined} onDelete={() => deleteMessage(message.id)} onRegenerate={message.role === 'assistant' && !message.isStreaming && !message.failedMidStream ? () => regenerateMessage(message.id) : undefined}/>))}
