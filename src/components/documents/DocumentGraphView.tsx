...
const NODE_STYLE: Record<DocumentGraphNode['type'], {
    color: string;
    radius: number;
}> = {
...
export function DocumentGraphView({ graph, insights, selectedDocumentId, onSelectDocument }: DocumentGraphViewProps) {
    ...
    const highlightedInsightNodeIds = useMemo(() => new Set(activeInsight?.documentIds.map((documentId) => `doc-graph:${documentId}`) ?? []), [activeInsight]);
    ...
    const visibleNodeIds = useMemo(() => {
        ...
        graph.nodes.forEach((node) => {
            if (node.type === 'document' || node.type === 'group')
                ids.add(node.id);
        });
        return ids;
    ...
    const decoratedNodes = useMemo<PositionedNode[]>(() => positionedNodes.map((node) => ({
        ...node,
    ...
    return (<div className="grid h-full min-h-0 grid-cols-[minmax(0,1fr)_300px] gap-4">
        ...
        <svg viewBox="0 0 860 530" className="relative z-10 mt-3 h-[calc(100%-4.25rem)] min-h-90 w-full">
          ...
          {visibleEdges.map((edge) => {
            ...
            return (<g key={edge.id}>
                <line x1={source.x} y1={source.y} x2={target.x} y2={target.y} stroke={edge.type === 'references' ? '#4D7CFF' : edge.type === 'tagged' ? '#35B98F' : edge.type === 'external-link' ? '#E45F68' : '#D9A441'} strokeOpacity={highlighted ? 0.62 : 0.22} strokeWidth={highlighted ? 2.2 : 1.2} strokeDasharray={edge.type === 'contains' ? '0' : edge.type === 'references' ? '8 5' : '3 6'}/>
              </g>);
        })}
          {decoratedNodes.map((node) => {
            ...
            const nodeMarkup = (<>
                <circle cx={node.x} cy={node.y} r={radius + 8} fill={style.color} opacity={node.active ? 0.2 : node.related ? 0.13 : 0.05} filter={node.active ? 'url(#graphGlow)' : undefined}/>
                <circle cx={node.x} cy={node.y} r={radius} fill={style.color} opacity={node.active ? 0.98 : node.related ? 0.88 : 0.68} stroke="rgba(255,255,255,.72)" strokeWidth={node.active ? 3 : 1.3}/>
            ...
            return documentId ? (<g key={node.id} role="button" tabIndex={0} onClick={() => {
                ...
                {nodeMarkup}
              </g>) : (<g key={node.id} className="outline-none">
                {nodeMarkup}
              </g>);
        })}
        </svg>
      ...
      <aside className="min-h-0 overflow-y-auto rounded-4xl border border-border-subtle/70 bg-surface-0/55 p-4">
        <section>
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            {EDGE_TYPES.map((edgeType) => (<UiButton unstyled key={edgeType} type="button" onClick={() => toggleEdge(edgeType)} className={`rounded-2xl border px-3 py-1.5 text-[10px] font-semibold transition-all ${enabledEdges.has(edgeType) ? 'border-accent/25 bg-accent/12 text-accent' : 'border-border-subtle/55 bg-surface-2/45 text-text-muted hover:text-text-primary'}`}>
        ...
        <section className="mt-4 rounded-3xl border border-border-subtle/60 bg-surface-1/50 p-4">
          ...
          <div className="mt-3 space-y-2">
            {insights?.insights.length ? insights.insights.slice(0, 4).map((insight) => (<article key={insight.id} className={`w-full rounded-2xl border px-3 py-2 text-left transition-all ${selectedInsightId === insight.id ? 'border-accent/30 bg-accent/10' : 'border-border-subtle/55 bg-surface-2/55 hover:border-accent/25 hover:bg-accent/8'}`}>
                <UiButton unstyled type="button" onClick={() => selectInsight(insight.id)} className="w-full text-left">
                ...
                <div className="mt-2 flex justify-end">
                  <UiButton unstyled type="button" onClick={() => openInsightDocument(insight.documentIds[0])} className="rounded-xl border border-accent/18 bg-accent/8 px-2.5 py-1 text-[10px] font-semibold text-accent hover:bg-accent/14">
                    {t('documents.openInsightDocument', 'Open')}
                    <span className="sr-only"> {insight.title}</span>
                  </UiButton>
                </div>
              </article>)) : (<p className="rounded-2xl border border-dashed border-border-subtle/55 px-3 py-4 text-center text-[11px] text-text-muted">{t('documents.noGraphInsights', 'No graph insights yet.')}</p>)}
          </div>
        </section>
        ...
        <section className="mt-4 rounded-3xl border border-border-subtle/60 bg-surface-1/50 p-4">
          ...
          {selectedNode ? (<div className="mt-3">
            ...
            </div>) : (<p className="mt-3 text-[11px] text-text-muted">{t('documents.graphNoSelection', 'Select a graph node to inspect it.')}</p>)}
        </section>

        <section className="mt-4 rounded-3xl border border-border-subtle/60 bg-surface-1/50 p-4">
          <h3 className="text-[11px] font-semibold uppercase tracking-[0.16em] text-text-muted">{t('documents.backlinks', 'Backlinks')}</h3>
          <div className="mt-3 space-y-2">
            {backlinks.length === 0 ? (<p className="rounded-2xl border border-dashed border-border-subtle/55 px-3 py-4 text-center text-[11px] text-text-muted">{t('documents.noBacklinks', 'No backlinks yet.')}</p>) : backlinks.map((documentId) => {
            const node = graph.nodes.find((item) => item.documentId === documentId);
            if (!node)
                return null;
            return (<UiButton unstyled key={documentId} type="button" onClick={() => onSelectDocument(documentId)} className="w-full rounded-2xl border border-border-subtle/55 bg-surface-2/55 px-3 py-2 text-left hover:border-accent/25 hover:bg-accent/8">
        ...
        <section className="mt-4 rounded-3xl border border-border-subtle/60 bg-surface-1/50 p-4">
          <div className="flex items-center justify-between gap-2">
            <UiButton unstyled type="button" onClick={() => setShowExport((value) => !value)} className="flex min-w-0 flex-1 items-center justify-between text-left text-[11px] font-semibold uppercase tracking-[0.16em] text-text-muted">
            ...
            <UiButton unstyled type="button" onClick={() => void copyGraphifyJson()} className="rounded-xl border border-border-subtle/55 bg-surface-2/55 px-2.5 py-1 text-[10px] font-semibold text-text-secondary hover:border-accent/25 hover:bg-accent/8 hover:text-accent">
          ...
          {showExport && (<pre className="mt-3 max-h-56 overflow-auto rounded-2xl border border-border-subtle/60 bg-surface-2/55 p-3 text-[10px] leading-relaxed text-text-secondary">
              {graphifyPreview}
            </pre>)}
        </section>
      </aside>
