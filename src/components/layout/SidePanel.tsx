...
export function SidePanel({ title, children, action, width, contentRef, onContentScroll }: SidePanelProps) {
  ...
  return (
    ...
    >
      <div className="flex min-h-13 items-center justify-between gap-3 border-b border-border/60 bg-surface-1/55 px-3.5 py-2.5 shrink-0 backdrop-blur-sm">
        <h2 className="truncate font-display text-[15px] font-semibold tracking-[0.01em] text-text-primary">{title}</h2>
