...
const navItems = [
    ...
    { path: '/pipeline', i18nKey: 'nav.pipeline', fallbackLabel: 'Pipeline', icon: (<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><circle cx="5" cy="6" r="2"/><circle cx="12" cy="12" r="2"/><circle cx="19" cy="18" r="2"/><path d="M6.7 7.2 10.3 10.8"/><path d="M13.7 13.2 17.3 16.8"/><path d="M7 6h5"/></svg>) },
    { path: '/timer', i18nKey: 'nav.timer', fallbackLabel: 'Timer', icon: (<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>) },
    { path: '/channels', i18nKey: 'nav.channels', fallbackLabel: 'Channels', icon: (<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><rect x="5" y="2" width="14" height="20" rx="2" ry="2"/><line x1="12" y1="18" x2="12.01" y2="18"/></svg>) },
    { path: '/agents', i18nKey: 'nav.agents', fallbackLabel: 'Agents', icon: (<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M12 2a4 4 0 0 1 4 4v2a4 4 0 0 1-8 0V6a4 4 0 0 1 4-4z"/><path d="M16 14H8a4 4 0 0 0-4 4v2h16v-2a4 4 0 0 0-4-4z"/></svg>) },
    { path: '/skills', i18nKey: 'nav.skills', fallbackLabel: 'Skills', icon: (<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"/></svg>) },
    { path: '/models', i18nKey: 'nav.models', fallbackLabel: 'Models', icon: (<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z"/><polyline points="3.27 6.96 12 12.01 20.73 6.96"/><line x1="12" y1="22.08" x2="12" y2="12"/></svg>) },
...
function NotificationBell() {
    ...
    return (<div className="relative" ref={panelRef}>
      <UiButton unstyled type="button" onClick={() => setOpen(!open)} title={notificationsLabel} aria-label={notificationsLabel} className={`relative flex h-10 w-10 items-center justify-center rounded-md transition-colors ${open ? 'nav-item-active text-accent' : navButtonInactive}`}>
        ...
        {unreadCount > 0 && (<span className="absolute right-0 top-0 flex h-4.5 min-w-4.5 items-center justify-center rounded-full bg-danger px-1 text-[9px] font-semibold text-white">
      ...
      {open && (<div role="dialog" aria-label={t('nav.notifications', 'Notifications')} className="glass-strong absolute bottom-0 left-full z-50 ml-2 flex max-h-120 w-88 flex-col rounded-lg border shadow-lg">
          ...
          <div className="flex items-center justify-between border-b border-border-subtle px-3 py-2.5">
            ...
            <div className="flex items-center gap-1">
              {unreadCount > 0 && (<UiButton unstyled type="button" onClick={markAllNotificationsRead} className="rounded px-2 py-1 text-[11px] text-text-muted hover:bg-surface-2 hover:text-accent">
                  {t('nav.markAllRead', 'Mark all read')}
                </UiButton>)}
              {notifications.length > 0 && (<UiButton unstyled type="button" onClick={clearNotifications} className="rounded px-2 py-1 text-[11px] text-text-muted hover:bg-surface-2 hover:text-danger">
          ...
          <div className="flex-1 overflow-y-auto">
            {notifications.length === 0 ? (<div className="flex items-center justify-center py-10 text-[13px] text-text-muted">{t('nav.noNotifications', 'No notifications')}</div>) : (notifications.slice(0, 50).map((n) => (<UiButton unstyled type="button" key={n.id} onClick={() => handleAction(n)} className={`flex w-full items-start gap-3 border-b border-border-subtle px-3 py-2.5 text-left transition-colors hover:bg-surface-2 ${!n.read ? 'bg-accent/10' : ''}`}>
...
function OfflineIndicator() {
    ...
    return (<div className="mb-1 flex w-10 flex-col items-center justify-center gap-1" role="status" aria-label={`${offlineLabel} — ${offlineDescription}`}>
