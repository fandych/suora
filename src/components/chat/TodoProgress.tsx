...
export function TodoProgress() {
    ...
    const loadTodos = useCallback(async () => {
        ...
        try {
            const result = await electronInvoke('db:loadPersistedStore', key) as {
                data?: unknown;
                error?: string;
            };
    ...
    return (<div className="mx-auto mb-4 w-full max-w-352 animate-fade-in">
      <div className="overflow-hidden rounded-[28px] border border-border-subtle/55 bg-surface-0/52 shadow-[0_18px_46px_rgba(15,23,42,0.08)] backdrop-blur-xl">
        <UiButton unstyled onClick={() => setExpanded(!expanded)} className="flex w-full items-center gap-3 px-4 py-3 transition-colors hover:bg-surface-2/42">
          ...
          <div className="flex-1 min-w-0">
            <div className="mb-1 flex flex-wrap items-center gap-2">
              ...
              {inProgress > 0 && (<span className="rounded-full border border-accent/18 bg-accent/10 px-2 py-0.5 text-[10px] font-medium text-accent">
                  {t('chat.todoInProgressCount', '{count} in progress').replace('{count}', String(inProgress))}
                </span>)}
              {pending > 0 && (<span className="rounded-full border border-border-subtle/55 bg-surface-2/65 px-2 py-0.5 text-[10px] font-medium text-text-muted">
                  {t('chat.todoPendingCount', '{count} pending').replace('{count}', String(pending))}
                </span>)}
            </div>
            <progress value={percent} max={100} className={`todo-progress-meter ${percent === 100 ? 'is-complete' : ''}`} aria-label={t('chat.todoProgressAria', 'Todo progress {percent}%').replace('{percent}', String(percent))}/>
          </div>
          <span className="text-[13px] font-bold tabular-nums text-accent">{percent}%</span>
          <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className={`text-text-muted/60 shrink-0 transition-transform duration-200 ${expanded ? 'rotate-180' : ''}`}>
            <polyline points="6 9 12 15 18 9"/>
          </svg>
        </UiButton>

        {expanded && (<div className="max-h-72 space-y-2 overflow-y-auto border-t border-border-subtle/45 bg-surface-2/28 px-4 py-3 animate-fade-in">
            {todos.map((todo) => (<div key={todo.id} className={`flex items-center gap-3 rounded-[18px] border px-3 py-2.5 text-[12px] transition-colors ${todo.status === 'done' ? 'opacity-50' : ''}`}>
                ...
                <span className={`flex-1 truncate ${todo.status === 'done' ? 'line-through text-text-muted' : 'text-text-primary'}`}>
                  {todo.title}
                </span>
                <span className="shrink-0 text-[10px]">{priorityIcon(todo.priority)}</span>
                <span className={`text-[10px] px-1.5 py-0.5 rounded-md font-medium ${todo.status === 'done' ? 'bg-success/10 text-success' :
