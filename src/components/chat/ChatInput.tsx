...
function fileToAttachment(file: File, onReject?: (file: File, reason: AttachmentRejectReason) => void, summaryCopy?: {
    audio: string;
    ...
    return new Promise((resolve) => {
        if (ACCEPTED_IMAGE_TYPES.includes(file.type)) {
            if (file.size > MAX_IMAGE_SIZE) {
                onReject?.(file, { kind: 'oversize', limitBytes: MAX_IMAGE_SIZE });
                resolve(null);
                return;
            }
        ...
        if (ACCEPTED_AUDIO_TYPES.includes(file.type) || file.type.startsWith('audio/')) {
            if (file.size > MAX_AUDIO_SIZE) {
                onReject?.(file, { kind: 'oversize', limitBytes: MAX_AUDIO_SIZE });
                resolve(null);
                return;
            }
            const reader = new FileReader();
        ...
        if (isTextFile(file)) {
            if (file.size > MAX_FILE_SIZE) {
                onReject?.(file, { kind: 'oversize', limitBytes: MAX_FILE_SIZE });
                resolve(null);
                return;
            }
            const reader = new FileReader();
        ...
        if (ACCEPTED_DOCUMENT_TYPES.includes(file.type)) {
            if (file.size > MAX_DOCUMENT_SIZE) {
                onReject?.(file, { kind: 'oversize', limitBytes: MAX_DOCUMENT_SIZE });
                resolve(null);
                return;
            }
            const attachment: MessageAttachment = {
...
function ComposerActionButton({ label, icon, onClick, disabled, active = false, }: {
    label: string;
    icon: ReactNode;
    onClick: () => void;
    disabled?: boolean;
    ...
    return (<UiButton unstyled type="button" onClick={onClick} disabled={disabled} title={label} aria-label={label} className={`inline-flex h-8 w-8 items-center justify-center rounded-md border text-[12px] font-medium transition-all ${active
            ? 'border-danger/18 bg-danger/10 text-danger'
            : 'border-border-subtle/55 bg-surface-0/65 text-text-secondary hover:border-accent/18 hover:bg-accent/10 hover:text-accent'} disabled:opacity-30`}>
      <span className="text-[13px]">{icon}</span>
      <span className="sr-only">{label}</span>
    </UiButton>);
}
function AttachmentTile({ attachment, removeLabel, onRemove, }: {
    ...
    const preview = attachment.type === 'image'
        ...
        : null;
    return (<div className="group relative min-w-0 rounded-[22px] border border-border-subtle/55 bg-surface-0/68 p-2.5 shadow-sm">
      <UiButton unstyled type="button" onClick={onRemove} className="absolute right-2 top-2 z-10 flex h-6 w-6 items-center justify-center rounded-full bg-black/45 text-white opacity-70 transition-opacity hover:opacity-100" aria-label={`${removeLabel} ${attachment.name}`} title={removeLabel}>
        <IconifyIcon name="ui-close" size={12} color="currentColor"/>
      </UiButton>

      <div className="flex items-start gap-3 pr-8">
        {attachment.type === 'image' ? (<img src={preview ?? undefined} alt={attachment.name} className="h-18 w-18 shrink-0 rounded-[18px] border border-border-subtle/55 object-cover"/>) : (<div className="flex h-18 w-18 shrink-0 items-center justify-center rounded-[18px] border border-border-subtle/55 bg-surface-2/75 text-accent">
        ...
        <div className="min-w-0 flex-1 pt-1">
          ...
          {attachment.type === 'audio' && attachment.duration && (<div className="mt-2 w-fit rounded-full bg-surface-2/80 px-2 py-1 text-[10px] text-text-muted/80">{attachment.duration}s</div>)}
          {attachment.type === 'file' && (<div className="mt-2 w-fit rounded-full bg-surface-2/80 px-2 py-1 text-[10px] uppercase text-text-muted/80">
...
export function ChatInput({ onSend, disabled, isStreaming, onStop, noModel, footer }: {
    ...
    const addFiles = useCallback(async (files: FileList | File[]) => {
        ...
        const results = await Promise.all(list.map((f) => fileToAttachment(f, (file, reason) => rejections.push({ file, reason }), summaryCopy)));
        ...
        for (const { file, reason } of rejections) {
            if (reason.kind === 'oversize') {
                toast.warning(t('chat.attachTooLarge', 'File too large'), `${file.name} — ${formatFileSize(file.size)} exceeds ${formatFileSize(reason.limitBytes)} limit`);
            }
            else if (reason.kind === 'unsupported') {
                toast.warning(t('chat.attachUnsupported', 'Unsupported file type'), `${file.name} (${file.type || 'unknown'}) cannot be attached`);
            }
            else if (reason.kind === 'read-failed') {
                toast.error(t('chat.attachReadFailed', 'Could not read file'), file.name);
            }
        }
    }, [t]);
    ...
    const handleDrop = useCallback((e: DragEvent<HTMLDivElement>) => { e.preventDefault(); setIsDragging(false); if (e.dataTransfer?.files?.length)
        addFiles(e.dataTransfer.files); }, [addFiles]);
    const handleFileSelect = useCallback((e: ChangeEvent<HTMLInputElement>) => { if (e.target.files?.length)
        addFiles(e.target.files); e.target.value = ''; }, [addFiles]);
            ...
            mediaRecorder.onstop = () => {
                ...
                if (blob.size > 0) {
                    if (blob.size > MAX_AUDIO_SIZE) {
                        toast.warning(t('chat.attachTooLarge', 'File too large'), `${t('chat.recordedAudio', 'Recorded audio')} — ${formatFileSize(blob.size)} exceeds ${formatFileSize(MAX_AUDIO_SIZE)} limit`);
    ...
    return (<div className="mx-auto w-full max-w-384">
      <div className={`relative overflow-visible rounded-md ${isDragging ? 'border border-accent/45 bg-accent/5' : 'bg-surface-1/32'}`} onDragOver={handleDragOver} onDragLeave={handleDragLeave} onDrop={handleDrop}>
        {isDragging && (<div className="absolute inset-0 z-20 flex items-center justify-center bg-accent/10 backdrop-blur-sm">
        ...
        <div className="relative z-10 px-3 py-2 sm:px-3.5 sm:py-2.5">
          <UiInput ref={fileInputRef} type="file" accept="image/png,image/jpeg,image/gif,image/webp,audio/webm,audio/ogg,audio/mp3,audio/mpeg,audio/wav,audio/mp4,.txt,.md,.json,.csv,.xml,.yaml,.yml,.toml,.js,.ts,.jsx,.tsx,.py,.java,.go,.rs,.rb,.html,.css,.scss,.sql,.sh,.bat,.ps1,.log,.c,.cpp,.h,.swift,.kt,.dart,.lua,.r" multiple className="hidden" onChange={handleFileSelect} aria-label={t('chat.attachFile', 'Attach file (image, audio, code, text)')}/>

          {attachments.length > 0 && (<div className="mb-3 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
              {attachments.map((attachment) => (<AttachmentTile key={attachment.id} attachment={attachment} removeLabel={t('common.remove', 'Remove')} onRemove={() => removeAttachment(attachment.id)}/>))}
            </div>)}

          {isRecording && (<div className="mb-3 flex items-center gap-3 rounded-2xl border border-danger/18 bg-danger/10 px-4 py-3 text-danger shadow-sm">
              ...
              <UiButton unstyled type="button" onClick={stopRecording} className="ml-auto rounded-2xl bg-danger/15 px-3 py-2 text-[12px] font-semibold text-danger transition-colors hover:bg-danger/22">{t('chat.stopRecording', 'Stop recording')}</UiButton>
            </div>)}

          <div className="space-y-2">
            {(voiceState === 'listening' || isRecording || attachments.length > 0) && (<div className="flex flex-wrap items-center gap-2 text-[10px] font-medium text-text-muted/62">
            ...
            <div className="rounded-md border border-border-subtle/55 bg-surface-0/70 px-3 py-1.5 transition-colors focus-within:border-accent/45">
              <TextArea ghost ref={textareaRef} value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={handleKeyDown} onPaste={handlePaste} aria-label={t('chat.messageInputAria', 'Message input')} placeholder={isRecording
            ...
            <div className="flex flex-col gap-2 lg:flex-row lg:items-center lg:justify-between">
              <div className="flex flex-wrap gap-1.5">
                <ComposerActionButton label={t('chat.attachFile', 'Attach file (image, audio, code, text)')} icon={<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21.44 11.05l-9.19 9.19a6 6 0 0 1-8.49-8.49l9.19-9.19a4 4 0 0 1 5.66 5.66l-9.2 9.19a2 2 0 0 1-2.83-2.83l8.49-8.48"/></svg>} onClick={() => fileInputRef.current?.click()} disabled={disabled || isRecording}/>
            ...
            : <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><circle cx="12" cy="12" r="3" fill="currentColor"/></svg>} onClick={isRecording ? stopRecording : startRecording} disabled={disabled || voiceState === 'listening'} active={isRecording}/>
            ...
            }} disabled={disabled || isRecording} active={voiceState === 'listening'}/>)}
              ...
              <div className="flex flex-wrap items-center justify-end gap-2">
                ...
                {isStreaming ? (<UiButton unstyled type="button" onClick={onStop} title={t('chat.stopGenerating', 'Stop generating')} aria-label={t('chat.stopGenerating', 'Stop generating')} className="inline-flex h-9 items-center gap-2 rounded-md bg-danger/90 px-3 text-[12px] font-semibold text-white transition-colors hover:bg-danger">
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><rect x="4" y="4" width="16" height="16" rx="2"/></svg>
                    <span>{t('chat.stopGenerating', 'Stop generating')}</span>
                  </UiButton>) : (<UiButton unstyled type="button" onClick={handleSubmit} disabled={disabled || (!input.trim() && !attachments.length)} title={t('chat.send', 'Send')} aria-label={t('chat.send', 'Send')} className="inline-flex h-9 items-center gap-2 rounded-md bg-accent px-4 text-[12px] font-semibold text-white transition-all hover:bg-accent-hover disabled:opacity-25">
