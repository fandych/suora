...
export function ModelComparisonPanel({ onClose }: {
    ...
    const runComparison = async () => {
        if (selectedModels.length < 2 || !prompt.trim() || running)
            return;
        setRunning(true);
        setResults({});
        ...
        <div className="space-y-4">
          ...
          <section className="rounded-4xl border border-border-subtle/55 bg-linear-to-br from-surface-1/96 via-surface-1/88 to-surface-2/70 p-5 shadow-[0_18px_46px_rgba(15,23,42,0.08)]">
            ...
            {enabledModels.length === 0 ? (<div className="mt-5 rounded-3xl border border-dashed border-border-subtle/60 bg-surface-0/35 px-4 py-10 text-center">
              ...
              </div>) : (<div className="mt-5 grid gap-3 md:grid-cols-2">
                {enabledModels.map((model) => {
                const selected = selectedModels.includes(model.id);
                return (<UiButton unstyled key={model.id} type="button" onClick={() => toggleModel(model.id)} className={`rounded-3xl border p-4 text-left transition-all ${selected ? 'border-accent/20 bg-accent/10 shadow-[0_12px_28px_rgba(var(--t-accent-rgb),0.08)]' : 'border-border-subtle/55 bg-surface-0/45 hover:bg-surface-2/60'}`}>
          ...
          <section className="rounded-4xl border border-border-subtle/55 bg-linear-to-br from-surface-1/96 via-surface-1/88 to-surface-2/70 p-5 shadow-[0_18px_46px_rgba(15,23,42,0.08)]">
            ...
            {selectedModels.length === 0 ? (<div className="mt-5 rounded-3xl border border-dashed border-border-subtle/60 bg-surface-0/35 px-4 py-10 text-center">
              ...
              </div>) : (<div className="mt-5 grid gap-3 xl:grid-cols-2">
                {selectedModels.map((modelId) => {
                const model = models.find((entry) => entry.id === modelId);
                const result = results[modelId];
                return (<article key={modelId} className="rounded-3xl border border-border-subtle/55 bg-surface-0/45 p-4 shadow-[0_10px_24px_rgba(15,23,42,0.05)]">
                      <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0 flex-1">
                      ...
                      {!result ? (<div className="mt-4 rounded-2xl border border-dashed border-border-subtle/60 bg-surface-2/45 px-4 py-10 text-center text-[12px] text-text-muted">
                          {running ? t('models.generating', 'Generating…') : t('models.awaitingRun', 'Awaiting run')}
                        </div>) : result.error ? (<div className="mt-4 rounded-2xl border border-red-500/18 bg-red-500/8 px-4 py-3 text-[12px] leading-6 text-red-400">{result.error}</div>) : (<div className="mt-4 max-h-80 overflow-y-auto rounded-2xl border border-border-subtle/45 bg-surface-2/65 px-4 py-3 text-[12px] leading-6 text-text-secondary whitespace-pre-wrap wrap-break-word">
                          {result.text}
                        </div>)}

                      {result && !result.error && (<div className="mt-4 flex flex-wrap gap-2 text-[10px] text-text-muted/75">
