...
export function evaluateExpression(expression: string, context: ExecutionContext): boolean {
  ...
  const leftVal = leftRaw.startsWith("{{") || leftRaw.startsWith("${") || leftRaw.startsWith("$") ? readPath(context, leftRaw) : readPath(context, leftRaw) ?? interpolate(leftRaw, context)
  const rightVal = rightRaw.startsWith("{{") || rightRaw.startsWith("${") || rightRaw.startsWith("$") ? readPath(context, rightRaw) : interpolate(rightRaw, context).replace(/^['"]|['"]$/g, "")
  const left = leftVal
