export const runtimeMigrations = [
  ...
  },
  {
    id: 9,
    statements: [
      `ALTER TABLE channels ADD COLUMN enabled INTEGER NOT NULL DEFAULT 0`,
      `ALTER TABLE channels ADD COLUMN status TEXT NOT NULL DEFAULT 'inactive'`,
      `ALTER TABLE channels ADD COLUMN connection_mode TEXT NOT NULL DEFAULT 'webhook'`,
      `ALTER TABLE channels ADD COLUMN webhook_path TEXT NOT NULL DEFAULT ''`,
      `ALTER TABLE channels ADD COLUMN webhook_secret TEXT NOT NULL DEFAULT ''`,
      `ALTER TABLE channels ADD COLUMN auto_reply INTEGER NOT NULL DEFAULT 1`,
      `ALTER TABLE channels ADD COLUMN reply_agent_id TEXT NOT NULL DEFAULT ''`,
      `ALTER TABLE channels ADD COLUMN created_at INTEGER NOT NULL DEFAULT 0`,
      `ALTER TABLE channels ADD COLUMN last_message_at INTEGER`,
      ...
      `ALTER TABLE channels ADD COLUMN runtime_json TEXT NOT NULL DEFAULT '{}'`,
      `UPDATE channels SET created_at = COALESCE(NULLIF(created_at, 0), updated_at)`,
      `UPDATE channels SET webhook_path = CASE WHEN webhook_path = '' THEN '/channels/' || id ELSE webhook_path END`,
      `UPDATE channels SET runtime_json = '{"messages":[],"users":[],"health":{"isHealthy":null,"errorCount":0},"debugLog":[]}' WHERE runtime_json = '{}'`,
    ],
    ignoreErrorsMatching: [
      /duplicate column name: enabled/i,
      /duplicate column name: status/i,
      /duplicate column name: connection_mode/i,
      /duplicate column name: webhook_path/i,
      /duplicate column name: webhook_secret/i,
      /duplicate column name: auto_reply/i,
      /duplicate column name: reply_agent_id/i,
      /duplicate column name: created_at/i,
      /duplicate column name: last_message_at/i,
      /duplicate column name: message_count/i,
      /duplicate column name: config_json/i,
