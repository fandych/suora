...
async function safeUrl(value) {
  ...
  if (!records.length || records.some((record) => isPrivate(record.address))) throw new Error('Private and local network URLs are not allowed.')
  return url
}
...
async function run(request) {
  try {
    ...
    const sandbox = {
      ...
      TextDecoder,
      TextEncoder,
      console: { log: (...args) => appendLog(logs, args), warn: (...args) => appendLog(logs, args), error: (...args) => appendLog(logs, args) },
...
function parseInput(value) { try { return value?.trim() ? JSON.parse(value) : {} } catch { return {} } }
function appendLog(logs, args) { if (logs.length >= MAX_LOG_ENTRIES) return; const next = args.map(String).join(' '); if (Buffer.byteLength(logs.join('\n') + next) <= MAX_LOG_BYTES) logs.push(next) }
