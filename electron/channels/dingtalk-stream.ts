...
export class DingTalkStreamClient {
  ...
  private connectWebSocket(connection: StreamConnectionInfo): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      const ignoreSsl = getPreferenceSettingsSnapshot().ignoreSslErrors || appState.currentProxySettings.ignoreSslErrors === true
      const agent = getProxyAgent(new URL(url), ignoreSsl) ?? new https.Agent({ rejectUnauthorized: !ignoreSsl, keepAlive: true })
