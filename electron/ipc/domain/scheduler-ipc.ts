...
export function registerSchedulerIpc() {
  ...
  ipcMain.handle("schedulers:get", async (_event, id: unknown) => { const schedulerId = parseIpcInput(entityIdSchema, id); const database = await getDatabase(); return database.prepare(`${selectScheduler} WHERE id = ?`).get(schedulerId) ?? null })
  ...
  ipcMain.handle("schedulers:save", async (_event, value: unknown) => {
    const payload = parseIpcInput(schedulerSaveSchema, value)
  ...
  ipcMain.handle("schedulers:setEnabled", async (_event, value: unknown) => { const payload = parseIpcInput(z.object({ id: entityIdSchema, enabled: z.boolean() }), value); const database = await getDatabase(); database.prepare("UPDATE schedulers SET enabled = ?, updated_at = ? WHERE id = ?").run(payload.enabled ? 1 : 0, Date.now(), payload.id); return database.prepare(`${selectScheduler} WHERE id = ?`).get(payload.id) ?? null })
  ipcMain.handle("schedulers:listRuns", async (_event, id: unknown) => { const schedulerId = parseIpcInput(entityIdSchema, id); const database = await getDatabase(); return database.prepare(`SELECT id, status, input_json as input, output_json as output, started_at as startedAt, finished_at as finishedAt FROM scheduler_runs WHERE scheduler_id = ? ORDER BY started_at DESC`).all(schedulerId) })
  ipcMain.handle("schedulers:delete", async (_event, id: unknown) => { const schedulerId = parseIpcInput(entityIdSchema, id); const database = await getDatabase(); database.prepare("DELETE FROM scheduler_runs WHERE scheduler_id = ?").run(schedulerId); const result = database.prepare("DELETE FROM schedulers WHERE id = ?").run(schedulerId); return { ok: Number(result.changes) > 0 } })
}
