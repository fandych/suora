...
export const browserNavigateSchema = z.object({ sessionId, url: z.string().trim().url().max(8192).optional(), visible: z.boolean().optional() })
export const browserStateSchema = z.string().trim().min(1).max(128).optional()
export const browserPageSchema = z.object({ sessionId, includeText: z.boolean().optional(), includeLinks: z.boolean().optional() })
