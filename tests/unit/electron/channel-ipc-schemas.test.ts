...
describe("channel IPC schemas", () => {
  it("accepts bounded message payloads", () => {
    expect(
      parseChannelIpcInput(channelMessageSchema, { channelId: "channel-1", chatId: "chat-1", content: " hello " }),
    ).toEqual({
      channelId: "channel-1",
      chatId: "chat-1",
      content: "hello",
  ...
  it("rejects empty identifiers and oversized messages", () => {
    expect(() =>
      parseChannelIpcInput(channelMessageSchema, { channelId: "", chatId: "chat-1", content: "hello" }),
    ).toThrow(/Invalid channel IPC payload/)
    expect(() =>
      parseChannelIpcInput(channelMessageSchema, {
        channelId: "channel-1",
        chatId: "chat-1",
  ...
  it("bounds WeChat login wait input", () => {
    ...
    expect(() => parseChannelIpcInput(wechatLoginWaitSchema, { sessionKey: "session-1", timeoutMs: 999 })).toThrow(
      /Invalid channel IPC payload/,
    )
  })
  ...
  it("accepts only bounded channel preview URLs", () => {
    ...
    expect(
      parseChannelIpcInput(channelPreviewOptionsSchema, { url: "https://example.com/qr", waitMs: 500 }),
    ).toMatchObject({ waitMs: 500 })
    expect(() => parseChannelIpcInput(channelPreviewOptionsSchema, { url: "file:///tmp/qr.png" })).toThrow(
      /Invalid channel IPC payload/,
    )
    expect(() =>
      parseChannelIpcInput(channelPreviewOptionsSchema, { url: "https://example.com", waitMs: 5001 }),
    ).toThrow(/Invalid channel IPC payload/)
  })
