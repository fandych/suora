...
├── store/appStore.ts        # Zustand グローバル状態（バージョ�?12�?
├── services/
...
**ストア名�?* `suora-store` · **バージョン：** 12 · **バックエンド�?* `~/.suora/data/`
...
```
Zustand �?fileStateStorage アダプタ�?�?IPC (store:load/save/remove) �?~/.suora/data/*.json
```
...
| スト�?| `store:load`、`store:save`、`store:remove` |
