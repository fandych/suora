...
├── store/appStore.ts        # Глобальное состояние Zustand (версия 18)
├── services/
...
**Имя хранилища:** `suora-store` · **Версия:** 18 · **Бэкенд:** `{workspace}/`
...
| Хранилище | `db:getSnapshot`, `db:loadPersistedStore`, `db:savePersistedStore`, `db:listEntities`, `db:saveEntity`, `db:deleteEntity` |
