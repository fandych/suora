...
├── store/appStore.ts        # Zustand 全局状态（版本 18�?
├── services/
...
**Store 名称�?* `suora-store` · **版本�?* 18 · **后端�?* `{workspace}/`
...
```
Zustand �?fileStateStorage 适配�?�?IPC (db:loadPersistedStore / db:savePersistedStore) �?{workspace}/{settings,models}.json + sessions/, agents/, channels/, …
```
