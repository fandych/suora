...
├── store/appStore.ts        # Zustand 글로벌 상태 (버전 18)
├── services/
...
**스토�?이름:** `suora-store` · **버전:** 18 · **백엔�?** `{workspace}/`
...
```
Zustand �?fileStateStorage 어댑�?�?IPC (db:loadPersistedStore / db:savePersistedStore) �?{workspace}/{settings,models}.json + sessions/, agents/, channels/, …
```
...
| 스토�?| `db:getSnapshot`, `db:loadPersistedStore`, `db:savePersistedStore`, `db:listEntities`, `db:saveEntity`, `db:deleteEntity` |
